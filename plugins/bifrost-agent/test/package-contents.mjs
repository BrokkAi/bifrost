...
const licenseNotices = [
  { packaged: "LICENSE.md", source: path.join(packageDir, "LICENSE.md") },
];
for (const notice of licenseNotices) {
  const packagedText = await fsp.readFile(path.join(packageDir, notice.packaged), "utf8");
  const sourceText = await fsp.readFile(notice.source, "utf8");
  assert.equal(
    packagedText,
    sourceText,
    `plugins/bifrost-agent/${notice.packaged} must be an exact copy of ${path.relative(repoRoot, notice.source)}`,
  );
}
