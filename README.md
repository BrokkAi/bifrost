...
Embeddings use `voyageai/voyage-4-nano`, downloaded from the HuggingFace hub on first use, and run in a PyTorch SDPA sidecar launched with `uv run scripts/voyage_sidecar.py`. Rust keeps the indexing pipeline and token counting in-process; the sidecar owns the model forward pass and selects CUDA, Apple Metal (MPS), or CPU at runtime. Environment overrides:

- `BIFROST_SEMANTIC_INDEX=auto` enables background indexing; the default is off
- `BIFROST_EMBED_MODEL_DIR`: local directory containing `config.json`, `tokenizer.json`, and `model.safetensors` (e.g. a fine-tune); takes precedence over the hub
- `BIFROST_EMBED_MODEL_ID`: alternate HuggingFace repo id
- `BIFROST_ACCELERATOR=auto|cpu|cuda|metal`: whether to advertise and start `semantic_search` based on the available accelerator (default `auto`); `cpu` hides the tool unless force-enabled with `--force-semantic-cpu`. This is a Bifrost tool-availability gate, not a CUDA device binding.
- `BIFROST_SIDECAR_DEVICES=<uuid|index,...>`: which devices the Rust scheduler should use. Bifrost launches one sidecar worker per listed device and sets that child's `CUDA_VISIBLE_DEVICES` value. If unset, it honors an existing `CUDA_VISIBLE_DEVICES` list; if that is also unset, it uses every GPU reported by `nvidia-smi`. If no CUDA GPU is visible, it launches one unpinned sidecar, which may use Metal or CPU.

The `nlp` cargo feature is opt-in; build with `--features nlp`. Without that feature, the `nlp` toolset publishes no tools and `core` degrades to `symbol|workspace`. There are no longer compile-time accelerator features; the sidecar selects CUDA, Apple Metal (MPS), or CPU at runtime.
