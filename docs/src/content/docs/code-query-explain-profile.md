...
- `work` reports budget-accounted files, source bytes, fact nodes, pipeline rows, references, provenance steps, resolved import files and edges, and semantic work. Its nested `semantic` object reports materialization attempts, unique materialized files, request-cache hits, semantic source bytes, procedures, program points, control edges, retained bytes, traversal steps, and whether the separate semantic budget was exhausted. When typestate runs, `semantic.typestate` adds solve/cache counts, reached and retained/omitted finding/witness counts, witness steps/bytes, solver termination counts, and finding-budget exhaustion; a pure `witness` projection adds no solve work.
