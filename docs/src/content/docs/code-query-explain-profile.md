...
- `physical_plan` maps each physical node to its logical node, selected operator, output kind, and ordered physical dependencies. A schema-v3 semantic step additionally carries `semantic_request`, with explicit `procedures`, `program_points`, and `control_edges` booleans describing the semantic facets it would need. Explain publishes that demand without materializing semantic artifacts. Logical and physical IDs are separate contracts even where the current planner selects them one-to-one.
...
- `work` reports budget-accounted files, source bytes, fact nodes, pipeline rows, references, provenance steps, resolved import files and edges, and semantic work. Its nested `semantic` object reports materialization attempts, unique materialized files, request-cache hits, semantic source bytes, procedures, program points, control edges, retained bytes, traversal steps, and whether the separate semantic budget was exhausted.
