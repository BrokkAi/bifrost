...
Bifrost's composable code-query engine is `query_code`. The compatible head is schema version 6: it retains version 3's source-backed procedure-local CFG and version 2's structural/declaration/reference/call/import/hierarchy/receiver steps, adds host-registered diagnostic-neutral typestate findings and bounded witnesses in version 4, declaration-bounded containment in version 5, and host-registered diagnostic-neutral value-flow endpoints and witnesses in version 6. It answers questions such as “find calls to this callee,” “which exact control edge leaves this entry?”, “does this registered resource protocol reach an error transition?”, and “which registered source can reach this sink?” across the active workspace.

Version 3's CFG surface remains deliberately procedure-local. Version 4 adds a narrow adapter from an exact procedure to an in-memory protocol/binding pair registered by the host, followed by a pure retained-witness projection. Version 5 adds `inside-decl` / `inside_decl`: containment that cannot cross a nested function, method, constructor, or lambda. Version 6 adds an equally narrow adapter from a procedure to a host-registered `ValueFlowPlan`; `value-flow` / `value_flow` returns typed endpoints and `witness` projects retained flow paths. Reachability, exact/may certainty, ambiguity, completeness, and budget termination remain orthogonal. No policy classification is performed. A query cannot load a protocol or flow plan from disk, change binding roles, compile policy severity, or invoke taint-policy semantics. Explicit version pins remain supported and reject later operations.
