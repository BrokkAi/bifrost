...
In VS Code, place the cursor or a selection inside an indexed declaration and run **Bifrost: Show Rune IR** from the editor context menu or command palette. The server reads the current unsaved overlay, preserves file-specific parser variants such as TSX, selects the smallest enclosing indexed code unit, and returns Rune IR plus starter RQL. The extension opens that server-rendered result as a **Bifrost Rune IR** document; `.rune` files use the same language mode and syntax highlighting. Preview headings are `;` comments, so saving the preview preserves a valid Rune IR document without introducing client-side structural vocabulary.
