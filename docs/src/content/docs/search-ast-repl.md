...
description: Use the experimental S-expression language for Bifrost's search_ast REPL.
---

RQL, the Rune Query Language, is the experimental S-expression adapter for Bifrost's `search_ast` query engine. It is designed for interactive use in the REPL:
...
## Relationship To search_ast

RQL is only a query language. It is not a second matcher or query engine.

Every RQL expression lowers into [JSON `AstQuery`](/search-ast-json/) before validation and execution. MCP hosts call the same engine through the `search_ast` tool, using JSON `AstQuery` directly. See [Code Querying](/code-querying/) for the schema and engine overview.

Use `:json` in the REPL to inspect the canonical JSON generated for the current RQL query.

## Syntax

RQL uses compact S-expressions:
...
RQL is not yet a stable external API. It is intended to make interactive exploration pleasant while preserving `search_ast` and JSON `AstQuery` as the stable integration surface.
