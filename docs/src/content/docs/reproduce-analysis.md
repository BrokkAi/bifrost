...
Replace example values with observed values; do not copy `0.8.2` into a future run without checking the binary. Record only environment variables that affect Bifrost behavior, and redact secrets before publication.
