...
Likewise, a reverse edge says that a file imports the target file—not that it calls `target`. To locate member calls once the relationship is known, use a separate structural call query or `scan_usages` for an indexed declaration. A future compositional facet can correlate import bindings with calls or member access while preserving that relationship as possible provenance rather than treating it as a synthetic symbol identity.
