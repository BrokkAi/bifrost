...
`query_code(...)` defaults to the compatible-head version-6 typed query surface. Pass `schema_version=2` for the pre-CFG vocabulary, `schema_version=3` for CFG without typestate, `schema_version=4` for typestate without declaration-bounded containment, or `schema_version=5` for containment without value flow. Pass exactly one source: positional `pattern`, `union=[...]`, `intersect=[...]`, or `except_=[...]`. Set operands are complete canonical plan dictionaries with compatible terminal domains; common `steps` run after composition. Supply ordered steps such as `[{"op": "enclosing_decl"}, {"op": "members"}, {"op": "references_of", "proof": "proven"}]` to return typed `CodeQueryReferenceSite` rows. Version 3 adds `procedure_of` and `cfg_*`; version 4 adds registered typestate plus bounded `witness`; version 5 adds `inside_decl`; version 6 adds `{"op":"value_flow","plan_ref":"embedding:request-to-sink"}` from a procedure and reuses `witness` for retained flow paths. The connected in-process host must pre-register the referenced protocol or value-flow plan for the current workspace. `CodeQueryFlowEndpoint` preserves reachability, exact/may certainty, ambiguity, completion, and budget status independently; it does not imply policy classification. `file_of` accepts every semantic source result and can feed import steps. See [Typed Set Composition](../code-query-tutorials/set-composition/), [Receiver Traversal](../code-query-tutorials/receiver-traversal/), [Reference Traversal](../code-query-tutorials/reference-traversal/), [Code Querying](../code-querying/), and [JSON CodeQuery](../code-query-json/) for the complete contract.

`CodeQueryResult.results` contains fourteen possible classes according to each item's `result_type`: `CodeQueryMatch`, `CodeQueryDeclaration`, `CodeQueryProcedure`, `CodeQueryProgramPoint`, `CodeQueryControlEdge`, `CodeQueryTypestateFinding`, `CodeQueryTypestateWitness`, `CodeQueryFlowEndpoint`, `CodeQueryFlowWitness`, `CodeQueryReferenceSite`, `CodeQueryCallSite`, `CodeQueryExpressionSite`, `CodeQueryReceiverAnalysis`, and `CodeQueryFile`. Typestate and flow models are frozen and strict about required identity/evidence fields and enum values. Findings and flow endpoints remain diagnostic-neutral; witnesses expose ordered `CodeQueryTypestateWitnessStep` or `CodeQueryFlowWitnessStep` values and truncation metadata. A receiver analysis row remains present for `unknown`, `unsupported`, and `exceeded_budget`; always inspect result-level `truncated` and diagnostics before consuming candidates. Compact output is the default and retains minimal provenance for derived results. Pass `result_detail="full"` when a rule, refactoring step, or follow-up tool call needs deterministic IDs, 1-based character columns, decorator ranges, and capture ranges.
