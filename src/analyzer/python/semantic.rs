...
impl ProgramSemanticsProvider for PythonAnalyzer {
    fn materialize(
        &self,
        file: &ProjectFile,
        ...
        self.inner
            .materialize_semantics_with_lowerer(&PythonSemanticLowerer, file, request)
    }
}
    ...
    fn lower(
        ...
        for spec in &specs {
            ...
            match lower_procedure(prepared, spec, &staged_budget, cancellation) {
                ...
                Err(PythonLoweringError::Cancelled(work)) => {
                    return Ok(SemanticOutcome::Cancelled {
                        partial: None,
                        work: sum_work(observed, *work),
                    });
                }
                Err(PythonLoweringError::Budget(exceeded, work)) => {
...
#[derive(Debug)]
enum PythonLoweringError {
    Cancelled(Box<SemanticWork>),
    Budget(SemanticBudgetExceeded, Box<SemanticWork>),
    Invalid(String),
}

impl From<SemanticBudgetExceeded> for PythonLoweringError {
...
impl<'tree, 'targets> LoweringContext<'tree, 'targets> {
    ...
    fn call_expression(
        ...
        let call_site = CallSiteId::new(
            u32::try_from(self.next_call_site)
                .map_err(|_| PythonLoweringError::Invalid("too many call sites".into()))?,
        );
        ...
        *occurrence += 1;
        let source = SourceMappingId::new(
            u32::try_from(self.next_source)
                .map_err(|_| PythonLoweringError::Invalid("too many source mappings".into()))?,
        );
        let evidence = EvidenceId::new(
            u32::try_from(self.next_evidence)
                .map_err(|_| PythonLoweringError::Invalid("too many evidence rows".into()))?,
        );
        let locator = SemanticLocator::new(
            self.locator.mount(),
    ...
    fn metadata(&self, point: ProgramPointId) -> Result<PointMetadata, PythonLoweringError> {
        self.point_metadata
            ...
            .copied()
            .ok_or_else(|| {
                PythonLoweringError::Invalid(format!("missing metadata for program point {point}"))
            })
    }
    ...
    fn value(
        ...
        let id = ValueId::new(
            u32::try_from(self.next_value)
                .map_err(|_| PythonLoweringError::Invalid("too many semantic values".into()))?,
        );
    ...
    fn add_gap(
        ...
        let id = SemanticGapId::new(
            u32::try_from(self.next_gap)
                .map_err(|_| PythonLoweringError::Invalid("too many semantic gaps".into()))?,
        );
