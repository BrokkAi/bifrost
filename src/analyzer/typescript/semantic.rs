//! TypeScript and TSX lowering into the shared executable-semantics IR.
...
const ADAPTER_VERSION: &[u8] = b"typescript-cfg-v1";
...
impl ProgramSemanticsProvider for TypescriptAnalyzer {
    fn materialize(
        ...
        self.inner
            .materialize_semantics_with_lowerer(&TypescriptSemanticLowerer, file, request)
    }
}

struct TypescriptSemanticLowerer;

impl ProgramSemanticsLowerer for TypescriptSemanticLowerer {
    fn identity(&self) -> SemanticAdapterIdentity {
        SemanticAdapterIdentity {
            adapter: AdapterSemanticsVersion::hash_bytes("typescript", ADAPTER_VERSION)
                .expect("adapter name is non-empty"),
            configuration: ConfigurationFingerprint::hash_bytes(
                b"typescript-intrafile-execution-defaults-v1",
            ),
...
fn typescript_capabilities() -> SemanticCapabilities {
    ...
    Some((
        ...
        ProcedureProperties {
            ...
            is_static: node.kind() == "class_static_block"
                || (node.kind() == "method_definition" && has_child_kind(node, "static")),
            is_synthetic: false,
        },
    ))
}
