...
- [x] (2026-07-22 22:20-05:00) Merged the current `origin/master` lineage, retained its broader compiler-builtin registry repair for #1015, repeated focused Python/Rust tests, all-feature Clippy, and the complete feature-enabled Cargo gate, then published integrated head `1ac755dd` directly to `origin/master`. Exact pushed-head probes proved all seven original Python witnesses consistent, and #1015 was closed with local and publication evidence.
- [x] (2026-07-22 23:05-05:00) The first complete post-publication Python replay removed all seven baseline rows but exposed fourteen additional legitimate #795 inverse gaps: five in Cirq and nine in Kornia. Oldskool audits independently accounted for every row. Thirteen were multi-hop namespace chains such as `K.feature.DISK`, `kornia.core.ops.eye_like`, and `cirq.testing.random_special_unitary`; the final row was the direct child-module qualifier `value` in `@value.value_equality`. The rejected artifact is `/mnt/optane/tmp/bifrost-fird/python-task-top5-1ac755dd-final.jsonl` (SHA-256 `aacb7a894935043fd54878513b8ce936015fa73011b50813abfb501a89573400`).
- [x] (2026-07-23 00:01-05:00) Completed the residual #795 implementation and dirty-head acceptance. Both target-specific and whole-workspace inverse scanners now walk tree-sitter attribute chains from an exact namespace-import root and canonicalize the complete path. The final Cirq row exposed a separate lexical-boundary defect: module scope facts recursively absorbed nested method parameters, so an unrelated parameter named `value` shadowed the module import at a class decorator. Module and function fact collection now stop at nested Python scopes while the separate class-field pass retains its intended recursive behavior. The 103-test targeted Python suite, 17-test whole-graph suite, the exact Cirq witness, and a complete persisted five-repository replay pass. The replay has zero missing rows, zero file errors, and no limits or truncations; JSONL SHA-256 is `8664615e41fc285339c098c407f899e6c574bb8f919f38db5acb8341333a6881`. Broad gates, publication, clean-head replay, issue closure, and the language summary remain.
- [x] (2026-07-23 00:43-05:00) Completed the Python publication boundary. After two concurrent `origin/master` integrations, formatting, 120 focused Python tests, isolated all-feature Clippy, and the complete `cargo test --features nlp,python` matrix passed at each shared-analyzer boundary. Integrated head `acd388c4` was pushed directly to `origin/master` and verified equal to the remote. Its clean release runner SHA-256 is `a0a2ff675ec92b51361367fc39f1ed46e7d8c851f56607c00db35afa67036752`.
- [x] (2026-07-23 00:43-05:00) Replayed all 21 formerly missing Python production sites at clean pushed head: 21 completed, zero missing, zero dirty flags, and zero file errors. The exact artifact SHA-256 is `2988e77b2b5b3e3aefd1184e93a6e5ba8ffe706b7a74d47aa01ff88cb38176f6`.
- [x] (2026-07-23 00:43-05:00) Completed the authoritative five-repository Python corpus. All five records are clean and completed with 40,105 sampled sites, 4,814 consistent, 5 unproven, 35,286 inconclusive, and zero missing; there are no file errors, candidate-limit failures, skipped targets, or target truncation. Corpus SHA-256 is `416ed1a025d9967e3fe1731a18f3c188935029de62877a10f9d84622cff99e99`. Posted evidence and closed solely assigned #795/#1096-#1098. See `.agents/docs/reference-differential/python-task-ranked-acd388c4-summary.md`.
- [x] Complete, publish, close owned issues for, and summarize the Python task-ranked leg.
...
- Observation: Python scope-fact extraction was structurally scoped by declaration ranges but recursively collected bindings from nested declarations inside those ranges.
  Evidence: Cirq's `@value.value_equality` resolved forward to module `cirq.value`, and the import binder correctly represented `from cirq import ..., value` as a namespace binding. The inverse scanner nevertheless reported `value` shadowed because a method parameter of the same name elsewhere in the decorated class had leaked into the containing module snapshot. A faithful grouped-import/class-decorator reduction failed before lexical-boundary collection and passes afterward.
...
- Decision: Resolve deep Python namespace references from tree-sitter attribute fields through the canonical export resolver, and make local binding inference explicitly lexical-scope aware.
  Rationale: The residual paths can contain any number of module/export hops, so immediate `module.symbol` matching is incomplete. Exact namespace binders plus structured attribute segments and canonical target comparison generalize without source splitting or same-name guessing. Shadow authority is equally structural: module and function snapshots must not inherit bindings from nested function, class, or lambda scopes, while class receiver-field inference remains a deliberately separate recursive pass.
...
Python is complete. The seven baseline rows and fourteen additional deep-namespace residuals are all absent at clean pushed source head `acd388c4`. All 21 exact production witnesses and the final five-repository corpus pass with zero missing rows, dirty flags, file errors, candidate-limit failures, skipped targets, or target truncation. Formatting, focused Python suites, isolated all-feature Clippy, and the complete feature-enabled Cargo gate pass after the final shared-analyzer integrations. Issues #795 and #1096-#1098 are closed with evidence. Final paths, hashes, per-repository counts, and implementation roots are recorded in `.agents/docs/reference-differential/python-task-ranked-acd388c4-summary.md`.
...
Revision note (2026-07-23): Completed the Python and overall task-ranked campaign boundary at pushed source head `acd388c4`: all 21 exact Python witnesses and all five final Python repositories have zero missing rows, the four assigned Python issues are closed, and all five languages governed by this plan now have durable summaries and accepted production evidence.
