...
Scala, Java, and Python remain. Historical fixes and issue closures in those languages remain regression inputs only until their task-ranked repository sets receive the same baseline, exhaustive triage, publication, and final-corpus treatment.

The three newly reproduced #661 inverse roots are locally implemented and behavior-tested. Their focused default-value, lexical-owner, and renamed-import regressions pass through both targeted and file-major scan paths, and the full Scala inverse graph suite passes 147 tests. Integration, production exact witnesses, publication, and final Scala corpus evidence remain part of the active Scala language boundary.

The three #664 wrong-forward roots are also locally implemented and behavior-tested. Exact nested constructor/named-field ownership, nonterminal import package boundaries with lexical declaration precedence, and anonymous refinement base-member precedence all pass their focused regressions; ambiguity, same-rendered-FQN decoys, qualified misses, and unindexed local templates fail closed. Integration with #661, broader gates, publication, issue closure, and the final Scala corpus remain root-owned.

#1073 is locally implemented through a licensed, provenance-recorded `tree-sitter-scala` derivative. Independent review regenerated the parser tables byte-for-byte and approved the packaging; the integrated raw-parser and public forward/inverse target passes 6 tests. The first persisted publication attempt exposed and now covers the required Scala analyzer-epoch cutover.

#1086 is locally implemented after a fresh ZIO row proved that same-FQN Scala 2/Scala 3 source replicas could erase anonymous-refinement inverse hits. The exact physical constructed base and direct type-member tier now remain authoritative, with public path-qualified coverage and an external-replica ambiguity barrier. Combined gates, publication, and the corrected final five-repository corpus remain active.
