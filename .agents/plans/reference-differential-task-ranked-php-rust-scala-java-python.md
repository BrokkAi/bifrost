...
#1073 is locally implemented through a licensed, provenance-recorded `tree-sitter-scala` derivative. Independent review regenerated the parser tables byte-for-byte and approved the packaging; the integrated raw-parser and public forward/inverse target passes 6 tests. All three Scala implementation tracks now pass their combined focused suites, with production replay and publication still active.
