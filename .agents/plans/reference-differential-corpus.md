...
- [ ] (2026-07-14) Filed #768 only after an inline pointer-field regression proved that unfielded `ERROR(identifier "__iomem")` inside `struct Reg __iomem *ip_serial_regs;` is persisted and resolved as pseudo-field `Port.__iomem`. The issue is assigned only to `jbellis`. Synchronous Oldskool implementation separates class-like discovery from repeated tree-sitter `declarator` fields, classifies those structural roles first, and consults a narrow explicit unfielded-declarator fallback only when no fielded declarator was handled; fielded ERROR recovery and #767's exact designator suppression remain intact. The 38 C++ analyzer, 56 C++ definition, 66 targeted usage, and 17 whole-workspace graph tests pass, as does affected all-feature clippy. A fresh one-file Git snapshot of the unchanged IOC4 production source proves both directions: `/tmp/c-exact-768-iomem-fixed.jsonl` changes bytes `16135..16142` to honest `no_definition`, while `/tmp/c-exact-768-field-fixed.jsonl` resolves `ip_serial_regs` bytes `25916..25930` to `ioc4_port.ip_serial_regs` with an identical exact inverse range and zero actionable findings. Push and pushed-head proofs remain before closure.
