...
- [ ] (2026-07-14) Filed #763 only after a consumer-only authoritative regression proved that forward lookup resolves `OwnerAlias *p; p->value` to `Owner.value` while inverse routing returns no proven hit. The issue is assigned only to `jbellis`. Synchronous Oldskool implementation canonicalizes the complete direct receiver-unit set through the existing structured, unique, cycle-safe alias path before member advance or target comparison, eliminating the nested-chain-only seam without a typedef fallback. The 59 C++ usage, 17 inverted graph, and 56 C++ get-definition tests pass, as does affected all-feature clippy. Dirty-tree `/tmp/c-exact-763-apps-fixed.jsonl` classifies RMerl `new_apps_info->new_file_name` bytes `1326..1339` as consistent with the identical inverse range and zero actionable findings; commit/push, pushed-head proof, and closure remain.
