...
- [ ] (2026-07-14) Filed #767 only after a faithful bounded PowerPC initializer regression reproduced the ten-site false-forward cluster: tree-sitter's later preprocessor recovery persisted `.cpu_features` as a bare global, so earlier valid designators resolved to that pseudo-declaration instead of `cpu_spec.cpu_features`. The issue is assigned only to `jbellis`. Synchronous Oldskool implementation suppresses only the exact adjacent ERROR-dot/init-declarator recovery before persistence, separates structured designator recognition from owner proof, and shares aggregate-owner resolution across forward lookup, targeted inverse, and whole-workspace inverted scanning. Root review required unresolved/nested designators to stop global fallthrough and remain explicitly unproven, and required recovered-only declarations to bypass fallback persistence. The 38 C++ analyzer, 56 C++ definition, 65 targeted usage, and 17 whole-workspace graph tests pass, as does affected all-feature clippy. A fresh two-file Git snapshot of the unchanged production sources avoids rebuilding the whole stale corpus cache; `/tmp/c-exact-767-cpu-features-fixed.jsonl` classifies PowerPC `cputable.c` bytes `7942..7954` as the exact `cpu_spec.cpu_features` field with an identical inverse range and zero actionable findings. Push and pushed-head proof remain before closure.
