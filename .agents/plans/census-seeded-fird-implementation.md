...
- [ ] Campaign: per-language depth-first passes over top-20 repos; file/fix/close/merge; per-language summary. Rediscovery audit against #1526/#1527/#1528/#1537/#1377 (#1376 already fixed and closed).
  ...
  - [x] (2026-08-07) #1638 (recursive self-call inverse drop) taken (assigned jbellis): mechanism is systemic - JVM/C#/PHP/Go drop `enclosing == target` hits at push time in each `hits.rs`, Rust/Python/JS-TS post-filter them. C++ already fixed it (#1664) via `push_recursive_reference_hit` -> `SelfReceiver` kind (editor-visible, external-excluded). Fix = generalize that contract to all languages; implementation delegated to Opus subagent with per-site brief; proven paths only, unproven drops left as-is.
