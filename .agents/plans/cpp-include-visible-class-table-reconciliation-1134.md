...
- [x] (2026-07-24) M4: `scan_usages` verified across the reconciliation — `file_scope_using_directive_nested_member_usage_is_scannable` scans the canonical `log4cxx.Outer$Inner.method` and finds the `scan.cpp` call site, since usage resolution flows through the same overlaid `definitions`. Sources/roles/canonical-selectors already proven by the flipped unification tests. The full cpp usage-graph suites (`usages_cpp_graph_test` 145, `usage_graph_cpp_test` 28) pass unchanged.
- [x] (2026-07-24) Final validation (partial): full cpp suites green (see Outcomes); `cargo fmt` applied; `cargo check --features nlp,python --lib` warning-clean. Remaining/blocked: `cargo clippy -D warnings` cannot run locally (`cc` `E0514` toolchain artifact, change-independent — see Outcomes); re-run in CI. reference-differential smoke run not executed (needs `--features nlp,python`, which fails to link libpython in this environment).
...
- (2026-07-24) Both #1134 shapes now unify end-to-end through `get_symbol_sources`, with the genuine `ns1::ns2::Klass::method` namespace chain unchanged and no epoch salt bump (the overlay is purely resolution-time). The fix is a single unified reconciler (`reconcile_out_of_line_member_identity`) plus a lazy `ReconciledDefinitionIndex` on `CppAnalyzer` and four coordinated `IAnalyzer` overrides (`definitions`, `get_definitions`, `ranges`, `signature_metadata`). The template-specialization shape passed on the first empirical run — evidence the "re-partition the whole qualifier against the class table" design generalizes across both shapes without shape-specific code.
- Regression sweep (default features): cpp lib 67; `issue_1121` 12; `issue_1093` 9 (one stale file-scope-using pin flipped to unification — that shape is exactly #1134); `issue_1092` 10; `cpp_analyzer_test` 43; `cpp_type_hierarchy_test` 16; `code_query_cpp_receiver` 13; `usage_graph_cpp_test` 28; `cpp_dead_code_smells` 20; `cpp_macro_call_arity` 7; `cpp_macro_sentinel_recovery_test` 10; `cpp_structural_clone_smells` 9; `cpp_test_assertion_smells` 18. All green.
- Lesson: the store keys *everything* by the stored `CodeUnit`, and decl/def pairing additionally needs `External` linkage + roles from `signature_metadata` — so a resolution-time overlay must map the re-keyed unit back on both `ranges` *and* `signature_metadata`, not just `definitions`. `occurrence_role` (AST-derived) and `canonical_selector` (grouped by canonical fq) need no mapping.
- Environment note: `cargo clippy` cannot run in this worktree — its build-script compile of the `cc` crate fails with `E0514` (an incompatible-rustc `cc` artifact) independent of these changes (it reproduces on a fresh isolated target and after `cargo clean -p cc`). `cargo check --features nlp,python --lib` is warning-clean (exit 0) and every test binary compiles, so the changes themselves are clippy-shaped-clean; the `-D warnings` gate should be re-run in CI or a clean local toolchain.
- Remaining: M4 audit of `scan_usages`/symbol-location surfaces for the two shapes (the shared `definitions`/`ranges` overlay likely already covers them, since usage/graph suites pass unchanged); no dedicated cross-surface test added yet.
