...
- [ ] M1: Pure reconciliation helper + unit tests (completed: —; remaining: implement `reconcile_out_of_line_member_identity` and its tests).
- [ ] M2: Reconciliation index in `CppAnalyzer` keyed on canonical `fq_name`, populated from ambiguous out-of-line definitions via `visible_type_units`.
- [ ] M3: Wire `get_definitions` to consult the index; flip the two pinned tests; bump the cpp epoch salt.
- [ ] M4: Extend reconciliation to the remaining resolution surfaces that unify decl/def (usages, occurrence roles, symbol locations) as needed and validate end-to-end.
...
- Decision: Implement the fix as a **resolution-time reconciliation overlay** (Option B) rather than rewriting stored `fq_name`s at extraction (Option A).
  Rationale: The issue mandates a class-table-keyed resolution-time reconciliation and states extraction is strictly per-file with no include resolution; the include-visible class table (`visible_type_units`) is an analyzer/resolution-layer structure, not available during per-file extraction. An overlay is additive, does not perturb persistence/epoch semantics of provisional identities beyond a salt bump, and matches the "no cross-file identity-reconciliation layer today — we are adding one" framing. Option A would couple extraction to a workspace-global structure and force the whole store through a re-key path.
