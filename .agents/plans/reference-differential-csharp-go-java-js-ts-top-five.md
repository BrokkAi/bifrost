...
- [x] (2026-07-20) Published and closed #968 after the exact Go partial-selector witness became clean at `e000d73d`; published and closed #944 at `89c45e9f` after same-binding JavaScript assignment/object-member reads became exact. Both release-built exact records are clean, strict, consistent, and have zero missing rows.
- [x] (2026-07-20) Completed the task-ranked TypeScript issue set. #963 was already fixed upstream and exact-proved for both Vue qualified-type witnesses at `89c45e9f`; #964 added canonical inverse seeds for anonymous JavaScript/TypeScript default exports and was published at `9dfe32ab`; #965 expanded intersection-alias receiver annotations through structured property-owner resolution and was published at `8b42786b`. All three issues are closed with clean exact evidence.
- [x] (2026-07-20) Published and closed C# #974 at `ff95321d`: logical partial-type ancestry now combines bases from every physical declaration, and the Terminal.Gui inherited `KeyBindings` witness is exact and consistent. Published and closed #971 at `a86ae862`: structured method-group value contexts now cover event/delegate assignments and initializers while preserving shadowing and overload ambiguity.
- [x] (2026-07-20) Published and closed C# #966 at `40474cd1`: object-initializer labels resolve the constructed type and select the nearest inherited member rather than requiring direct owner equality. Both Granit inherited-property witnesses are clean and exact. The publication gate included formatting, all-target/all-feature Clippy, the full serialized `cargo test --features nlp,python` suite, direct `origin/master` integration, and release-built proof.
- [x] (2026-07-20) Published and closed Go #970 at `7f99a891`: ordinary `=` writes preserve an unshadowed package-variable binding in both targeted and inverted scanners while `:=`, local `var`, and parameter shadows remain lexical. The clean Ollama exact record is consistent with zero actionable findings; formatting, Clippy, focused Go suites, and the complete serialized all-feature test suite passed before publication.
- [x] (2026-07-20) Published and closed shared runner issue #969 at `e59c36b8`: Go field/type declaration names and C# tuple labels are excluded only from the reference frontier, while semantic tokens and neighboring real references remain intact. Published exact probes reject both declaration sites before sampling and clean adjacent-reference controls are consistent.
- [x] (2026-07-20) Published and closed Java #977 at `384f3044` and #978 at `5f288b3b`: explicit single-type imports now constrain same-named lookup, package graph parents no longer masquerade as nested lexical types, and enclosing/inherited fields outrank static wildcard imports. Clean Dubbo and fastjson2 exact records prove the corrected identities with zero actionable findings.
- [x] (2026-07-20) Completed the task-ranked Java issue set. #976 now dispatches selector segments from structured AST roles; #979 records object creation and constructor method references while preserving receiver-type identity; #985 maps Lombok-generated accessors back to fields with arity and declared-method precedence; #986 resolves declaration-backed nested-type method-reference receivers; and #989 scopes try-resource bindings sequentially without leakage. All five are published and closed with clean LanguageTool/Halo exact records and zero actionable findings.
- [x] (2026-07-20) Completed the task-ranked Go issue set. #967 replaced dotted-text selector dispatch with tree-sitter focus roles, #968 retained legitimate partial-selector evidence, #969 excluded declaration roles from the runner, and #970 preserved package variables across ordinary assignment. All four are published and closed; clean zshellcheck/Ollama/cli exact evidence has zero missing findings.
- [x] (2026-07-20) Published and closed C# #972 at `1933b709` and #981 at `76f43eb6`: nested types now resolve through lexical/logical owners, and tuple elements distinguish the type reference from the same-spelled declaration name with an analysis-epoch bump. Clean Terminal.Gui and Mapperly exact records are consistent with zero actionable findings.
- [x] (2026-07-20) Published and closed C# #983 at `4029677b`: generic owner resolution now preserves arity when similarly named owners coexist. The clean Mapperly exact record is consistent with an exact inverse range and zero diagnostics.
- [x] (2026-07-20) Published and closed C# #973 at `9a60c098`: explicit and target-typed object initializers share structured owner inference, including the narrow tree-sitter recovery shape present in Terminal.Gui, and unknown owners fail closed. The clean exact `Shortcut.Key` witness is consistent with zero diagnostics or file errors.
- [x] (2026-07-20) Published and closed C# #980 at `c735ff72`: targeted and persisted inverse scans now recognize direct, qualified, global-qualified, and nested type operands in `nameof` while rejecting value shadows. Four clean Mapperly/CsvHelper exact records are consistent with exact inverse ranges.
- [x] (2026-07-20) Published and closed C# #984 at integrated head `0b05d428`, then cleanly exact-proved it at `94f402a7`: forward extension lookup now requires structured receiver-type evidence and matches the candidate's declared `this` type, so an external `System.Nullable` receiver cannot borrow ClosedXML's same-named local extension. The exact site now fails closed with `unsupported_csharp_receiver`, zero targets, and zero actionable findings.
- [x] (2026-07-20) Published and closed the final C# issue #982 at `6b03b910`: `base` dispatch now selects the unique logical direct class/record base across split partial declarations, excludes interfaces structurally, and applies call arity at each hierarchy level. The clean Mapperly witness is consistent with the exact inverse range; 105 targeted usage, 32 graph, and 543 definition tests pass.
- [x] (2026-07-20) File/assign, implement, review, test, and exact-prove every legitimate C# root cause not owned by another user.
