...
- [ ] Implement, review, and exact-prove the assigned C# correctness issues, then complete and integrity-check the C# top-five baseline and exhaustively classify Roslyn raw missing rows.
...
- Observation: The production-shaped #423 receiver witnesses do not fail in a persisted reduced project at the current head.
  Evidence: Duplicate physical declarations for `List<T>` and `NodeFactory` recover `argNames.ToArray()`, `parts.Reverse()`, and `factory.Target` through `WorkspaceAnalyzer::build_persisted`. All three queries share one global usage-index build and execute zero definition-candidate SQL queries. #423 remains open pending a clean exact corpus rerun or a more faithful failing reduction.
