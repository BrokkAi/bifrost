...
- [x] (2026-07-08 10:56Z) C++ milestone: implemented and registered C++ structural search, added focused coverage, ran checks, completed Brokk guided review, fixed accepted findings, reran checks, and prepared the milestone commit.
- [x] (2026-07-08 11:17Z) Rust milestone: implemented and registered Rust structural search, added focused coverage, ran checks, completed Brokk guided review, fixed accepted findings, reran checks, and prepared the milestone commit.
- [x] (2026-07-08 11:37Z) PHP milestone: implemented and registered PHP structural search, added focused coverage, ran checks, completed Brokk guided review, fixed accepted findings, reran checks, and prepared the milestone commit.
- [x] (2026-07-08 11:56Z) Scala milestone: implemented and registered Scala structural search, added focused coverage, ran checks, completed Brokk guided review, fixed accepted findings, reran checks, and prepared the milestone commit.
- [x] (2026-07-08 12:12Z) C# milestone: implemented and registered C# structural search, added focused coverage, ran checks, completed Brokk guided review, fixed accepted findings, reran checks, and prepared the milestone commit.
- [x] (2026-07-08 12:33Z) Ruby milestone: implemented and registered Ruby structural search, added focused coverage, ran checks, completed Brokk guided review, fixed accepted findings, reran checks, and prepared the milestone commit.
- [x] (2026-07-08 12:39Z) Final integration milestone: added full `Language::ANALYZABLE` shared-query coverage, confirmed unsupported-adapter diagnostics are gone, completed Brokk guided review, fixed accepted maintainability feedback, reran checks, and prepared the final cleanup commit.
...
- Observation: C++ constructor definitions cannot be identified by comparing node spans in `Service::Service()` because the scope and terminal name are distinct tokens with equal text. Tree-sitter still gives structured `qualified_identifier.scope` and `name` nodes, so constructor refinement can compare those node texts without parsing source syntax manually.
  Evidence: The C++ guided review found span comparison would miss constructors. The accepted fix compares the two structured node texts and covers `Service::Service() {}` with a constructor query.

- Observation: Rust expression-position signed numbers parse as `unary_expression` with an anonymous `-` token and a numeric child, while `negative_literal` exists for literal-pattern contexts. Mapping only `negative_literal` leaves assignment right-role kind queries unable to match `const LIMIT: i32 = -3`.
  Evidence: The Rust milestone test initially failed on `right: { "kind": "numeric_literal" }` for `const LIMIT: i32 = -3;`. The accepted fix maps `unary_expression` to `numeric_literal` only when tree-sitter child tokens prove it is a negative numeric wrapper, and suppresses the unsigned numeric child fact under that wrapper.

- Observation: PHP's `use_declaration` is trait composition inside class/enum bodies, not namespace importing. Namespace imports are represented by `namespace_use_declaration`, with grouped imports using a shared `namespace_name` prefix plus concrete `namespace_use_clause` leaves.
  Evidence: The PHP guided review found `use_declaration` would make `class Service { use Loggable; }` falsely match `kind: "import"`. The accepted fix removed `use_declaration` from the kind table, attaches module roles only from concrete namespace-use clauses, and covers grouped, aliased, full-qualified, and shared-prefix import queries.

- Observation: PHP expression-position signed numbers parse as `unary_op_expression` around numeric children, so numeric right-role queries miss signed literals unless the unary wrapper becomes the numeric fact.
  Evidence: The PHP guided review found `right: { "kind": "numeric_literal" }` would miss `LIMIT = -3`. The accepted fix maps `unary_op_expression` to `numeric_literal` only for structured `+`/`-` numeric wrappers and suppresses the wrapped unsigned child fact.

- Observation: Scala named arguments parse as `assignment_expression` nodes inside an `arguments` node, while real assignments use the same node kind elsewhere. Scala block-style calls parse as `call_expression.arguments = block` with a `lambda_expression` child.
  Evidence: The Scala guided review found the initial adapter would both expose `auditNamed(code = "named")` as a `kwargs.code` role and falsely emit it as an assignment fact. It also found calls such as `ListBuffer(1).foreach { value => audit(value.toString) }` needed non-parenthesized argument handling. The accepted fixes skip assignment facts under `arguments`, keep those nodes for `Role::Kwarg`, and attach block/case/colon call arguments through their single contained structural child when available.

- Observation: C# single-segment using directives parse as `using_directive` with an `identifier` target, while alias directives also expose the alias as the `name` field. C# null-conditional access parses as `conditional_access_expression` with a `condition` field and a `member_binding_expression` child.
  Evidence: The C# guided review found the initial adapter skipped `using System;`, made alias imports match the target terminal `Writer` as well as `WriterAlias`, and missed `service?.Run(...)` / `service?.Name`. The accepted fixes exclude only the alias node by id, expose only the alias binding for aliased usings, and map conditional access roles from `condition` plus binding `name`.

- Observation: Ruby bare method calls such as `audit` parse without the same call shape as explicit `audit()`, so the cross-language rollout fixture now uses `audit()` for the shared call query while the Ruby focused test covers ordinary call extraction in realistic method bodies.
  Evidence: The Ruby rollout test fixture was changed from `audit` to `audit()` and now returns a Ruby match with no unsupported-adapter diagnostics.

- Observation: Ruby import-like method names are only safe to refine as imports when they are bare calls, and interpolated string arguments cannot provide a precise module role.
  Evidence: Brokk guided review found `loader.require("plugin")` was initially refined as an import and `require "plugins/#{tenant}"` exposed the static prefix as a module name. The accepted fixes require no receiver for Ruby import refinement and only attach `Role::Module` names for fully static string nodes.

- Observation: Ruby qualified class/module declarations such as `class App::External` need terminal-name extraction for normalized declaration queries.
  Evidence: Brokk guided review found `name: "External"` would miss qualified declarations when the raw `scope_resolution` node was used as the fact name. The accepted fix routes declaration names through the Ruby expression-name helper.

- Observation: The final all-language integration test is most maintainable when one local table drives both fixture creation and expected rows while separately asserting that its labels equal `Language::ANALYZABLE`.
  Evidence: Final Brokk guided review found the first version duplicated the language universe between fixtures and expected rows. The accepted fix uses one `cases` table for file inputs and expected result rows.
...
- C++ milestone outcome (2026-07-08 / Codex): C++ now exposes a `StructuralSpec` through `CppAdapter::structural_spec()` and `CppAnalyzer::structural_search_providers()`. The adapter covers calls including scoped receivers and `new_expression`, assignments, field access, `#include` modules, class/struct/alias/function/method/constructor declarations, lambdas, identifiers, literals including `char_literal`, returns, throws, catches, conditionals, loops, and explicit unsupported diagnostics for kwargs/decorators. Brokk guided review found substantive gaps in out-of-line method classification, constructor classification, `new_expression`, scoped receivers, operator/destructor/dependent names, and `char_literal`; accepted findings were fixed and covered by focused assertions. The devops review also noted that the new `src/analyzer/cpp/structural.rs` file must be explicitly staged. The duplicate unquoted-span-helper note is recorded for later shared cleanup, but no milestone-blocking bug remained. Verification passed after fixes with `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language cpp_structural_adapter_matches_normalized_shapes -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language remaining_languages_report_missing_structural_adapters_during_issue_527_rollout -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test structural --lib`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_python --test structural_search_planner --test structural_search_cross_language`, `cargo fmt --check`, and `PATH=/Users/dave/.cargo/bin:$PATH cargo clippy-no-cuda`.

- Rust milestone outcome (2026-07-08 / Codex): Rust now exposes a `StructuralSpec` through `RustAdapter::structural_spec()` and `RustAnalyzer::structural_search_providers()`. The adapter covers calls including turbofish functions and generic method receivers, assignments including `let`, `const`, `static`, and compound assignment forms, field access, grouped `use` trees, class-like `struct`/`enum`/`trait` declarations, trait method signatures, function and method declarations, closures, identifiers, string/numeric/boolean literals including signed numeric spans, returns, conditionals, loops, and explicit unsupported diagnostics for kwargs/decorators. Brokk guided review found substantive gaps in trait method signatures, const/static initializers, signed numeric literals, turbofish calls, `let mut`/`let ref` patterns, grouped-use module roles, compound assignments, and generic method receivers; accepted findings were fixed and covered by focused assertions. The devops review also noted that the new `src/analyzer/rust/structural.rs` file must be explicitly staged. Verification passed after fixes with `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language rust_structural_adapter_matches_normalized_shapes -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language remaining_languages_report_missing_structural_adapters_during_issue_527_rollout -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test structural --lib`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_python --test structural_search_planner --test structural_search_cross_language`, `cargo fmt --check`, and `PATH=/Users/dave/.cargo/bin:$PATH cargo clippy-no-cuda`.

- PHP milestone outcome (2026-07-08 / Codex): PHP now exposes a `StructuralSpec` through `PhpAdapter::structural_spec()` and `PhpAnalyzer::structural_search_providers()`. The adapter covers calls including instance, nullsafe, static, object-creation, positional, and named-argument calls; assignments including variables, properties, constants, augmented assignments, and reference assignments; field/member access including instance, nullsafe, scoped static property, and class constant access; namespace imports including full-qualified, grouped, and aliased bindings; class/interface/trait/enum, function, method, constructor, and lambda declarations; PHP attributes as decorators; identifiers; string/numeric/boolean/null literals including signed numeric spans; returns, throws, catches, conditionals, and loops. Brokk guided review found substantive gaps in named arguments, namespace import binding semantics, trait `use` false imports, full namespace module matching, grouped import prefix overmatching, class constant field access, signed numeric literals, formatting, and explicit staging of the new file; accepted findings were fixed and covered by focused assertions. Verification passed after fixes with `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language php_structural_adapter_matches_normalized_shapes -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language remaining_languages_report_missing_structural_adapters_during_issue_527_rollout -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test structural --lib`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_python --test structural_search_planner --test structural_search_cross_language`, `cargo fmt --check`, and `PATH=/Users/dave/.cargo/bin:$PATH cargo clippy-no-cuda`.

- Scala milestone outcome (2026-07-08 / Codex): Scala now exposes a `StructuralSpec` through `ScalaAdapter::structural_spec()` and `ScalaAnalyzer::structural_search_providers()`. The adapter covers calls including generic functions, receiver calls, named arguments, and block-style arguments; assignments including `val`, `var`, and assignment expressions while excluding named arguments as assignment facts; field access; imports including full-qualified, grouped, and aliased selectors; class/object/trait/enum, method, function, and lambda declarations; annotations as decorators; identifiers; string/numeric/boolean/null literals including signed numeric spans; returns, throws, catches, conditionals, and loops. Brokk guided review found substantive gaps in explicit `method` capability support, named-argument assignment false positives, and non-parenthesized Scala call arguments; accepted findings were fixed and covered by focused assertions. Verification passed after fixes with `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language scala_structural_adapter_matches_normalized_shapes -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language remaining_languages_report_missing_structural_adapters_during_issue_527_rollout -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test structural --lib`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_python --test structural_search_planner --test structural_search_cross_language`, `cargo fmt --check`, and `PATH=/Users/dave/.cargo/bin:$PATH cargo clippy-no-cuda`.

- C# milestone outcome (2026-07-08 / Codex): C# now exposes a `StructuralSpec` through `CSharpAdapter::structural_spec()` and `CSharpAnalyzer::structural_search_providers()`. The adapter covers calls including object creation, member/static generic calls, named arguments, and null-conditional calls; assignments including variable declarators and assignment expressions while excluding uninitialized declarators; field/member access including null-conditional member access; `using` imports including single-segment, full-qualified, and alias bindings; class/interface/struct/enum/record, constructor, method, property declaration, local function, and lambda facts; attributes as decorators; identifiers; string/numeric/boolean/null literals including signed numeric spans; returns, throws, catches, conditionals, and loops. Brokk guided review found substantive gaps in single-segment `using` imports, alias target false positives, null-conditional call/member access, and explicit staging of the new file; accepted findings were fixed and covered by focused assertions. Verification passed after fixes with `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language csharp_structural_adapter_matches_normalized_shapes -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language remaining_languages_report_missing_structural_adapters_during_issue_527_rollout -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test structural --lib`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_python --test structural_search_planner --test structural_search_cross_language`, `cargo fmt --check`, and `PATH=/Users/dave/.cargo/bin:$PATH cargo clippy-no-cuda`.

- Ruby milestone outcome (2026-07-08 / Codex): Ruby now exposes a `StructuralSpec` through `RubyAdapter::structural_spec()` and `RubyAnalyzer::structural_search_providers()`. The adapter covers calls including receiver calls, keyword arguments, and block/lambda arguments; assignments including constants and signed numeric right-hand sides; `App::Service`-style field access; bare `require`/`require_relative`/`load`/`autoload` imports with static module names; module/class, top-level function, instance method, singleton method, and qualified class declarations; lambdas; identifiers; string/numeric/boolean/null literals; returns, rescues, conditionals, and loops. Ruby explicitly reports unsupported decorator roles. Brokk guided review found substantive gaps in receiver calls named `require` being misclassified as imports, dynamic import strings exposing partial module names, qualified class declarations missing terminal-name queries, a stale rollout test name, duplicated child-kind scans, and lost coverage for the generic no-provider diagnostic path; all accepted findings were fixed and covered by focused assertions or restored planner coverage. Verification passed after fixes with `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language ruby_structural_adapter_matches_normalized_shapes -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language remaining_languages_search_without_unsupported_adapter_diagnostics_during_issue_527_rollout -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_planner -- diagnostic --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_planner formerly_unsupported_languages_are_searched_after_adapter_registration -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test structural --lib`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_python --test structural_search_planner --test structural_search_cross_language`, `cargo fmt --check`, and `PATH=/Users/dave/.cargo/bin:$PATH cargo clippy-no-cuda`.

- Final integration outcome (2026-07-08 / Codex): `tests/structural_search_cross_language.rs` now has `shared_call_query_matches_every_analyzable_language_without_adapter_diagnostics`, which builds one inline fixture for every current `Language::ANALYZABLE` language and runs the same normalized call query for `audit`. The test asserts empty diagnostics, exactly one match per analyzable language, the case table's labels equal `Language::ANALYZABLE`, and the returned language/path/text rows match the fixtures. Brokk guided review found one low maintainability issue around duplicated language tables; the accepted fix made one local `cases` table drive both the files and expected rows. Verification passed after fixes with `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_cross_language shared_call_query_matches_every_analyzable_language_without_adapter_diagnostics -- --nocapture`, `BIFROST_SEMANTIC_INDEX=off cargo test structural --lib`, `BIFROST_SEMANTIC_INDEX=off cargo test --test structural_search_python --test structural_search_planner --test structural_search_cross_language`, `cargo fmt --check`, and `PATH=/Users/dave/.cargo/bin:$PATH cargo clippy-no-cuda`.
...
Revision note, 2026-07-08: Updated after the C++ milestone to record the new adapter coverage, accepted guided-review findings, rerun validation commands, and C++ constructor-refinement discovery.

Revision note, 2026-07-08: Updated after the Rust milestone to record the new adapter coverage, accepted guided-review findings, rerun validation commands, and Rust signed-literal grammar discovery.

Revision note, 2026-07-08: Updated after the PHP milestone to record the new adapter coverage, accepted guided-review findings, rerun validation commands, and PHP import/signed-literal grammar discoveries.

Revision note, 2026-07-08: Updated after the Scala milestone to record the new adapter coverage, accepted guided-review findings, rerun validation commands, and Scala grammar-shape discoveries.

Revision note, 2026-07-08: Updated after the C# milestone to record the new adapter coverage, accepted guided-review findings, rerun validation commands, and C# using/conditional-access grammar discoveries.
