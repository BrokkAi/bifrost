...
- [x] (2026-07-30 16:56Z) Completed cross-producer integration tests, public documentation, schema regeneration, all five specialist reviews and follow-up fixes, focused Rust validation, strict featureless Clippy, docs checks/build, and clean-consumer package validation.
- [ ] Run the required combined `bifrost.code-smells` repository policy gate after the Bifrost plugin contributes `list_policies` and `run_policy` to a fresh task; the current task has neither tool and launcher doctor reports that the required 0.8.16 binary is missing.
...
- Observation: Clippy can select Homebrew `cargo-clippy` while the active Cargo and rustc come from `/Users/dave/.local/bin`, producing incompatible crate metadata despite the same Rust commit string because the LLVM builds differ.
  Evidence: two isolated Clippy attempts failed with E0514; rerunning the isolated gate with `/opt/homebrew/bin/cargo`, rustc, and clippy on one matched PATH passed with `-D warnings`.
...
- Observation: specialist review found that source-level Java names, Java wildcards/compiler-generated members, C# by-ref signatures/events, caller-controlled identity namespaces, malformed signature handling, and archive/metadata allocation limits could all make an apparently complete pack semantically incomplete or unstable.
  Evidence: follow-up fixtures now compare imported generics, records, enums, implicit constructors, and a legal top-level `$` class across source/class JARs; C# retains by-ref and event facts; IDs use canonical `jvm`/`cli` namespaces and return types; lowered signature depth, ZIP directory preflight, record budgets, signature cardinality, array rank, heap-copy budget, and early diagnostic bounds are covered by focused tests.
- Observation: the installed policy skill does not imply a callable policy service.
  Evidence: neither `list_policies` nor `run_policy` is present in this task, and `bifrost-launcher.mjs doctor` reports `status=missing required=0.8.16`. The skill explicitly forbids substituting a CLI policy run, so the repository policy gate remains unverified until a fresh task has a registered compatible MCP server.
...
- Decision: Derive declaration IDs from semantic identity and exclude origin information.
  Rationale: equivalent source and binary declarations must merge predictably. Type IDs use producer-owned canonical semantic universes (`jvm` and `cli`) plus normalized fully qualified identity rather than caller metadata. Member IDs contain owner, kind, name, generic arity, ordered parameter types, and return type so distinct CLI conversion operators cannot collide. Artifact paths, source paths, metadata tokens, parameter names, and digests do not participate.
  Date/Author: 2026-07-30 / Codex
...
Milestone four implementation is complete. Architecture, security, intent, duplication, and DevOps reviews drove a hardening pass over identity, structured type fidelity, source/class parity, generated Java API surface, malformed metadata behavior, and resource caps. The final focused evidence includes two producer-bound tests, eleven C# tests, four Java producer tests with real `javac`/`jar`, two cross-producer semantic integration tests, exact schema drift coverage, formatting, matched-toolchain strict featureless library Clippy, docs check/build with 5,561 internal links, and all five packaged clean-consumer configurations. The only incomplete acceptance item is the MCP policy run: the tools are absent and the required plugin binary is missing, so no clean policy result is claimed.
...
Stable declaration IDs allow facts from different origins to refer to the same semantic declaration. The helper introduced by this plan hashes a canonical, length-framed identity key under a versioned domain and renders a lowercase stable ID. Type keys contain the producer-owned canonical JVM or CLI universe and normalized fully qualified type identity. Member keys contain the owner ID, member kind, normalized member name, generic arity, ordered parameter types, and return type. Return type distinguishes valid CLI metadata such as conversion operators; parameter names remain excluded because binary names are optional. The hash framing reuses the repository's existing canonical hashing helpers rather than building ad hoc concatenated strings.
...
Give both producers malformed, oversized, deeply nested, and count-limited inputs. They must not panic, recurse without a bound, invent declarations, or turn analyzer construction into an error. When safe facts remain, the result contains a partial pack and bounded diagnostics. When the exact artifact cannot be read or identified, the result contains no pack and a bounded fatal diagnostic when the caller's diagnostic budget permits; otherwise the suppressed count records it. The compatibility index treats either result as no external declarations.
...
Revision note (2026-07-30 16:56Z): Recorded milestone four implementation, all specialist-review fixes, final focused/docs/package evidence, the canonical JVM/CLI and return-sensitive identity corrections, and the single remaining policy-tool registration blocker.
