...
Milestone 4i is complete in the working tree. The C/C++ adapter now enumerates functions, methods, constructors, destructors, operators, lambdas, and executable initializer fragments, then lowers the common core plus switch fallthrough, goto, C VLA bounds, explicit throw, handlers, and known cleanup boundaries through the same immutable CFG and demand-materialized ICFG as all ten preceding languages. Structured header/implementation coalescing and template-qualified resolution preserve one exact call transfer while configuration, default arguments, implicit conversions and calls, temporary and RAII cleanup, exceptions, spawn, coroutines, `noexcept`, preprocessor selection, evaluation-order latitude, and platform extensions remain capability- and point- or call-site-scoped gaps. Shared ICFG review also made return evidence path-sensitive, made every unresolved boundary non-complete, and retained exact unsupported capabilities. The common provider now serves every `Language::ANALYZABLE` value; no language-specific CFG storage, resolver facade, or ICFG implementation was introduced. Milestone 5 is the remaining #817 lifecycle measurement.
