...
Plan revision note (2026-07-18): Completed Milestone 4a. JavaScript and JSX now route through a flavor-aware shared JS/TS semantic lowerer without changing TypeScript/TSX fingerprints or topology. The first all-language conformance cases prove exact CFG adjacency, matched ICFG returns, deterministic rendering, dialect identity, stable class-field-arrow naming, resource initializer order, terminal `for using`/`for await using` boundaries, and point-scoped JavaScript resource, async, generator, and JSX gaps. C# is the next independently reviewed checkpoint.
