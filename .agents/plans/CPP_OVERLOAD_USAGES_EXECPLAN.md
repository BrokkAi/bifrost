...
- [x] Milestone 1: shared `cpp_call_match.rs` helpers + dedup of signature param parsing, with inline unit tests (name/unit/pointer-depth/unknown-arg/empty-fallback). Evidence: `BIFROST_SEMANTIC_INDEX=off cargo test --lib cpp_call_match` passed 5/5; the guard command `BIFROST_SEMANTIC_INDEX=off cargo test --test usages_cpp_graph_test --test get_definition_test` passed before behavior changes with 338 get_definition tests and 43 C++ usage graph tests.
- [x] Milestone 2: get_definition narrows the fixture lookups (new tests in tests/get_definition_test.rs, written to fail first). Evidence: `cpp_same_arity_free_function_overloads_filter_by_argument_type` failed against baseline source with `format(first)` returning all four overload units, then passed after the implementation.
- [x] Milestone 3: forward scan excludes proven non-target overload call sites (new tests in tests/usages_cpp_graph_test.rs, including the auto-from-virtual-call direction and the unknown-argument conservative guard). Evidence: `cpp_graph_filters_same_arity_free_function_overloads_by_argument_type` failed against baseline source because the string overload included `parity::format(7)`, then passed after the implementation; `cpp_graph_keeps_unknown_argument_overload_calls_conservative` passes and proves the fallback keeps an unresolved field argument on both overloads.
- [x] Milestone 4: fmt + clippy (long-form command) clean; the four named test suites green; optional usagebench run recorded here. Evidence: `cargo fmt` completed with no diff after formatting; `cargo clippy --all-targets --features nlp,python -- -D warnings` finished clean; `BIFROST_SEMANTIC_INDEX=off cargo test --test usages_cpp_graph_test --test get_definition_test --test usage_graph_cpp_test --test cpp_analyzer_test` passed with 45 C++ usage graph tests, 339 get_definition tests, 12 usage_graph C++ tests, and 33 C++ analyzer tests. The optional usagebench clone/run was skipped as planned because it is non-gating and network-dependent.
...
- The C++ analyzer normalizes signature display text as `(std::string &)` rather than preserving `std::string&`; tests that select overloads by signature should use a robust fragment such as `std::string`.
- Temporarily reversing only the source-code patch while keeping the new tests in place proved the failure mode before the fix: get_definition returned all four `parity.format` units for `format(first)`, and scan_usages for the string overload included the `parity::format(7)` call.
...
- **Use `CppType { name, unit: Option<CodeUnit>, indirection, alias_unit }` in get_definition.** The name is always the normalized type spelling, while `unit` is present only for indexed workspace types. Receiver resolution requires `unit`; overload filtering can compare `name` when either side is unindexed.
- **Type signed numeric literals through unary plus/minus wrappers.** tree-sitter-cpp represents `-3` as a `unary_expression` around a `number_literal`; the shared literal helper unwraps `+` and `-` and applies the same int/double rules to the inner literal. Other unary expressions remain unknown.
- **Implemented the structured auto-from-method-call path.** The extractor now supplies a bindings-backed receiver resolver to `infer_cpp_initializer_binding`, so `auto first = base.handle("Ada")` receives the unindexed return name `std::string` without changing `inverted.rs`.
- **Ordinary method calls use the same overload argument filter; explicit operator calls remain on the old path.** Operators are explicitly out of scope for this issue, and keeping that path unchanged avoids broadening the change beyond same-arity free functions and normal method calls.

## Outcomes & Retrospective

All four milestones are complete. The shared `cpp_call_match.rs` module now owns C++ signature parameter extraction, literal typing, pointer-depth handling, and the set-level overload filter. `get_definition` now carries named-but-unindexed C++ types, so primitive and standard-library signatures such as `int` and `std::string` can disambiguate same-arity overloads without inventing source-text fallbacks. The forward scanner now carries the same name-aware binding information and filters ordinary free-function and method call sites when argument typing conclusively selects a different overload.

The canonical parity fixture is covered in both directions. In `tests/get_definition_test.rs`, `cpp_same_arity_free_function_overloads_filter_by_argument_type` verifies that `parity::format(first)` resolves only to the string overload declaration plus source definition, and `parity::format(7)` resolves only to the int overload declaration plus source definition. In `tests/usages_cpp_graph_test.rs`, `cpp_graph_filters_same_arity_free_function_overloads_by_argument_type` verifies that scanning the string header declaration returns exactly the source definition and `format(first)`, while scanning the int declaration returns exactly the source definition and `format(7)`. The same suite's `cpp_graph_keeps_unknown_argument_overload_calls_conservative` proves the intended fallback: unresolved arguments still keep arity-only behavior.

Known gaps remain by design: constructor overloads still conflate by arity; string-literal arguments never disambiguate; explicit operator calls remain on the old scanner path; `usage_graph` remains overload-agnostic by `fq_name()` node key; `CppQueryResolver` still scans only `overloads.first()` for an ambiguous selector. The optional usagebench run was not performed.
