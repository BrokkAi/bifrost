...
The final all-feature run exposed no #816 regression. Cache-coupled tests pass against fresh schema-v8 databases without mutating the primary worktree's schema-v9 cache; the two tests whose contract is the default cache-root selection also pass when rerun without the suite-wide override. Four assertions remain red only on the stale issue-branch base and are the exact cases corrected by upstream `0955e1c7`. Publication is now authorized as a ready-for-review pull request; integrating the current base remains a review/merge action rather than unfinished semantic-oracle work.
