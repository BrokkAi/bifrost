...
- [x] (2026-07-22 11:16+02:00) Rebased the publication branch onto `origin/master` at `917f874b` and reconciled the semantic cache and workspace-query paths without regressing either side: `CompleteValueCache` retains atomic acquisition plus snapshot-aware keying, and Results, Explain, and Profile modes all execute through the workspace-aware query path. The post-rebase architecture audit found and closed two remaining work-amplification seams. Source projection now charges and checks cancellation for every candidate and observation traversal, while receiver setup, neutral semantic projection, and compatibility decoration share one aggregate ledger. Semantic work dimensions are classified once, including nested source candidate/observation visits as scope traversal, and CodeQuery partitions its one scalar fact cap across scope and summary dimensions rather than granting the cap twice. A SearchTools regression proves ordinary and Profile execution return the same precise Java receiver fact. The two source-projection regressions, eleven receiver-query tests, eighteen semantic cache/keying tests, focused CodeQuery, SearchTools, and LSP tests, formatting, diff checks, and isolated strict all-target/all-feature Clippy pass; the serialized all-feature publication gate is running against the rebased tree.
...
Issue #816 has completed its implementation, review, measurement, and branch-local validation milestones. Its contract/dispatch checkpoint, shared-lowering milestone, reference-language value-fact milestone, production value-flow/call-binding milestone, bounded heap-oracle milestone, receiver/dispatch-refinement milestone, measured lifecycle decision, and post-review schema check are complete on the issue branch. Semantic IR schema v6 now includes neutral declaration-backed dispatch extensibility in addition to the finite evidence-backed oracle vocabulary; exact query, dispatch-arm, and relation-arena ownership; structured direct/spread/unclassified arguments and one/rest formals; proof-aware grouped call bindings; bounded access paths, contexts, retained arenas, and partial target projection; strong-update proof obligations; explicit dispatch coverage; typed gap impacts; complete artifact-key validation; stable cancellation semantics; and a separate workspace semantic facade. The lowering milestone removes the repeated emission engine from all language adapters while preserving their syntax and topology ownership.
...
The publication rebase integrates the upstream navigation and workspace-query changes while preserving atomic semantic-cache acquisition and snapshot-aware keying. The final architecture audit additionally closes every identified budget-multiplication seam: source projection charges every traversal, receiver phases consume one aggregate ledger, and CodeQuery projects its scalar fact cap across semantic dimensions once. Focused regressions and strict all-target/all-feature Clippy pass on the rebased tree; the serialized `nlp,python` publication gate is the final running check before the ready-for-review pull request.
...
Plan revision note (2026-07-22): Recorded the publication rebase onto `origin/master` at `917f874b` and the final architecture cleanup. The integrated tree preserves atomic generation-aware cache acquisition and all workspace execution modes, charges and cancels every source-projection traversal, applies one receiver work ledger across all phases, classifies source-candidate visits as scope work, partitions CodeQuery's scalar fact cap once, and proves Results/Profile parity before publication.
