...
Issue #816 remains open, but its contract/dispatch checkpoint, shared-lowering milestone, reference-language value-fact milestone, and production value-flow/call-binding milestone are complete on the issue branch. The checkpoint freezes semantic IR schema v5; finite evidence-backed oracle vocabulary; exact query, dispatch-arm, and relation-arena ownership; structured direct/spread/unclassified arguments and one/rest formals; proof-aware grouped call bindings; bounded access paths, contexts, retained arenas, and partial target projection; strong-update proof obligations; explicit dispatch coverage; typed gap impacts; complete artifact-key validation; stable cancellation semantics; and a separate workspace dispatch facade. The lowering milestone removes the repeated emission engine from all language adapters while preserving their syntax and topology ownership.

TypeScript/JavaScript and Java production artifacts now contain exact formal and receiver ports, source-backed expression values, scope-distinct locals, assignment and local/parameter/receiver/return flows, direct and spread call actuals, results and thrown values, object and closure-environment allocations, indexed locations and loads/stores, and lexical receiver captures into exact child-procedure slots. `WorkspaceSemanticOracle` projects those rows into bounded, evidence-backed relation arenas and exact candidate-specific boundary bindings without source reparsing or name/range matching. The shared fixture proves block shadowing, Java sibling-scope same-name negatives, competing branch definitions, static receiver absence, factory construction, call and return wiring, indexed memory, capture-environment consistency, direct/rest mappings, receiverless targets, open defaults/spreads, cap truncation, budget partials, and cancellation. The next concrete milestone is bounded heap, alias, and update-eligibility queries; later milestones still need receiver compatibility, dispatch refinement, and measurement before #816 itself can be accepted.
