...
Milestone 3 is complete. The shared coordinator now loads suppressions through the same workspace capability as policy dependencies, evaluates every requested policy before joining exact strong finding IDs, and records invalid input as a batch diagnostic without skipping analyzer work. Schema-2 reports retain a deterministic evaluation context, active suppression metadata on retained findings, and one orthogonal audit review for every loaded record. Current strong matches are excluded from the failure threshold; expired, weak-current, absent, unselected, incomplete-policy, policy-hash-drift, and omitted-result states remain distinct. Applied suppressed results receive retention priority, and audit preflight or result-retention exhaustion disables or explicitly marks suppression state and makes the batch unreliable.

Validation for this checkpoint: `cargo check --lib`; `cargo check --all-targets`; `cargo fmt --all -- --check`; matching-toolchain `cargo clippy --all-targets -- -D warnings`; all 16 report-model unit tests; all 9 coordinator unit tests; all 8 suppression-evaluation integration tests; all 15 suppression-loading tests; all 13 policy-match tests; all 8 policy-doc tests; all 4 JSON/human rendering tests; all 14 SARIF rendering tests; and `git diff --check`. The suppression integration suite proves identity stability across unrelated lines and policy presentation drift, reactivation after selected-source edits or expiration, invalid-input unreliability, unselected/incomplete audit states, and unchanged analyzer work apart from retained report-byte accounting. Renderer-specific presentation and host flags remain for milestones 4 and 5.
