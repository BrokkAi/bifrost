...
- [ ] Milestone 2a implementation: implement the shared capability-rooted loader plus workspace-safe policy/endpoint/selector loading and explicit transactional match-directory traversal.
- [ ] Milestone 2b implementation: implement category/exact endpoint selection, explicit catalog registration, syntactic composition, and stored resolved taint/typestate specifications.
- [ ] Milestone 2c implementation: implement schema/directory/endpoint manifests, supersedes validation, deterministic source/semantic/projection hashes, collision handling, and bounded immutable registry authority.
- [ ] Milestone 2 validation/review/checkpoint: run loader/catalog/hash suites on normal, Unicode, Windows-shaped, and symlink paths; fix accepted review findings; update this plan; and checkpoint the milestone.
- [ ] Milestone 3a implementation: make `CodeQueryResult` completeness machine-readable at every diagnostic origin.
- [ ] Milestone 3b implementation: add canonical report/run/finding/evidence/identity types and incremental report budgets.
- [ ] Milestone 3c implementation: execute location-bearing match policies without context-free query-to-diagnostic conversion.
- [ ] Milestone 3 validation/review/checkpoint: run diagnostic-origin, completeness, match-domain, range, fingerprint, and budget suites; fix accepted review findings; update this plan; and checkpoint the milestone.
- [ ] Milestone 4a implementation: validate #824's dominance-projected endpoint facts, implement unique finding-combination precedence, generated flow messages, terminal-violation projection, and generic broad/refined classification from exact evidence facts.
- [ ] Milestone 4b implementation: implement evidence-backed CVSS v4.0 reduction, coherent variants, exact provenance, and scored versus unscored results.
- [ ] Milestone 4 validation/review/checkpoint: run classification/CVSS/variant/score/license suites; fix accepted review findings; update this plan; and checkpoint the milestone.
- [ ] Milestone 5a implementation: implement bounded deterministic human and canonical JSON writers.
- [ ] Milestone 5b implementation: implement the bounded private SARIF DTO/writer and offline schema validation.
- [ ] Milestone 5c implementation: add the repeatable one-shot CLI, thresholds, partial-report rules, atomic output, and exact exit behavior.
- [ ] Milestone 5 validation/review/checkpoint: validate every report gold offline against SARIF, run CLI threshold/status/output tests and the checked end-to-end transcript, fix accepted review findings, update this plan, and checkpoint the milestone.
- [ ] Milestone 6a implementation: add the distinct `.rqlp` editor language, schema validation, hover, formatting, and grammar.
- [ ] Milestone 6b implementation: add executable documentation, canonical fixtures, and the checked preview page.
- [ ] Milestone 6 validation/review/checkpoint: run LSP/VS Code/docs/link tests, visually inspect a fresh docs preview, fix accepted review findings, update this plan, and checkpoint the milestone.
- [ ] Milestone 7 adversarial review/fixes: audit the complete public and implementation boundary listed below, resolve every accepted finding with a regression, and update this plan with the final contract.
- [ ] Milestone 7 final gates/checkpoint: run the complete feature-enabled Rust, Python, editor, docs, SARIF, dependency/license, and whitespace matrix; record exact evidence and handoff; and make the final reviewed checkpoint without pushing or opening a PR.
...
- Observation: the live #709/#824 prose still overlaps on who expands catalogs and performs classification, while the detailed roadmap needs one owner to avoid duplicate reducers.
  Evidence: this plan and the synchronized umbrella assign syntactic catalog registration/composition plus the generic classification/CVSS reducer to #709, and semantic selector compilation plus analysis-specific evidence/adaptation to #824. Before implementation reaches those seams, mirror that exact split into both issue bodies (or record maintainer acknowledgement in their discussion); do not let the stale wording drive a second implementation.
...
Canonical JSON is the one deterministic JSON serialization of a typed value after validation and normalization. A source hash covers the original bytes and therefore changes with comments or layout; a semantic hash covers canonical typed meaning and therefore does not. Provenance is bounded evidence explaining where a result came from. A DTO is a private data-transfer record used only to serialize a standard wire shape.
...
The sink form has `:role sink`, an exact matched-value/receiver/return/argument binding, and optional `(sink-semantics :accepts [...] :tags [...] :impacts [...])`. Role and taint-semantics variant must agree. Source labels and sink accepts are required when taint semantics is present; typestate-only endpoints may omit taint semantics. `display-name` is a bounded phrase used verbatim in generated messages. Categories are sorted/deduplicated exact identifiers. `supersedes` is a sorted duplicate-free endpoint-ID set; the selected endpoint universe must contain every referenced ID and be acyclic. The endpoint semantic hash covers resolved schema and selector versions, role, canonical selector, binding, categories, taint semantics, and supersedes. Its separate analysis-projection hash covers only resolved query semantics, role, typed binding, taint transfer semantics, and behavior-bearing supersedes declarations; it excludes names, display/description/help text, categories, source bytes, filesystem identity, and directory provenance. Category changes may alter which endpoints a policy selects and therefore its resolved policy hash, but cannot invalidate a solver summary when the selected analysis projections are otherwise identical.
...
| Match directories | 64 refs per consumer; recursion depth 32; 4,096 candidate endpoint leaves; 64 categories/ref; optional one SHA-256 manifest pin | Transactional load error; no partial set |
...
Implement typed-to-canonical-JSON serialization and a source-preserving, registry-driven S-expression formatter. Keep the shared generic/RQL default output unchanged; the RQLP formatter uses 100 columns by default, never separates a field keyword from its value, lays out large tagged-record vectors predictably, and preserves version omission. Canonical JSON always materializes resolved versions. Canonicalize semantic sets by stable ID and preserve only sequences whose order is a contract, such as classification refinements and witness steps. Add `tests/policy_source.rs` with exact endpoint/match/taint/typestate examples, generated-message/combination/terminal shapes, canonical JSON golds, parser/formatter idempotence, comments, Unicode, incomplete buffers, schema/variant errors, limits, and nested RQL diagnostic ranges. Test omitted policy -> 1 and RQL -> 2, explicit/implicit semantic-hash equality with source/origin inequality, explicit unsupported no-fallback behavior, test-only compatible versus explicit-only successors, and all four `rql-file` precedence cases. Store full-document formatting golds at 80, 100, and 120 columns. Update this plan, inspect the milestone diff, run formatting and focused tests, and make a multiline checkpoint commit containing only this milestone when implementation begins under this ExecPlan.
...
- Match, taint, and typestate examples parse into a tagged `PolicyAnalysis` whose canonical JSON contains exactly `analysis.type = match|taint|typestate`; endpoint canonical JSON is the distinct document variant. Fields owned by another document/analysis variant fail at their field range.
...
- Inline selectors remain native AST children. Inline and file selectors lower to equal `CodeQuery` values and semantic hashes; schema origin and referenced source identity/path remain provenance only. Neither is reparsed during evaluation.
...
`OrganizationalRiskAssessmentHash` is a lowercase 64-hex SHA-256 newtype over the normalized assessment content. Before sorting, selecting, or retaining an overlay, the evaluator compares the input length with `PolicyBudget.max_organizational_risk_overlays`; at most 64 are accepted by the schema-version-1 hard cap. Exceeding the effective bound retains no organizational-risk assessment and makes the affected run inconclusive with `PolicyIncompleteReason::OrganizationalRiskOverlayBudget`, with a structured diagnostic and CLI status 2 rather than silently discarding context. For an in-budget set, apply the same scope partial order as CVSS and retain the one assessment at the unique maximal applicable scope; byte-identical/hash-identical duplicates are idempotent. Different assessments at the same or incomparable maximal scopes fail the run with `ConflictingOrganizationalRiskOverlay` rather than depending on slice order. Lower scopes remain unused context and are not merged. This keeps the single `PolicyFinding.organizational_risk` field deterministic; it never feeds severity, CVSS, identity, or solver state.
    ...
    impl<'a> DefaultPolicyEvaluator<'a> {
        pub fn new() -> Self;
        pub fn with_taint(mut self, adapter: &'a dyn TaintPolicyEvaluator) -> Self;
        pub fn with_typestate(mut self, adapter: &'a dyn TypestatePolicyEvaluator) -> Self;
    }

The named `new`/`with_taint`/`with_typestate` methods are the normative construction surface: each builder replaces only its corresponding explicit trait object while preserving the other. The following dispatch contract uses those methods; no alternative constructor shape is left to #824.
...
Add runtime dependencies `cap-std = { version = "4.0.2", default-features = false }` for capability-relative, race-resistant workspace reads and `cvss = { version = "2.2.0", default-features = false, features = ["std", "v4"] }` for scoring. Keep SARIF serialization on existing Serde/`serde_json` and stable hashing on existing `sha2`. Add `jsonschema = { version = "0.48.0", default-features = false }` under dev-dependencies; the test constructs `jsonschema::draft4` directly, needs no file/HTTP resolver, and never performs network access. Any dependency change must update `Cargo.lock`, `licenses/THIRD_PARTY_LICENSES.html`, and supplemental notices when the repository generators say they changed; never hand-edit generated license contents.
