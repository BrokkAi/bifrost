...
- [x] (2026-07-31) Milestone 6 complete, with two scope changes the owner approved. The hand-written stack is **kept** rather than deleted, and after a brief period defaulting to `rmcp` (`906debfb`) the default was returned to the hand-written host: `rmcp` is opt-in via `BIFROST_MCP_RMCP=on` until it has run against real clients. See the Decision Log.
...
- Observation: **`rmcp` destroys the message arrival ordering that Bifrost's Roots lifecycle depends on between handlers, but one public seam restores it: the transport.** The serve loop reads messages in order, but it *spawns a task* for every notification and every request, and it resolves a server-to-client response by waking the task that awaits it. Three things Bifrost needs ordered against each other - a `roots/list` answer, a `notifications/roots/list_changed`, and a `tools/call` - therefore race. Both directions are observable: a `list_changed` can lose to the answer it is meant to supersede (so a revoked root briefly becomes analyzer scope), and a `tools/call` sent immediately after a roots answer can win against the answer (so the client gets a spurious "not bound to a workspace"). The hand-written host was immune because a single reader thread processed every message in arrival order; that is what `McpConnectionState::pending_roots_request` and `roots_refresh_requested` were built on. Deferring the bind to first use (recording roots instead of binding them) removes the *filesystem* damage from the first direction, but does not remove the second. Measured after that fix: 4 of 8 full-suite runs fail on `rootless_mcp_binds_to_client_roots_without_analyzing_process_cwd`. `Service::invalidate_response_cache` is the only *handler-adjacent* hook the loop awaits inline, and it is a `ServiceRole` method, so it cannot be overridden per server -- which is what first suggested an upstream change was required. It is not, because `Transport::receive` is documented as sequential and `ServiceExt::serve` accepts any `IntoTransport`: a wrapping transport observes every inbound message in wire order before the loop dispatches or spawns anything. See `crates/bifrost-mcp/src/ordered_transport.rs` and the resolution in `Decision Log`.
  Evidence: `rmcp-3.0.1/src/service.rs:1588` spawns notification handlers via `spawn_service_task`; `rmcp-3.0.1/src/service.rs:1510` shows `invalidate_response_cache` as the only inline-awaited per-notification hook. Failure transcript: `assertion left == right failed: {"error":{"code":-32603,"message":"Bifrost is not bound to a workspace..."},"id":5}` at `crates/bifrost-mcp/tests/bifrost_mcp_server.rs:2904`, immediately after the client answered `roots/list` with the replacement root.

- Observation: `rmcp` refuses any pre-`initialize` request other than `ping` and ends the session, where the hand-written host answered a pre-`initialize` `tools/call` with an ordinary "not bound to a workspace" error. The SDK is right and the new behavior is strictly stronger for Bifrost: Codex sandbox metadata is never even interpreted before capability negotiation. `rootless_mcp_binds_from_codex_sandbox_state_and_revokes_per_call_scope` was updated to assert the stronger contract on its own short-lived server.
  Evidence: `rmcp-3.0.1/src/service/server.rs:494-517`; the session ends with `MCP initialization failed: expect initialized request, but received: ... CallToolRequest`.

- Observation: `rmcp` permits a duplicate `initialize` and simply updates peer info. Bifrost must not: the handshake is what decides whether a connection may bind workspaces from client roots or from Codex sandbox metadata, so a second `initialize` would let a peer re-negotiate that authority mid-session. The handler refuses it explicitly.
  Evidence: `rmcp-3.0.1/CHANGELOG.md` 1.8.0, "update peer info on duplicate initialize"; the contract suite asserts "duplicate initialize must not replace the connection authority".
...
- OPEN QUESTION (blocks Milestone 6): how should the legacy `2025-11-25` Roots lifecycle behave now that `rmcp` no longer processes messages in arrival order? Three candidates, in the author's order of preference:

  1. **Pull the roots exchange into the request that needs it.** Stop asking for roots from `on_initialized`. When a tool call arrives at a rootless server whose client advertised Roots and which has no workspace, issue `peer.list_roots()` *inside that `call_tool` handler* and await it there. The answer is then consumed by the only request that cares, so nothing races. This mirrors what MRTR does for `2026-07-28` and unifies the two revisions onto one shape. Cost: a client that refuses to answer `roots/list` until an outstanding tool call returns would deadlock, and `rootless_mcp_binds_to_client_roots_without_analyzing_process_cwd` asserts a tool call issued mid-refresh returns "not bound" rather than waiting, so that assertion would have to change to "waits and then serves the new scope". That is arguably better product behavior - it removes a spurious error class - but it is a behavior change that needs an owner's sign-off.

  2. **Accept the ordering window and relax the two assertions.** Cheapest, and wrong: it ships a real defect. Under load, a client that answers `roots/list` and immediately calls a tool receives "Bifrost is not bound to a workspace" roughly half the time in the measured suite. That is exactly the failure agents would report as flaky code intelligence.

  3. **Ask upstream for an ordered notification hook**, or implement `Service<RoleServer>` directly if a future `rmcp` exposes in-order dispatch. Correct but not actionable today.

  RESOLVED 2026-07-30 by the repository owner: candidate 1. Implemented in `BifrostMcpHandler::activate_workspace_from_client_roots`, and `rootless_mcp_binds_to_client_roots_without_analyzing_process_cwd` was rewritten around the new shape, keeping every security property it asserted. Measured effect on the full suite: from 4 failures in 8 runs down to 2 in 16.

  RESIDUAL CLOSED 2026-07-30, and without an upstream change. `Transport::receive` is documented as sequential and `ServiceExt::serve` accepts any `IntoTransport`, so a wrapper around `rmcp::transport::stdio()` sees every inbound message in wire order, synchronously, before the serve loop dispatches or spawns anything. `crates/bifrost-mcp/src/ordered_transport.rs` counts `notifications/roots/list_changed` there; a binding records the count it was made under, and both the request path and the notification handler test the binding against the current count. Ordering is therefore decided by the wire, not by which task the runtime polls first, which restores exactly the guarantee the single-reader-thread host had. `on_roots_list_changed` is now only a promptness optimization -- it stops in-flight analyzer work early -- and it carries the same staleness guard, because it can be scheduled after a tool call has already re-negotiated a fresh scope and must not tear that one down. Measured: 15 consecutive clean full-suite runs, from 4 failures in 8 before any of this work.

  The lesson worth keeping: `rmcp` gives no ordering between handlers, but it does give one ordered seam, and it is the transport.

  Milestone 6 is therefore unblocked.
...
- Decision: `rmcp` ships opt-in behind `BIFROST_MCP_RMCP=on`, not as the default. An earlier revision of this milestone made it the default; the owner reversed that, and the reversal is the more conservative reading of the same caution that kept the old host alive at all. Flipping the default is now a deliberate follow-up step, and deleting the hand-written stack the one after it, both recorded in `AGENTS.md`.
  Rationale: the `rmcp` host is complete and the contract suite exercises both, but no configured adapter has driven it yet, and a protocol stack that has never met a real client is not something to make the default on the strength of tests alone. Opt-in gets it in front of users who choose it while the fallback stays untouched.
  Date/Author: 2026-07-31, David Baker Effendi (decision), Claude (implementation).

- Decision (superseded): Milestone 6 flips the default to `rmcp` but keeps the hand-written host behind `BIFROST_MCP_RMCP=off` instead of deleting it.
