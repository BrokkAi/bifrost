...
- [ ] BLOCKED, needs an owner decision: the legacy Roots lifecycle is not ordering-safe under `rmcp` (see `Surprises & Discoveries` and the open question in `Decision Log`). 4 of 8 full-suite runs fail on `rootless_mcp_binds_to_client_roots_without_analyzing_process_cwd`. Everything else in milestones 2 through 4 is done.
...
- OPEN QUESTION (blocks Milestone 6): how should the legacy `2025-11-25` Roots lifecycle behave now that `rmcp` no longer processes messages in arrival order? Three candidates, in the author's order of preference:
  ...
  RESIDUAL, still open: one ordering dependency survives, because it is between a notification and a *following* request rather than between a response and a request. `notifications/roots/list_changed` and the `tools/call` after it are both spawned tasks; `rmcp` spawns the notification first but cannot guarantee it is polled first, so a tool call can occasionally read the binding before the revocation lands and be served once from the scope the client just revoked. Nothing outside `rmcp` can close this: the only per-notification hook the serve loop awaits inline is `ServiceRole::invalidate_response_cache`, which is not a handler method. Closing it inside Bifrost would mean re-asking the client for roots on every tool call, which costs a client round trip per call and is not acceptable for interactive latency. The options are to accept and document the window, or to ask upstream for either an in-order dispatch mode or an overridable inline notification hook. This must be decided before Milestone 6 flips the default.

  Until the residual is resolved, the `rmcp` host stays behind `BIFROST_MCP_RMCP` and the hand-written host remains the default, so nothing shipped is affected.
