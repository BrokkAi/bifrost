...
- [ ] File or reuse issues assigned only to `jbellis`, delegate substantial implementations to Oldskool, root-review each change, and prove fixes with focused tests and exact production reruns. (Completed: filed and assigned #924 through #932 and committed reviewed fixes for all nine issues. #925's final source-ordered macro cursor passed a 1,000-call linear-scale proof after the first quadratic design was rejected. #931 provides one set-valued, macro-aware bare-call resolver shared by forward, targeted, and whole-graph analysis. #926 extends that resolver with lazy, name-filtered ordinary/directive import tiers; its concurrent scale gate proves one project source-index pass, zero absent-name expansion, bounded requested-name inspection, and one logical callable-metadata build. Exact clean-head production witnesses for #924 and both #928 shapes are consistent. Remaining: run the outstanding production proofs and reduce only recurring final residual families.)
