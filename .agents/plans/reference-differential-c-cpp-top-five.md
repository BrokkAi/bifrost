...
- [x] (2026-07-19 05:55Z) Filed and assigned #924 through #932 only to `jbellis`, delegated and root-reviewed substantial implementations, and committed all nine fixes. Every issue now has an acceptance-grade exact production or causal performance proof. Final-head probes made #925 and #931 consistent with exact inverse hits, retained #932's exact intended constructor conservatively as unproven with zero actionable discrepancy, made #927 consistent, and proved #929's 113-candidate path uses one prepared parse and zero manual parses.
- [x] (2026-07-19 05:55Z) Passed `cargo fmt`, all-target/all-feature Clippy with warnings denied, all focused suites, and the complete unrestricted `cargo test --features nlp,python` suite at clean integrated head `f7a2bb56`. The first sandboxed suite attempt had 1,040 library tests green and only three process-I/O tests denied by sandbox policy; the unrestricted rerun passed completely.
- [ ] Publish the integration checkpoint directly to `origin/master` without waiting for CI, rebuild from that exact clean pushed head, and freeze it for the final corpus.
...
- Observation: Integrating constructor/type adjudication against the shared visibility machinery exposed regressions that focused fixture success alone would have missed.
  Evidence: #931's first integrated form reintroduced a later same-FQN free function during constructor canonicalization, and its first full definition run dropped an out-of-line definition whose same-signature header declaration was visible. Type-filtered canonicalization plus logical-symbol activation keyed by kind, FQN, and signature fixed both; the final patch passes 137 usage tests, 498 definition integration tests, and 26 definition unit tests.

- Observation: Exact production probes found cross-fix integration failures after every isolated issue suite was green.
  Evidence: The first integrated #925 Tree-sitter witness still returned no definition because a split `extern "C"` preprocessor wrapper failed declaration visibility, and #931 initially regressed unknown-arity macro overloads plus QGIS exported-class identity. Structured wrapper recovery, bounded unproven overload candidates, and displaced exported-class ownership now pass the combined seven macro, 138 C++ usage, and 498 definition tests. Final clean-head probes are consistent for the Tree-sitter macro and QGIS direct temporary, while the default-argument witness retains the exact intended inverse as explicitly unproven rather than manufacturing certainty.
...
The strongest interim conclusion is that the Chromium work generalized: large C is nearly closed, and C++ misses concentrate in a small number of cross-repository resolver environments rather than recurring Chromium-only parser recovery. The baseline also exposed two concrete performance costs: the shared visibility batch's OceanBase setup and repeated per-candidate class-strength parsing (#929). All nine filed root-cause families (#924--#932) now have committed fixes with focused behavior, persistence, scale, and acceptance-grade production or causal proofs. Exact clean-head reruns moved the FreeBSD #924 witness and both RavynOS #928 witnesses from missing to consistent. Final-head proofs made #925, #927, and #931 consistent, kept #932's intended constructor and exact inverse site conservatively unproven with zero actionable discrepancy, and reduced #929's 113 sibling classifications to one prepared parse with no fallback parsing. #926 was the decisive implementation because explicit import witnesses conservatively account for 315 of 529 genuine rows; its first eager and first precedence designs were rejected before the final lazy direct-before-directive implementation passed the adversarial matrix, 138 usage tests, 498 definition tests, concurrent scale gate, formatting, all-feature Clippy, and the complete feature-enabled test suite. This section remains incomplete until the clean all-ten confirmation, issue closure, reports, and final master synchronization are complete.
...
Revision note (2026-07-19 05:55Z): Recorded the fully integrated nine-issue fix stack, cross-fix production regressions and their structured repairs, final-head acceptance proofs for #925/#927/#929/#931/#932, and successful formatting, all-feature Clippy, focused, and complete feature-enabled test gates before the authoritative all-ten rerun.
