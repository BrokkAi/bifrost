...
- [x] (2026-07-14) Optically aligned the shared `.rql`/`.rune` Explorer icon by translating its unchanged artwork 12 pixels right on the 512-pixel transparent canvas (about 0.4 pixels at Explorer size). A raw-pixel comparison proves translation was the only image change, and all 50 VS Code tests pass.
...
Revision note (2026-07-14): Completed the final Explorer polish by optically shifting the shared RQL/Rune file icon right without redrawing or resizing it; verified the exact pixel translation and reran the extension suite.
