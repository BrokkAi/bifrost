...
After the final rebase, strict isolated all-target/all-feature Clippy is warning-free and every ordinary `cargo test --features nlp,python` target passes, including 1,856 library tests, 193 LSP tests, 28 MCP tests, 631 definition tests, 135 semantic-language conformance tests, and the large language usage-graph suites. The command's final doctest invocation initially selected Homebrew `rustdoc` while compilation used rustup `rustc`; those binaries report the same Rust release but different LLVM builds and reject each other's metadata. A fresh isolated doctest run pinned to rustup's matching `rustdoc` passes with zero doctests, confirming an environment-path mismatch rather than a source failure.
