...
- [ ] Commit the final post-validation sweep, refresh and reconcile with current `origin/master`, open one pull request that closes #1107 and #1108 through #1115, wait for green CI, and squash-merge it.
...
Before the final publication rebase, strict isolated all-target/all-feature Clippy was warning-free and every ordinary `cargo test --features nlp,python` target passed, including 1,856 library tests, 193 LSP tests, 28 MCP tests, 631 definition tests, 135 semantic-language conformance tests, and the large language usage-graph suites. The command's final doctest invocation initially selected Homebrew `rustdoc` while compilation used rustup `rustc`; those binaries report the same Rust release but different LLVM builds and reject each other's metadata. A fresh isolated doctest run pinned to rustup's matching `rustdoc` passed with zero doctests, confirming an environment-path mismatch rather than a source failure. On the final `bb95e973` rebase, strict Clippy passed and the first all-feature run reached the definition target before exposing the upstream C++ lexical-scope interaction; after the same-source correction, all 633 definition tests pass and the complete all-feature rerun remains the last local publication gate.
