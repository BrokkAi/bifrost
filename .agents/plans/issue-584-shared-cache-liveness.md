...
MCP CI repair note (2026-07-10): x86 Windows exposed two integration tests that compared serialized canonical paths, where `canonicalize()` legitimately returns a verbatim `\\?\` spelling. The tests now share a canonical-directory assertion, so they verify the workspace identity returned by the spawned server without assuming a Windows path representation. This is test-only; no production normalization behavior changed.

MCP service CI repair note (2026-07-10): the same representation assumption remained in the `searchtools_service` workspace activation tests. All serialized `workspace_path` assertions in that module now use one canonical-directory helper; activation arguments and unrelated path-rendering assertions remain unchanged.
