...
- Observation: prepared requests need to retain registrations independently of later alias removal.
  Evidence: `PreparedQueryCode` now captures a cloned immutable `ProtocolRegistrationSet`; the service lifecycle test prepares a query, unregisters the only alias, and still executes through the retained snapshot while the live store releases its accounted bytes.
