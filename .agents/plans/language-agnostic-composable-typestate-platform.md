...
The first three post-reference rollout checkpoints validate that boundary from reuse and independent-adapter directions. JavaScript/JSX share the TypeScript lowering family; C# and Python map their own structured syntax into the same builder, exact-location dispatch oracle, and context-bearing ICFG. Python adds an orthogonal invocation-timing property so coroutine and generator object creation returns to the caller without falsely entering suspended bodies. Conformance now covers direct calls, common control, nested callables, handlers and cleanup, advanced-feature gaps, grammar-specific evaluation order, and deferred call boundaries without adding language-specific graph services.
