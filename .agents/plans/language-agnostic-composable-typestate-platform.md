...
The first six post-reference rollout checkpoints validate that boundary from reuse and independent-adapter directions. JavaScript/JSX share the TypeScript lowering family; C#, Python, Go, Rust, and PHP map their own structured syntax into the same builder, exact-location dispatch oracle, and context-bearing ICFG. Python and Rust use orthogonal invocation timing so deferred coroutine/generator construction never becomes false immediate body entry. Go keeps scheduled and selected work distinct from immediate calls, Rust uses shared cleanup markers to expose unknown Drop behavior without fabricating destructor topology, and PHP owns whole-chain nullsafe continuations without changing graph mechanics. Conformance now covers direct calls, common control, nested callables, handlers and cleanup, advanced-feature gaps, grammar-specific evaluation order, deferred call boundaries, scheduling boundaries, destructor uncertainty, nullish flow, and chain-scoped short circuiting without adding language-specific graph services.
