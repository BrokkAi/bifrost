...
- [x] (2026-07-15T16:13:34Z) Validated the checked-in corpus, ran 24 full-checkout scenarios on pinned Gson, Gin, and Ky repositories with zero comparator regressions, passed formatting and warnings-as-errors Clippy, and passed the complete `nlp,python` library, integration, and doctest suite with the host's matching rustup tools.
...
- Observation: the representative regression harness found no scenario-specific regressions after all promoted changes. Full pinned Gson, Gin, and Ky runs covered 24 scenarios, including workspace construction, imports/relevance, usages, hierarchy, TypeScript structural queries, and dead-code consumers; every scenario succeeded and all three comparisons reported zero regressions over the 20% plus 50 ms threshold.
  Evidence: candidate reports `run-20260715T160158Z.json`, `run-20260715T160235Z.json`, and `run-20260715T160307Z.json` compare cleanly against the 2026-07-15 checked-in Ubuntu baseline. Broad local speedups are treated as host variance rather than attributed to this branch.

- Observation: full `nlp,python` validation on this macOS host needs both PyO3's CI linker flags and matching rustup `rustc`/`rustdoc` executables. The initial sandboxed run exposed expected child-process and git permission failures; the unsandboxed rerun passed every library and integration test, and a matching-rustdoc pass completed the doctest gate.
  Evidence: `RUSTFLAGS='-C link-arg=-undefined -C link-arg=dynamic_lookup'` resolves extension-module symbols; the final doctest command also sets `RUSTDOC=/Users/dave/.rustup/toolchains/1.96.0-aarch64-apple-darwin/bin/rustdoc` to avoid the previously documented Homebrew/rustup metadata mismatch.
...
- Decision: finish issue #748 with shared compact storage scoped to immutable read snapshots, while retaining maps for mutable discovery, rich semantic identity, and query-local singular relations.
  Rationale: the promoted structural-role, import, and common reverse-hierarchy paths all freeze dense rows and show material measured value. RQL's incremental resolver map, usage resolver/catalog maps, final weighted usage edge vector, Go/Rust specialized hierarchy indexes, and per-file ownership path either encode lifecycle/identity semantics or failed the promote threshold. A universal graph object would erase those useful boundaries without evidence.
  Date/Author: 2026-07-15 / Codex
...
Milestone 6 outcome 2026-07-15T16:13:34Z: the checked-in manifest validates all 10 supported languages and 11 required scenarios. Full pinned runs covered nine Gson, seven Gin, and eight Ky scenarios; all 24 succeeded, and each report compared to the current checked-in Ubuntu baseline with zero threshold or actionable regressions. Formatting and all-target/all-feature Clippy with warnings denied pass. The full `cargo test --features nlp,python --jobs 1` library and integration run passes outside the sandbox with macOS PyO3 linker flags; the matching-rustup doctest rerun also passes. Issue #748 therefore ends with three promoted shapes: direct compact structural role rows, reusable compact rows plus bidirectional file CSR/CSC, and exact-identity one-way reverse hierarchy CSR. Ownership CSC and weighted usage adjacency are discarded from production. Mutable discovery maps and rich identity catalogs remain domain-owned, then freeze into compact snapshots only where measurements justify it.
...
Revision note 2026-07-15T16:13:34Z: Completed the representative regression suite and full validation, classified the promoted and discarded candidates, and closed the experiment with compact storage limited to measured immutable read snapshots.
