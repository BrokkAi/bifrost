...
- Decision: model one npm package installation as one `ResolvedDependency` containing its package manifest plus every selected declaration entry point in deterministic entry-point order.
  Rationale: package metadata, exports routing, and declaration bytes jointly determine the public API. Binding all of them into one generated-production key makes an entry-point or metadata change invalidate exactly that package and prevents equal-rank packs for a single installed version.
...
Create `crates/bifrost-analysis/src/analyzer/js_ts/external.rs` and export its public adapter/discovery types from `crates/bifrost-analysis/src/analyzer/js_ts/mod.rs` and `crates/bifrost-analysis/src/analyzer/mod.rs`. Implement `resolve_js_ts_semantic_pack_dependencies(config, project, limits, cancellation) -> DependencyDiscoveryOutcome` with the same cancellation and bounded-diagnostic semantics as the JVM/C# resolvers. Discover only supported local lockfiles, initially `package-lock.json`, `npm-shrinkwrap.json`, `pnpm-lock.yaml`, and `yarn.lock`; parse each format structurally enough to extract an installed package name, version, and package-root evidence. If a format cannot express a unique installed path/version relationship, report a bounded `npm.lockfile.incomplete` diagnostic instead of guessing.
