...
- [x] (2026-08-02 11:02Z) Milestone 3: proved discovery, preparation, generated-catalog reuse, exact activation, source-locator navigation, symbol/source/signature/hierarchy lookup, version filtering, per-package regeneration, and workspace-file isolation through the existing shared overlay.
- [x] (2026-08-02 11:24Z) Milestone 4: added and ran an ignored 250-interface measurement at commit `c31358c7`, recording complete cold generation, warm reuse, activation, lookup, compiled bytes, catalog bytes, retained bytes, artifacts, and records in `.agents/docs/issue-1349-npm-declaration-pack-measurement.md`.
- [x] (2026-08-02 12:18Z) Ran formatting, focused producer/discovery/overlay/dependency-coordinator tests, featureless strict Clippy with the coherent rustup toolchain, and an acceptance-focused diff review; all of those checks pass.
- [ ] The required `bifrost.code-smells` policy gate remains `unreliable`: repository-wide policies exhaust structural discovery budgets even though the final reports contain no findings in changed files. Timing and timeout evidence is recorded on #1398.
...
- Observation: strict Clippy initially failed before checking Bifrost source because `cargo` and `rustc` resolved through rustup while the `cargo clippy` subcommand resolved the same-version Homebrew binary with different LLVM metadata.
  Evidence: putting `/Users/dave/.rustup/toolchains/1.96.0-aarch64-apple-darwin/bin` first in `PATH` made featureless strict Clippy complete cleanly; the rustup compiler reports LLVM 22.1.2 while the Homebrew compiler reports LLVM 22.1.6.
- Observation: the Scala producer precedent and live issue text require source declaration locators, not binary-style artifact locators or temporary `node_modules` paths.
  Evidence: TypeScript facts now retain deterministic package-module paths such as `widget.d.ts` and symbols containing byte spans; the generic source and location tools expose those authored external locators while `Project::all_files()` still excludes dependency files.
- Observation: the built-in policy pack cannot currently produce a trustworthy repository-wide result on this workspace.
  Evidence: the first run exceeded the interactive deadline after 5.022 seconds; the final warm rerun took 2.446 seconds but remained `unreliable` because five performance policies exhausted structural discovery budgets. No final-run finding targets a changed file. The exact timeout evidence was added to existing issue #1398.
...
Milestones 1 through 4 are complete. Root npm `package-lock.json` and `npm-shrinkwrap.json` files with a version-two/three `packages` table now resolve exact installed package name/version evidence into one dependency per declaration entry point. Discovery supports `types`, `typings`, static `exports` type targets, conventional `index.d.ts`, scoped packages, and `@types` import-name mapping while ignoring workspace-only lock entries. It rejects version/name disagreement, unsafe or escaped paths, wildcard/ambiguous exports, missing declarations, malformed/oversized metadata, and cancellation with explicit incomplete diagnostics. The declaration producer structurally emits deterministic module/type/member/signature/hierarchy facts for supported exported and ambient TypeScript syntax, including local re-export aliases, namespace and interface merging, class-plus-namespace members, global augmentation, and local hierarchy IDs that power reverse reference paths. The full integration fixture prepares two packages, activates them through the shared runtime, navigates source declaration locators, exercises symbol/source/signature/hierarchy/reference paths, filters a wrong exact version, reuses the unchanged package when declaration bytes change, and never indexes `node_modules` as workspace source. The reproducible measurement confirms complete cold generation and warm reuse for 1,003 records. Formatting, focused tests, strict Clippy, and diff review pass; only the repository-wide policy gate remains unreliable because of the separately tracked policy execution budget failure.
...
Use the repository's TypeScript tree-sitter grammar to parse each exact declaration artifact. Walk nodes iteratively and emit `AuthoredSemanticModelPack` facts for exported modules/namespaces, interfaces, classes, type aliases, functions, variables, constructors, methods, properties, overload signatures, type parameters, extends/implements relationships, explicit re-exports, declaration-merging participants, and proven `declare global` additions. Use semantic IDs built from package identity, module entry path, declaration kind, owner, and declaration name; use deterministic source locators containing declaration byte spans. Maintain declaration order only until canonical compilation sorts facts; never use source text matching to infer an export, a type, or a route.
...
The test must prove `Project::all_files()` excludes `node_modules` and exercise the generic location, symbol, source/signature, hierarchy, and reference entry points that the current overlay supports. It must also prove an exact lock/version match activates, a wrong version does not, a changed declaration regenerates only that package, and incomplete discovery cannot be composed into an authoritative empty activation request. Source locations must use deterministic package-module paths rather than temporary project-relative `node_modules` paths.
...
Acceptance is demonstrated by an inline TypeScript workspace whose normal source imports a symbol from an installed, lockfile-proven package. The workspace file listing contains only the authored source and metadata files, not dependency declaration files. The shared coordinator produces one complete generated/reused semantic pack with exact npm package/version/digest evidence. Public navigation preserves a deterministic external source locator, symbol search includes it, signatures retain overload information, hierarchy returns declared inheritance, and an external reference is visible where the overlay can model it.
...
Revision note (2026-08-02): Reconciled the implementation with the live acceptance text and Scala precedent by preserving deterministic source declaration locators, adding re-export/merging/global/reference coverage, and recording the unresolved repository-wide policy reliability gate.
