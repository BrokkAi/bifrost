...
- [x] (2026-07-27 02:46Z) Fetched and cleanly rebased all six issue commits onto `origin/master` commit `af0968fa`; the current reviewed milestone checkpoints are `c34fd4ec` (semantic foundation), `d5bf04a2` (protocol summaries), `a8c0b4ec` (taint summaries), and `997495a3` (lifecycle measurement), with `.brokk/` still untracked and excluded from every commit.
- [x] (2026-07-27 02:46Z) Completed post-sync validation: the focused matrix enumerated 117 tests, with 116 passed and one ignored benchmark; every library/binary/integration target passed under `nlp,python`, the pinned doctest gate passed, and strict pinned all-target/all-feature Clippy, formatting, and diff checks were clean.
- [x] (2026-07-27 02:53Z) Refreshed `origin/master` once more, cleanly rebased all seven issue commits onto `990421fc`, verified the first six milestone patches as identical with `git range-diff`, and reran the focused matrix with 116 passed and one ignored. The seventh validation-record patch changed only to record the new base and current checkpoints: `cbdbf648` (semantic foundation), `c05debd6` (protocol summaries), `241d11f1` (taint summaries), and `57cf88e5` (lifecycle measurement).
...
- Observation: the exact semantic summaries currently describe a callee entry relation but do not expose the outer-entry mapping needed to flatten a dependency-bearing procedure's nested typestate effects without changing their meaning.
  Evidence: Milestone 2 can safely publish dependency-free procedures and effect-free recursive SCCs. It deliberately leaves non-recursive composed procedures and effectful recursive SCCs uncached while retaining the complete live solve, rather than assigning nested effects to a guessed caller entry.
...
- Observation: the complete all-feature suite has two independent host boundaries on this macOS worktree: sandboxed socket permission and mixed rustup/Homebrew rustdoc metadata.
  Evidence: the sandboxed library run passed 1,934 tests and failed only three `benchmark::mcp_session` socket tests with `Operation not permitted`; the host-permission rerun passed all 1,937 non-ignored library tests plus every binary and integration target. Cargo then selected Homebrew `rustdoc` and reported E0514 against rustup artifacts, while the isolated `cargo test --doc --features nlp,python` rerun with Rust 1.96 `PATH` and `RUSTDOC` pinned passed. Both test invocations used CI's `-undefined dynamic_lookup` flags and disabled semantic indexing.
...
- Decision: make the generic cross-query provider context-independent for one exact procedure and entry fact.
  Rationale: the solver intentionally deduplicates that entry identity. Allowing a provider to return different relations by call-site context would make the first caller contaminate later callers. Context-sensitive reuse must wait for a solver-level context key rather than hiding extra selection state inside the provider.
  Date/Author: 2026-07-26 / Codex

- Decision: scope protocol binding validity locally for dependency-free procedures and over the complete member closure for recursive SCCs.
  Rationale: unrelated caller bindings must not invalidate a reusable leaf, while every recursive member participates in the SCC fixed point. The SCC contract and exact validated manifest are computed once per solve under the data-flow budget and reused for O(1) lookup and publication.
  Date/Author: 2026-07-26 / Codex

- Decision: treat protocol caching as an optional optimization that cannot invalidate a complete analysis.
...
All four milestones are complete on top of `origin/master` commit `990421fc`. Post-sync focused validation enumerated 117 tests: 116 passed and one lifecycle entry point was intentionally ignored. The complete `nlp,python` gate passed every library, binary, and integration target at the host boundary; the separately pinned Rust 1.96 doctest gate passed; and strict `cargo-clippy --all-targets --all-features -- -D warnings` passed with its managed target removed. The later three-commit upstream delta changed only agent planning/evidence files; the first six milestone patches remained identical through that final rebase, the validation record changed only to record the new base, and the focused matrix passed again. Final milestone reviews have no unresolved Critical or High finding, and the unrelated `.brokk/` directory remains untracked and excluded from every commit.
...
The final synced baseline is `origin/master` commit `990421fc`. Issue #821 landed earlier as `3a634356` through PR #1180 on top of `5d228346` for the reusable typestate client and `4dceaf47` for bounded IDE propagation. The query-local recursive summary kernel landed in `3e94f809`, and bounded witnesses landed in `c8df49d3`. The earlier #821 branch snapshot at `b4267793` is historical only; all taint-summary work uses the landed contracts. The lifecycle report intentionally retains pre-final-sync issue commit `ab8b90f3` because that is the exact code revision measured; the clean final rebases remapped the implementation checkpoints without changing the retained evidence.
...
Plan revision note (2026-07-27): Completed all four milestones, recorded the measured no-persistence decision, rebased cleanly onto `origin/master` at `990421fc`, and reran focused, complete all-feature, doctest, formatting, diff, and strict Clippy gates. The final three upstream commits touched only agent planning/evidence files; a range-diff showed the first six milestone patches unchanged, the final validation record was updated only for the new base, and another focused run verified the result. The pre-rebase revision in the benchmark report remains the truthful provenance for the retained measurements.
