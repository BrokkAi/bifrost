...
- [ ] Stage 7 (edge-pipeline unproven-inbound signal): bulk dead-code/public-symbol analysis treats unproven-only inbound as inconclusive.
- [ ] Stage 8 (JS/TS default-export seed gap): root-cause trace + failing repro, then default-export-binding seeds.
...
- Final retrospective (2026-07-07 / Claude): All five stages landed as five commits on master in one day — f852b12 (Go receiver facts), b758106 (C# type identity), 6a5c2e4 (Rust seeding), be51910 (proof tiers + verified-absent), 243e2bb (hint contract) — against the plan's original purpose: all three issue-#528 repro shapes now return real usages; "candidates seen but unproven" is returned as a labeled tier instead of failing (C#/Go/Rust) or silently vanishing (Go); complete zero scans claim `verified_absent`; and no hint recommends the recovery the query just used. All six root-cause repro tests pass in their suites; the full workspace suite (`--features nlp,python`) exits 0; fmt and clippy `--all-features -D warnings` are clean.
  Deliberately out of scope and known gaps: Python timeouts (#528 item 4, separate thread); unproven-tier emission for Java/Ruby/C++ (they keep boolean gates — adoption path is D5); proof tiers in the inverted-edge pipeline used by bulk dead-code/public-symbol analysis and relevance (D22); JS/TS l10n default-export seeds (#528 side note); glob imports of never-exported Rust items stay unseeded (structurally correct — a glob only binds exports).
