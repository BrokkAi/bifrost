...
- [ ] Milestone 1: reusable typed row-field and relational assertion foundation (2026-08-05 16:25Z progress: analyzer-owned row schemas and projections, declarative RQLP parsing/formatting/canonical loading, static type validation, and the bounded iterative in-memory join/group/aggregate evaluator are implemented and tested for clean/finding/unreliable plus source/join limits. Receiver outcome/evidence expansions now type-check. Remaining: execute named query/expansion bindings through `run_policy`, assemble relational violations as findings, and prove exact source-range diagnostics/end-to-end fixtures).
- [ ] Milestone 2: receiver outcome/evidence rows and stable occurrence-to-receiver correlation (2026-08-05 17:05Z progress: schema v11 exposes `receiver-outcome` and `receiver-evidence`; every analyzed site has a content-scoped `site_id`, an exact fact-backed `site_ast_id` when available, one mandatory outcome projection, and deterministic parent-linked factory evidence rows. Unknown/budget cases preserve a zero-evidence outcome with explicit coverage. Receiver operations now accept occurrence rows, preserve the input occurrence AST ID on both outcome and evidence rows, and join that ID to the production resolver's candidate trace without text/range comparison. Focused TypeScript tests cover typed-local correlation, occurrence-to-candidate correlation, budget absence semantics, and stable factory chains. Remaining: richer declared/inferred type, proof-source, substitutions and semantic provenance fields; alias/branch/nested-chain and unsupported/open fixtures; consumer/docs migration).
