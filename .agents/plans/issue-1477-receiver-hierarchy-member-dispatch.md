...
- [ ] Milestone 2: receiver outcome/evidence rows and stable occurrence-to-receiver correlation (2026-08-05 16:25Z progress: schema v11 exposes `receiver-outcome` and `receiver-evidence`; every analyzed site has a content-scoped `site_id`, an exact fact-backed `site_ast_id` when available, one mandatory outcome projection, and deterministic parent-linked factory evidence rows. Unknown/budget cases preserve a zero-evidence outcome with explicit coverage. Focused TypeScript tests cover typed-local correlation, budget absence semantics, and stable factory chains. Remaining: richer declared/inferred type, proof-source, substitutions and semantic provenance fields; occurrence-row entry; alias/branch/nested-chain and unsupported/open fixtures; consumer/docs migration).
