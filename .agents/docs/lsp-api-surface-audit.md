...
| `textDocument/semanticTokens` | Full-document semantic tokens are implemented by #577 for analyzer-known declarations and structured references. The fixed legend mirrors Bifrost's high-level declaration taxonomy, requests read unsaved overlays, and source/candidate budgets bound synchronous work. | High. Editor highlighting is visible, broadly supported, and Bifrost classifies symbols from tree-sitter/analyzer structure rather than lexical tokenization. | Covered by stable-legend, multi-language, UTF-16, CRLF, overlay, unsupported-file, repeated-binding-name, and request-budget tests. Range/delta result state and refresh behavior remain separate work. | Keep full-document support focused on high-confidence resolved symbols. Add range or delta only with a measured client or performance need. |
