...
Do each removal after the specified release is available.

These removals are in this file instead of in tracker items. An agent reads this file during each session. The agent does not search for an issue that this file does not identify.

## After v0.9.0: delete the hand-written MCP stack

`crates/bifrost-mcp/src/mcp_common.rs` still contains the MCP implementation from before `rmcp`.

The hand-written implementation is currently the default. `BIFROST_MCP_RMCP=on` selects the `rmcp` host.

Complete these two steps before the removal:

1. Change the default to `on`.
2. Use the new default for a sufficient period.

These steps exist because `rmcp` 3.0 was only a few days old when issue #1328 adopted it. Do not keep these temporary precautions longer than necessary.

Remove these protocol items:
...
- `handle_response`
- `McpConnectionState`
- `McpRequestCancellations`
- `OutboundMcpResponse`
- `try_queue_response`
- `initialize_result`
- `success_response` and `error_response`
- The duplicate `prepare_tool_call`
- The duplicate `reconcile_codex_sandbox_workspace`
- The JSON-RPC constants
- `run_stdio_server_with_build_identity`
- The `MCP_RMCP_HOST_ENV` switch
- The related unit tests
- The `McpHost` matrix in `crates/bifrost-mcp/tests/bifrost_mcp_server.rs`

Keep these Bifrost items. The `rmcp` host already calls them and does not copy them:

- `McpServerSpec`
- `McpRenderOptions`
- `build_server_spec*`
- The `tool_descriptor` family
- `fit_get_summaries_output_to_budget` and its budgeting block
- `client_root_to_path`
- `file_uri_to_path`
- `serial_tool_request`
...
The crate entry point currently dispatches from the old module to `rmcp_host`. When you delete the old module, move the entry point. Also move the helpers that you keep to suitable locations.

Do this removal promptly.

While both hosts exist, the Codex sandbox authorization boundary has two copies. You must update both copies together. This duplicate code caused the pre-handshake bypass in `09e37dd1` to pass the test suite. A copied security predicate lost one condition.

This risk can occur again. The first upstream synchronization after the hosts diverged added the `run_policy` feature in #1394 only to the hand-written host. A later change had to add it manually to the other host.

Until you delete the old host, treat each MCP behavior change as a change to two implementations. Share a function instead of copying it.

# Dogfooding the rmcp MCP host

The Bifrost MCP server can use one of two protocol stacks. See `Scheduled removals`.

The `rmcp` stack is optional. It needs actual use before it can become the default. Use this plugin to test it.

At the start of a session that will use Bifrost code-intelligence tools, run:

    echo "${BIFROST_MCP_RMCP:-unset}"

If the result is not `on`, tell the user one time how to enable it.

The launcher passes its environment without changes. The user can export the variable in the shell that starts the agent. For example:

    BIFROST_MCP_RMCP=on claude
    BIFROST_MCP_RMCP=on codex

A desktop application that starts from a GUI does not inherit the shell environment. Explain that this procedure applies to sessions that start from a command-line interface.

Give this information one time. Do not repeat it. Do not stop the work because of this setting.

Do not set the variable yourself. The variable selects the protocol stack for the session. The user must make that selection.

Report a possible MCP failure as a finding. Do not ignore it or work around it without a report.

Possible failures include these conditions:

- A Bifrost tool call does not complete.
- A tool call returns a protocol error.
- A tool call unexpectedly reports "not bound to a workspace".
- A response is lost.
- A response contains results from the wrong workspace.
- A tool call fails immediately after a change to a folder or roots.

Report these details to the user:

- The active stack, from `BIFROST_MCP_RMCP`
- The exact tool and arguments
- The returned result or error
- Whether a retry changes the result
- Whether a change of stack changes the result

A different result from the two implementations for the same input is the most useful signal while both implementations exist.

Search the open issues. If no issue covers the failure, create an issue.

The latency rule in `Review findings as RQL regressions` covers slow calls. This rule covers incorrect calls.
