//! Find-references corner cases borrowed from Roslyn's C# find-references tests
//! (shapes ported with workspace-local types).
//!
...
use crate::common::lsp_client::LspServer;
...
// References to a method found through a `new` receiver call site (line 6). Caret
// on the method declaration.
#[test]
fn roslyn_refs_method() {
    assert_refs(
        &[(
            "a.cs",
            "class Foo {\n    public int Bar<caret>() { return 1; }\n}\nclass Program {\n    void Run() {\n        var f = new Foo();\n        f.Bar();\n    }\n}\n",
        )],
        &[("a.cs", 6)],
    );
}

// References to a static method through a static call (line 5). Caret on the
// static method declaration.
#[test]
fn roslyn_refs_static_method() {
    assert_refs(
        &[(
            "a.cs",
            "class Foo {\n    public static int Bar<caret>() { return 1; }\n}\nclass Program {\n    void Run() {\n        Foo.Bar();\n    }\n}\n",
        )],
        &[("a.cs", 5)],
    );
}

// References to a class: the `new Foo()` construction (line 5). Caret on the class
// declaration.
#[test]
fn roslyn_refs_class() {
    assert_refs(
        &[(
            "a.cs",
