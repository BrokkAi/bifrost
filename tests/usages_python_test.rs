...
use brokk_analyzer::usages::{
    CONFIDENCE_THRESHOLD, FuzzyResult, RegexUsageAnalyzer, TextSearchCandidateProvider, UsageAnalyzer,
    UsageFinder,
};
use brokk_analyzer::usages::CandidateFileProvider as _;
use brokk_analyzer::{
    CodeUnit, IAnalyzer, Language, ProjectFile, PythonAnalyzer,
};
...
fn regex_usage_analyzer_finds_class_usages() {
    ...
    let result = regex.find_usages(&analyzer, &[target.clone()], &candidates, 1000);
...
fn usage_finder_default_strategy_returns_results() {
    ...
    assert!(!hits.is_empty(), "expected non-empty usages via UsageFinder");
}
