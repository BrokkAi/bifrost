...
use brokk_bifrost::policy::{
    ...
    FindingSeverity, LoadedPolicy, MatchResultDomain, PolicyAnalysisType, PolicyBudget,
    PolicyCapability, PolicyEvaluationContext, PolicyEvaluator, PolicyFailureReason,
    PolicyFindingEvidence, PolicyFindingId, PolicyIncompleteReason, PolicyRegistry,
    PolicyRegistryLimits, PolicyRun, PolicyRunCompletion, PolicyRunError, PolicySourceIdentity,
    PolicyWorkReport, ResolvedTaintPolicySpec, TaintCatalogRegistry, TaintPolicyEvaluator,
};
...
struct WrongPolicyTaintAdapter<'a> {
    wrong_policy: &'a LoadedPolicy,
}

impl TaintPolicyEvaluator for WrongPolicyTaintAdapter<'_> {
    fn evaluate_taint(
        &self,
        _policy: &LoadedPolicy,
        _spec: &ResolvedTaintPolicySpec,
        _context: &PolicyEvaluationContext<'_>,
        _budget: &mut PolicyBudget,
    ) -> PolicyRun {
        let adapter_budget = PolicyBudget::default();
        PolicyRun::try_new(
            self.wrong_policy.definition().metadata.id.clone(),
            self.wrong_policy.semantic_hash(),
        ...
        .expect("empty fake run")
    }
}

#[test]
fn future_adapter_cannot_return_a_run_for_another_policy() {
    let (_project, analyzer) = typescript_analyzer("export function noop() {}\n");
    let catalogs = Arc::new(TaintCatalogRegistry::new_without_workspace(
        CatalogRegistryLimits::default(),
    ));
    ...
    registry
        .register_policy_bytes(
            PolicySourceIdentity::new("test:target"),
            taint_policy("test.target").as_bytes(),
        )
        .expect("target policy");
    registry
        .register_policy_bytes(
            PolicySourceIdentity::new("test:wrong"),
            taint_policy("test.wrong").as_bytes(),
        )
        .expect("wrong policy");
    let target = registry
        .policies()
    ...
    let wrong = registry
        ...
        .expect("wrong");
    let adapter = WrongPolicyTaintAdapter {
        wrong_policy: wrong,
    };
    ...
    let run = DefaultPolicyEvaluator::new()
        ...
        .expect("semantic mismatch should produce a bounded failed run");

    assert_eq!(run.policy_id(), &target.definition().metadata.id);
    assert_eq!(run.policy_hash(), target.semantic_hash());
    assert!(matches!(
        run.completion(),
        PolicyRunCompletion::Failed { reasons }
            if reasons.contains(&PolicyFailureReason::InternalInvariant)
    ));
    ...
    let mut tiny = PolicyBudget::builder()
        ...
        .build()
        .expect("budget");
    let error = DefaultPolicyEvaluator::new()
        .with_taint(&adapter)
        .evaluate(
            target,
            &PolicyEvaluationContext {
        ...
        .expect_err("a semantic-mismatch failure cannot exceed the host cap");
    assert_eq!(
        error,
        PolicyRunError::RetainedReportBytesExceeded { max: 1 }
    );
...
struct BudgetWideningTaintAdapter;

impl TaintPolicyEvaluator for BudgetWideningTaintAdapter {
    fn evaluate_taint(
        &self,
        policy: &LoadedPolicy,
        _spec: &ResolvedTaintPolicySpec,
        ...
        *budget = PolicyBudget::default();
        ...
        .expect("empty fake run under widened budget")
    }
}

#[test]
fn future_adapter_cannot_widen_the_original_host_budget() {
    let (_project, analyzer) = typescript_analyzer("export function noop() {}\n");
    let registry = registry_with_policy(&taint_policy("test.budget-authority"));
    let policy = registry.policies().next().expect("policy");
    let mut budget = PolicyBudget::builder()
        .with_max_retained_report_bytes(0)
        .expect("zero retained bytes")
        .build()
        .expect("budget");
    let error = DefaultPolicyEvaluator::new()
        .with_taint(&BudgetWideningTaintAdapter)
        .evaluate(
            policy,
            &PolicyEvaluationContext {
                analyzer: &analyzer,
        ...
        .expect_err("the original zero-byte host budget cannot retain a run");
