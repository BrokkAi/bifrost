...
fn isolated_target_is_removed_after_interruption() {
    ...
    while entries(temp.path()).is_empty() && std::time::Instant::now() < deadline {
...
fn cleanup_is_dry_run_by_default_and_apply_is_prefix_scoped() {
    let temp = tempfile::tempdir().expect("temporary root");
    let stale = temp.path().join("bifrost-stale");
...
fn cleanup_preserves_young_active_retained_open_and_symlinked_directories() {
    ...
    let active = temp.path().join("bifrost-active");
    let retained = temp.path().join("bifrost-retained");
    let open = temp.path().join("bifrost-open");
