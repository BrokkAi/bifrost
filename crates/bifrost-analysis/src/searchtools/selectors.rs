...
/// [`definition_selector`] for the un-anchored branch of [`distinct_definitions`],
/// where the selector is also the *group key*.
///
/// Scala keys on the display spelling (`display_symbol_name` drops the companion
/// `$`). A case class and its own companion object are two indexed names for one
/// declaration site — `org.http4s.CharsetRange$.Atom` and
/// `…CharsetRange$.Atom$` — but one spelling in every summary
/// (`org.http4s.CharsetRange.Atom`). Keying on the raw FQN split them into two
/// ambiguity candidates that print identically once displayed, so the caller was
/// asked to choose between indistinguishable selectors and the summary spelling
/// never round-tripped (#2451). Anchored selectors keep the raw FQN: they exist
/// to separate declarations in *different files*, which the `$` is not doing
/// here.
fn non_anchored_definition_selector(unit: &CodeUnit) -> String {
    if language_for_target(unit) == Language::Scala {
        return crate::analyzer::common::display_symbol_name(Language::Scala, &unit.fq_name());
    }
...
pub(super) fn distinct_definitions(
    ...
    for unit in overloads {
        ...
        let selector = if cross_domain || collision_split_fqns.contains(&fqn) {
            ...
            non_anchored_definition_selector(&unit)
        };
        match groups
            .iter_mut()
