...
pub(super) fn resolve_cpp<'a>(
    ...
    if report_stats {
        eprintln!(
            "BIFROST_CPP_FORWARD_STATS phase=visibility status=started file={}",
    ...
    if report_stats {
        eprintln!(
            "BIFROST_CPP_FORWARD_STATS phase=visibility status=completed file={} elapsed_ms={}",
            file.rel_path().display(),
            visibility_started.elapsed().as_millis(),
        );
    }
    let class_ranges_started = Instant::now();
    if report_stats {
        eprintln!(
            "BIFROST_CPP_FORWARD_STATS phase=class_ranges status=started file={}",
            file.rel_path().display(),
        );
    }
    ...
    if report_stats {
        eprintln!(
            "BIFROST_CPP_FORWARD_STATS phase=class_ranges status=completed file={} elapsed_ms={}",
            file.rel_path().display(),
            class_ranges_started.elapsed().as_millis(),
        );
    }
    let root = tree.root_node();
    let node_select_started = Instant::now();
    if report_stats {
        eprintln!("BIFROST_CPP_FORWARD_PHASE phase=node_select status=started");
    }
    ...
    if report_stats {
        eprintln!(
            "BIFROST_CPP_FORWARD_PHASE phase=node_select status=completed kind={} elapsed_ms={}",
            node.kind(),
            node_select_started.elapsed().as_millis(),
        );
    }
    ...
    if node.kind() == "this" && is_c_source_file(file) {
        ...
        let bindings = cpp_local_bindings_before(ctx, token, node, node.start_byte());
    ...
    if report_stats {
        eprintln!("BIFROST_CPP_FORWARD_PHASE phase=macro status=started");
    }
    let macro_resolution = visibility.resolve_ordinary_macro_reference(&graph, file, node, source);
    if report_stats {
        eprintln!(
            "BIFROST_CPP_FORWARD_PHASE phase=macro status=completed elapsed_ms={}",
            macro_started.elapsed().as_millis(),
        );
    }
    match macro_resolution {
    ...
    if let Some(CppReferenceNode::Type(type_node)) = reference {
        ...
        let resolve_type_started = Instant::now();
        if report_stats {
            eprintln!("BIFROST_CPP_FORWARD_PHASE phase=resolve_cpp_type status=started");
        }
        let outcome = resolve_cpp_type(
            analyzer,
            token,
            context,
            file,
        ...
        if report_stats {
            eprintln!(
                "BIFROST_CPP_FORWARD_PHASE phase=resolve_cpp_type status=completed elapsed_ms={}",
    ...
    match reference {
        ...
        Some(CppReferenceNode::Identifier(identifier)) => {
            ...
            let bindings =
                cpp_local_bindings_before(ctx, token, identifier, identifier.start_byte());
            if bindings.is_shadowed(text) {
                return no_definition(
                    "local_variable_reference",
                    format!("`{text}` is a local C++ value"),
            ...
            if let Some(initializer) = identifier
                .parent()
                .filter(|parent| parent.kind() == "field_initializer")
                && initializer
                    .child_by_field_name("name")
                    .or_else(|| initializer.named_child(0))
                    == Some(identifier)
                && let Some(enclosing_owner) = cpp_enclosing_class(
                    ctx.analyzer,
                    ctx.support,
                    ctx.visibility,
                    ctx.file,
...
fn resolve_cpp_bounded_in_session(
    ...
    match cpp_bounded_reference_node(node, provider.session) {
        ...
        Some(CppReferenceNode::Identifier(identifier)) => {
            resolve_cpp_bounded_callable(provider, file, source, root, identifier)
        }
        None => no_definition(
            "unsupported_cpp_reference_shape",
            format!(
...
fn resolve_cpp_bounded_member(
    ...
    let external_owner_name =
        cpp_external_receiver_type_name_from_ast(file, source, root, receiver, provider.session);
    let Some(resolution) =
...
fn cpp_external_receiver_type_name_from_ast(
    ...
    let type_node = match receiver.kind() {
        "identifier" | "field_identifier" => {
            cpp_bounded_binding_type_node(file, source, root, receiver, session)?
        }
...
fn resolve_cpp_bounded_callable(
    ...
    if node.id() == name_node.id()
        && cpp_bounded_binding_type_node(file, source, root, name_node, provider.session).is_some()
    {
        return no_definition(
...
fn resolve_cpp_bounded_call_target(
    ...
    if function.kind() == "field_expression" {
        ...
        resolve_cpp_bounded_callable(provider, file, source, root, function)
    }
}
...
fn cpp_bounded_type_resolution_for_node(
    ...
    match node.kind() {
        ...
        "identifier" | "field_identifier" => {
            let type_node =
                cpp_bounded_binding_type_node(file, source, root, node, provider.session)?;
...
fn cpp_bounded_binding_type_node<'tree>(
    ...
    while let Some(node) = stack.pop() {
        ...
        if let Some(binding) = cpp_c_autoptr_statement_binding(file, source, node)
            && cpp_node_text(binding.name_node, source) == name
        {
            let scope = cpp_bounded_enclosing_scope(node, callable, session)?;
...
fn cpp_reference_node(node: Node<'_>) -> Option<CppReferenceNode<'_>> {
    ...
    while let Some(parent) = current.parent() {
        ...
        // A qualified template-id can be represented as a `template_type`
        // nested in the `name` field of its `qualified_identifier` (for
        // example `std::remove_reference_t<C>`).  Keep that explicit scope
        // on the reference.  Template-types used as the `scope` of a
        // qualified access (for example `Vector<T>::value`) remain focused on
        // the template itself and must not climb into the outer member path.
        if (current.kind() != "template_type"
            || parent
                .child_by_field_name("name")
                .is_some_and(|name| cpp_same_node(name, current)))
            && matches!(
                parent.kind(),
                "qualified_identifier" | "scoped_identifier" | "scoped_type_identifier"
            )
...
fn cpp_type_node_is_unqualified_name(node: Node<'_>) -> bool {
    cpp_unqualified_type_name_node(node).is_some()
}

fn cpp_unqualified_type_name_node(node: Node<'_>) -> Option<Node<'_>> {
    match node.kind() {
        "type_identifier" | "namespace_identifier" => Some(node),
        "template_type" => node
            .child_by_field_name("name")
            .filter(|name| matches!(name.kind(), "type_identifier" | "namespace_identifier")),
        _ => None,
    }
}
...
fn resolve_cpp_type(
    ...
    if !recovered_declarator_type && let Some(qualifier) = cpp_focused_type_qualifier(node, source)
    {
            ...
            )
        {
            let unit = if unit.is_class() && !cpp_unit_is_type_alias(analyzer, &unit) {
                let Some(unit) =
                    ...
                    return ambiguous_without_candidates(format!(
                        "`{}` does not have a unique visible canonical C++ type declaration",
                        qualifier.qualified_reference
                    ));
                };
                unit
...
fn resolve_cpp_type_without_focused_qualifier(
    ...
    {
        // Sentinel recovery re-owns the malformed class body and records an
        // exact class range even though the original tree leaves its members
    ...
    if node.kind() == "type_identifier"
        && let Some(owner) = cpp_out_of_line_function_owner_for_type(
            analyzer, support, visibility, file, source, node,
        )
    {
        let candidates = cpp_direct_type_member_candidates(analyzer, support, &owner, text)
            .into_iter()
            .filter(|candidate| {
    ...
    if node.kind() == "qualified_identifier"
        && let Some(mut components) = cpp_type_name_components(node, source)
        && let Some(member) = components.pop()
        && !components.is_empty()
    {
        qualified_owner_lookup_attempted = declaration_type_context;
        let globally_qualified = is_globally_qualified_cpp_name(node);
        let owner_reference = components.join("::");
        let fallback_owner = || {
            let rendered = if globally_qualified {
                format!("::{owner_reference}")
            } else {
        ...
        .and_then(|owner| {
            visibility.canonical_visible_full_type_unit(&dispatch.source(), file, &owner)
        });
    ...
    if matches!(
        ...
        // Preserve a qualified template-id's arguments while re-opening the
        // visible-name fallback.  The owner/member probe above intentionally
        // handles nested types, but a namespace-qualified template alias (for
        // example `base::android::ScopedJavaGlobalRef<int>`) has no type owner
        // to resolve.  Falling back on the terminal spelling alone would let an
        // unrelated visible alias win (`std::remove_reference_t` ->
        // `absl::remove_reference_t`), so derive the lookup spelling from the
        // structured components and require an exact qualified-path match.
        let qualified_reference = cpp_type_name_components(node, source)
        ...
        .into_iter()
        .filter(|candidate| {
            (!qualified_owner_lookup_attempted
                || cpp_qualified_type_candidate_matches_reference(node, source, candidate))
                && visibility.external_type_candidate_visible_at(file, candidate, node.start_byte())
        })
        .flat_map(|unit| {
            let resolved = match template_arguments.as_deref() {
                Some(arguments) => {
                    match visibility.resolve_template_arguments(file, unit, arguments) {
                        Ok(unit) => Some(unit),
                        Err(error) => {
    ...
    if node.kind() == "type_identifier" || recovered_macro_decorated_declarator_type(node).is_some()
    {
        let graph_source = dispatch.source();
        let lexical_resolution = cpp_resolve_type_components_lexically_at_preserving_alias(
            node,
            &[text.to_string()],
            false,
            &graph_source,
    ...
    // aliases, inheritance, and using/import bindings above. Falling through
    // to a short-name-only workspace bucket can only erase that lexical
    // evidence: an unindexed local/global declaration then borrows an
    // unrelated same-spelled namespace type (#2503). Keep the coarse fallback
    // for recovered/non-terminal type shapes, where no structured lexical
...
fn cpp_active_template_parameter_reference(node: Node<'_>, source: &str) -> bool {
    let Some(components) = cpp_type_name_components(node, source) else {
        return false;
    };
    ...
    cpp_fragmented_class_template_parameter_reference(node, source, name)
        || cpp_fragmented_function_template_parameter_reference(node, source, name)
}

/// Recover a class-template binder when an unmatched namespace-closing macro
/// makes tree-sitter detach the qualified class specialization from its
/// `template <...>` head. In this error shape the template head is the class
/// specifier's immediately preceding `ERROR` sibling. Tree-sitter still keeps
/// the `template`, angle-bracket, `typename`/`class`, and parameter identifier
/// children, so the binder remains recoverable without parsing source text.
fn cpp_fragmented_class_template_parameter_reference(
    node: Node<'_>,
    source: &str,
    name: &str,
) -> bool {
    ...
    let Some(error) = class.prev_named_sibling() else {
        return false;
    };
    if error.kind() != "ERROR" || error.end_byte() > class.start_byte() {
        return false;
    }

    let children = (0..error.child_count())
        .filter_map(|index| error.child(index))
        .collect::<Vec<_>>();
    let Some(template_index) = children
        .iter()
        .rposition(|child| child.kind() == "template")
    else {
        return false;
    };
    let Some(open_index) = children[template_index + 1..]
        .iter()
        .position(|child| child.kind() == "<")
        .map(|index| template_index + 1 + index)
    else {
        return false;
    };
    let Some(close_index) = children[open_index + 1..]
        .iter()
        .position(|child| child.kind() == ">")
    ...
    children[open_index + 1..close_index]
        .windows(2)
        .any(|pair| {
            (matches!(
                pair[0].kind(),
                "primitive_type" | "type_identifier" | "placeholder_type_specifier"
            ) || matches!(cpp_node_text(pair[0], source), "typename" | "class"))
                && matches!(pair[1].kind(), "identifier" | "type_identifier")
                && cpp_node_text(pair[1], source) == name
        })
}
...
fn cpp_template_parameter_name(node: Node<'_>, source: &str) -> Option<String> {
    ...
    if matches!(
        ...
        let name = cpp_node_text(candidate, source).trim();
        return (!name.is_empty()).then(|| name.to_string());
    }
    extract_variable_name(candidate, source)
}

fn cpp_active_block_type_alias_node<'tree>(
    node: Node<'tree>,
    name: &str,
    source: &str,
) -> Option<Node<'tree>> {
    ...
    while let Some(parent) = current.parent() {
        if matches!(parent.kind(), "compound_statement" | "function_definition") {
            for child in parent.named_children(&mut parent.walk()) {
                ...
                if cpp_type_alias_declaration_name(child, source).as_deref() == Some(name) {
                    return Some(child);
                }
            }
        }
...
fn cpp_active_block_tag_type_node<'tree>(
    node: Node<'tree>,
    name: &str,
    source: &str,
) -> Option<Node<'tree>> {
    ...
    while let Some(candidate) = ancestor {
        let matching_tag = matches!(
            candidate.kind(),
            "class_specifier" | "struct_specifier" | "union_specifier" | "enum_specifier"
        ) && candidate
            .child_by_field_name("name")
            .is_some_and(|name_node| cpp_node_text(name_node, source).trim() == name);
        if matching_tag {
            let mut scope = candidate.parent();
    ...
    while let Some(parent) = current.parent() {
        if matches!(parent.kind(), "compound_statement" | "function_definition") {
            for child in parent.named_children(&mut parent.walk()) {
                ...
                let specifier = if matches!(
                    child.kind(),
                    "class_specifier" | "struct_specifier" | "union_specifier" | "enum_specifier"
                ) {
                    Some(child)
                } else if matches!(child.kind(), "declaration" | "field_declaration") {
                    child.child_by_field_name("type").filter(|type_node| {
                        matches!(
                            type_node.kind(),
                            "class_specifier"
                                | "struct_specifier"
...
fn cpp_indexed_type_alias_for_node(
    analyzer: &dyn IAnalyzer,
    support: &dyn BoundedDefinitionLookup,
    file: &ProjectFile,
    alias_node: Node<'_>,
    ...
    support
        ...
        .into_iter()
        .filter(|candidate| {
            candidate.source() == file && cpp_unit_is_type_alias(analyzer, candidate)
        })
        .find(|candidate| {
            analyzer.ranges(candidate).into_iter().any(|range| {
                range.start_byte <= alias_node.start_byte()
                    && alias_node.end_byte() <= range.end_byte
            })
        })
}

fn cpp_type_alias_declaration_name(node: Node<'_>, source: &str) -> Option<String> {
...
#[allow(clippy::too_many_arguments)]
fn cpp_malformed_class_alias_candidate(
    analyzer: &dyn IAnalyzer,
    support: &dyn BoundedDefinitionLookup,
    visibility: &CppVisibilityIndex,
    file: &ProjectFile,
    ...
    let mut class_body = function;
    while let Some(parent) = class_body.parent() {
        if parent.kind() == "compound_statement"
            && parent
                .parent()
                .is_some_and(|owner| owner.kind() == "declaration_list")
            && parent.start_byte() <= function.start_byte()
        {
            class_body = parent;
            break;
        }
    ...
    let owner = class_ranges.map_or_else(
        || {
            cpp_enclosing_class(
                analyzer,
                support,
    ...
    cpp_direct_type_member_candidates(analyzer, support, &owner, name)
        ...
        .find(|candidate| {
            analyzer.ranges(candidate).into_iter().any(|range| {
                range.start_byte < node.start_byte() && range.end_byte <= node.start_byte()
            })
        })
}

fn cpp_error_contains_class_header(node: Node<'_>) -> bool {
...
fn cpp_out_of_line_function_owner_for_type(
    analyzer: &dyn IAnalyzer,
    support: &dyn BoundedDefinitionLookup,
    visibility: &CppVisibilityIndex,
    file: &ProjectFile,
    ...
    while let Some(candidate) = current {
        if candidate.kind() == "function_definition" {
            return cpp_function_definition_owner(
                analyzer,
                support,
                visibility,
                file,
...
fn cpp_direct_type_member_candidates(
    analyzer: &dyn IAnalyzer,
    support: &dyn BoundedDefinitionLookup,
    owner: &CodeUnit,
    member: &str,
...
fn cpp_type_definition_candidates(
    ...
    let is_complete_class = |candidate: &CodeUnit| {
        candidate.is_class()
            && cpp_class_declaration_strength(&graph_source, candidate)
                == CppClassDeclarationStrength::Full
    };
    let mut indexed = visibility
        .visible_identifier_candidates(file, target.identifier())
        .filter(|candidate| candidate.fq_name() == target.fq_name())
        .filter(|candidate| {
            cpp_unit_is_type_alias(analyzer, candidate) || is_complete_class(candidate)
        })
        .cloned()
        .collect::<Vec<_>>();
    if indexed.is_empty() {
        ...
        if report_stats {
            eprintln!(
                "BIFROST_CPP_FORWARD_FQN_FALLBACK_STATS status=started fqn={}",
                target.fq_name(),
            );
        }
        indexed = support
        ...
        if report_stats {
            eprintln!(
                "BIFROST_CPP_FORWARD_FQN_FALLBACK_STATS status=completed fqn={} candidates={} elapsed_ms={}",
                target.fq_name(),
                indexed.len(),
                started.elapsed().as_millis(),
            );
...
///
/// `cpp_navigation_type_target` is intentionally used for a single alias: it
/// resolves an alias in the alias declaration's scope, rather than resolving
/// its right-hand side in the consumer's scope. When the same alias is copied
/// into multiple headers, however, canonicalizing first loses the alias family
/// and leaves only physical class redeclarations for the later deduplication
/// pass. Those redeclarations have the same canonical type but can point at a
/// header outside the reference's include closure. Identical aliases are one
/// logical declaration, so select the reachable physical spelling here and
/// let the caller canonicalize that exact spelling. Returning the alias itself
/// would make definition navigation stop at a declaration-only typedef even
/// when its tagged struct body is indexed.
///
/// Aliases with different signatures are deliberately not handled here. They
/// can name different targets and must continue through canonical resolution,
/// where they remain a real ambiguity.
fn cpp_reachable_identical_alias(
    analyzer: &dyn IAnalyzer,
    visibility: &CppVisibilityIndex,
    file: &ProjectFile,
    support: &dyn BoundedDefinitionLookup,
    ...
    let mut aliases = visibility
        .visible_identifier_candidates(file, unit.identifier())
        .filter(|candidate| {
            candidate.fq_name() == unit.fq_name() && cpp_unit_is_type_alias(analyzer, candidate)
        })
        .cloned()
        .collect::<Vec<_>>();
    if aliases.is_empty() {
        aliases = support
            .fqn(&unit.fq_name())
            .into_iter()
            .filter(|candidate| cpp_unit_is_type_alias(analyzer, candidate))
            .collect();
    }
    sort_units(&mut aliases);
    aliases.dedup();
    if aliases.len() < 2
        || !aliases
            .iter()
            .all(|candidate| same_logical_symbol(&aliases[0], candidate))
    {
        return None;
    }
    collapse_same_fqn_alias_spellings(analyzer, visibility, file, &mut aliases);
    debug_assert_eq!(
        aliases.len(),
        1,
        "one same-logical-symbol alias family must collapse to one physical spelling"
    );
...
fn resolve_cpp_call(
    ...
    if let Some(operator) = cpp_explicit_operator_name(call) {
        ...
        let owners = cpp_receiver_type_units(ctx, token, function, false, None);
        let candidates = cpp_member_candidates_lazy(ctx, token, owners, member, call_arity, || {
    ...
    match function.kind() {
        ...
        "qualified_identifier" => {
                    ...
                    .and_then(cpp_callable_name_node)
            {
                ...
                owners.dedup();
                candidates =
                    cpp_member_candidates_lazy(ctx, token, owners, member, call_arity, || {
                        cpp_call_argument_types(
                            ctx.analyzer,
                            token,
                            ctx.support,
        ...
        | "literal_operator_name" => {
            ...
            let bindings = cpp_local_bindings_before(ctx, token, name_node, name_node.start_byte());
            ...
            if let Some(using_resolution) = cpp_resolve_block_using_call_target(
                ...
                match using_resolution {
                    CppBlockUsingCallTargetResolution::Target(resolution) => {
                        if let Some(outcome) = cpp_bare_call_target_outcome(
                            ctx, token, call, call_arity, name, resolution,
                        ) {
                            return outcome;
                        }
                    }
            ...
            if is_c_source_file(ctx.file) {
                ...
                let mut later = ctx
                    ...
                    .cloned()
                    .chain(ctx.support.file_identifier(ctx.file, name))
                    .filter(|candidate| {
                        cpp_unit_matches_kind(
                            ctx.analyzer,
                            ctx.support,
                ...
                .filter(|candidate| {
                    ctx.analyzer
                        ...
                        .flat_map(|field_type| {
                            ...
                            aliases.extend(field_type.alias_unit);
                            aliases
                        })
                        .any(|alias| {
                            ctx.analyzer
...
fn cpp_bare_call_target_outcome(
    ...
    match resolution {
        CppBareCallTargetResolution::FreeFunctions(units) => {
            let mut candidates =
                cpp_bare_free_function_definition_candidates(ctx, token, units, call.start_byte());
            candidates = cpp_filter_candidates_by_call_lazy(
                candidates,
                call_arity,
                || {
...
fn resolve_cpp_construction_type(
    ...
    if let Some(unqualified_name_node) = cpp_unqualified_type_name_node(type_node) {
        let unqualified_name =
            normalize_cpp_type_text(cpp_node_text(unqualified_name_node, ctx.source));
        if let Some(alias_node) = (type_node.kind() == "type_identifier")
            .then(|| cpp_active_block_type_alias_node(type_node, &text, ctx.source))
            .flatten()
        {
            if let Some(alias) = cpp_indexed_type_alias_for_node(
                ctx.analyzer,
                ctx.support,
                ctx.file,
                alias_node,
        ...
        if cpp_active_template_parameter_reference(type_node, ctx.source) {
            return no_definition(
                "unresolved_template_parameter",
                format!(
                    "`{text}` is an active C++ template parameter without an indexed definition"
        ...
        if !member_candidates.is_empty() {
            if type_node.kind() == "template_type"
                && let Some(unit) =
                    cpp_choose_canonical_type(ctx.analyzer, member_candidates.clone())
            {
                if cpp_unit_is_type_alias(ctx.analyzer, &unit) {
                    return candidates_outcome(vec![unit]);
                }
                let Some(arguments) = cpp_template_reference_arguments(type_node, ctx.source)
                else {
                    unreachable!("a template application has template arguments");
                };
                return match ctx
                    .visibility
                    .resolve_template_arguments(ctx.file, unit, &arguments)
                {
                    Ok(unit) => candidates_outcome(cpp_type_definition_candidates(
                        ctx.analyzer,
                        ctx.visibility,
                        ctx.file,
                        ctx.support,
                    ...
                    Err(error) => {
                        ambiguous_without_candidates(cpp_template_resolution_message(&text, &error))
                    }
                };
            }
            return candidates_outcome(member_candidates);
        ...
        if let Some(unit) = resolve_in_enclosing_scopes(
            ...
            &unqualified_name,
            type_node.start_byte(),
            |unit| unit.is_class() || cpp_unit_is_type_alias(ctx.analyzer, unit),
        ) {
            return candidates_outcome(cpp_type_definition_candidates(
                ctx.analyzer,
                ctx.visibility,
        ...
        if type_node.kind() == "type_identifier"
            && cpp_active_block_tag_type_node(type_node, &text, ctx.source).is_some()
        {
            return no_definition(
                "unresolved_local_type",
                format!("`{text}` names a local C++ tag type without an indexed definition"),
            );
        }
    }
...
fn resolve_cpp_field(
    ...
    let external_owner_name = cpp_external_receiver_type_name_from_ast(
        ctx.file,
        ctx.source,
        ctx.root,
        receiver,
    ...
    candidates = candidates
        ...
        .flat_map(|unit| {
            ...
            indexed.retain(|candidate| {
                cpp_callable_definitions_share_identity_evidence_with_visibility(
                    analyzer, token, &graph, visibility, &unit, candidate,
                )
            });
            if indexed.is_empty() {
                vec![unit]
...
fn cpp_member_lookup(
    ...
    let mut seen = HashSet::default();
    cpp_inherited_member_candidates(ctx, token, owners, member, &mut seen, member_trace)
}
...
fn cpp_member_candidates(
    ...
    let mut member_trace = trace::recording().then(CppMemberTrace::default);
    let found = cpp_member_lookup(ctx, token, &owners, member, member_trace.as_mut());
...
{
    let mut member_trace = trace::recording().then(CppMemberTrace::default);
    let mut found = cpp_member_lookup(ctx, token, &owners, member, member_trace.as_mut());
...
fn cpp_filter_candidates_by_call(
    ...
    let arity_filtered =
        cpp_filter_candidates_by_arity(candidates, arity, analyzer, token, visibility);
...
fn cpp_call_argument_types(
    ...
    Some(
        cpp_argument_children(args)
            .map(|arg| {
                cpp_expression_type(
                    analyzer, token, support, visibility, file, source, root, arg,
                )
            })
            .collect(),
    )
...
fn cpp_expression_type(
    ...
    match node.kind() {
        ...
        "parenthesized_expression" => node
            ...
            .and_then(|inner| {
                cpp_expression_type(
                    analyzer, token, support, visibility, file, source, root, inner,
                )
            }),
        "pointer_expression" => {
            ...
            let inner = node
                .child_by_field_name("argument")
                .or_else(|| node.named_child(0))?;
            let mut inner_type = cpp_expression_type(
                analyzer, token, support, visibility, file, source, root, inner,
            )?;
...
fn cpp_receiver_type_units(
    ...
    match receiver.kind() {
        ...
        "parenthesized_expression" | "pointer_expression" => receiver
            ...
            .map(|inner| {
                cpp_receiver_type_units(ctx, token, inner, unwrap_template_alias, seeded_bindings)
            })
            .unwrap_or_default(),
        _ => Vec::new(),
    }
...
fn cpp_field_receiver_type_units(
    ...
    cpp_field_receiver_type_units_with_bindings(
        analyzer, token, support, visibility, file, source, root, field, receiver, None,
    )
}
...
fn cpp_receiver_unit_for_access(
    ...
    if unwrap_template_alias
        ...
        && let Some(target) = cpp_alias_arrow_target_unit(ctx, token, alias)
    {
        return Some(target);
    }
    cpp_type.unit
...
fn cpp_out_of_line_function_owner(
    ...
    loop {
        if node.kind() == "function_definition" {
            ...
            return cpp_function_definition_owner(
                analyzer, support, visibility, file, source, node, byte,
            );
        }
        node = node.parent()?;
    }
...
#[allow(clippy::too_many_arguments)]
fn cpp_function_definition_owner(
    analyzer: &dyn IAnalyzer,
    support: &dyn BoundedDefinitionLookup,
    visibility: &CppVisibilityIndex,
    file: &ProjectFile,
    ...
    let globally_qualified = is_globally_qualified_cpp_name(qualified);
    cpp_resolve_owner_type_in_lexical_namespace(
        analyzer,
        support,
        visibility,
        file,
...
fn cpp_macro_fractured_function_before(root: Node<'_>, cutoff_start: usize) -> Option<Node<'_>> {
    ...
    for child in root.named_children(&mut cursor) {
        ...
        if child.kind() == "function_definition" {
            candidate = child
                .child_by_field_name("body")
                .filter(|body| {
                    subtree_contains(*body, |node| {
                        matches!(node.kind(), "preproc_def" | "preproc_function_def")
                    })
                })
                .filter(|_| {
                    cpp_macro_fracture_boundary(child).is_some_and(|end| cutoff_start < end)
                })
                .map(|_| child);
        } else if child.kind() == "ERROR" {
            // The unmatched closing brace is the structured boundary emitted
            // for the fractured function. Never carry its locals farther.
            candidate = None;
        }
    }
    candidate
...
fn cpp_macro_fracture_boundary(node: Node<'_>) -> Option<usize> {
...
fn cpp_seed_function_macro_local_binding(
    ...
    if let Some(binding) = cpp_c_autoptr_statement_binding(ctx.file, ctx.source, node) {
        cpp_seed_binding(
            ctx.analyzer,
            token,
            ctx.support,
            ...
            cpp_node_text(binding.name_node, ctx.source),
            Some(cpp_node_text(binding.type_node, ctx.source)),
            Some(binding.type_node),
            1,
            None,
            None,
            bindings,
...
#[derive(Clone, Copy)]
struct CppAutoptrStatementBinding<'tree> {
    name_node: Node<'tree>,
    type_node: Node<'tree>,
}

/// Recover the two structured shapes tree-sitter uses for
/// `g_autoptr(T) value = initializer`: one malformed assignment containing the
/// macro call and name, or adjacent macro-call and assignment statements.
fn cpp_c_autoptr_statement_binding<'tree>(
    file: &ProjectFile,
    source: &str,
    statement: Node<'tree>,
) -> Option<CppAutoptrStatementBinding<'tree>> {
    if !is_c_source_file(file) || statement.kind() != "expression_statement" {
        return None;
    }
    ...
    if left.kind() == "identifier" {
        let macro_statement = statement.prev_named_sibling()?;
        if macro_statement.kind() != "expression_statement" {
            return None;
        }
        let type_node = cpp_c_autoptr_call_type_node(macro_statement.named_child(0)?, source)?;
        return Some(CppAutoptrStatementBinding {
            name_node: left,
            type_node,
        });
    }
    let type_node = cpp_c_autoptr_call_type_node(left, source)?;
    ...
    let name_node = error.named_child(0)?;
    if name_node.kind() != "identifier" {
        return None;
    }
    Some(CppAutoptrStatementBinding {
        name_node,
        type_node,
    })
}

fn cpp_c_autoptr_call_type_node<'tree>(call: Node<'tree>, source: &str) -> Option<Node<'tree>> {
    if call.kind() != "call_expression" {
        return None;
    }
    let function = call.child_by_field_name("function")?;
    if function.kind() != "identifier" || cpp_node_text(function, source) != "g_autoptr" {
        return None;
    }
    let arguments = call.child_by_field_name("arguments")?;
...
fn cpp_seed_binding(
    ...
    let autoptr_type_node =
        type_node.and_then(|node| cpp_c_autoptr_pointee_type_node(file, source, node));
    let autoptr_type_text = autoptr_type_node.map(|node| cpp_node_text(node, source).to_string());
    let effective_type_text = autoptr_type_text.as_deref().or(type_text);
    let effective_type_node = autoptr_type_node.or(type_node);
    let declarator_depth = declarator_depth + if autoptr_type_node.is_some() { 1 } else { 0 };
    let resolved = effective_type_text
...
/// The C `g_autoptr(T)` declaration macro is represented by tree-sitter as a
/// `macro_type_specifier`, with its argument retained as a `type_descriptor`.
/// Preserve that structure instead of interpreting the macro's source text.
fn cpp_c_autoptr_pointee_type_node<'tree>(
    file: &ProjectFile,
    source: &str,
    node: Node<'tree>,
) -> Option<Node<'tree>> {
    if !is_c_source_file(file) || node.kind() != "macro_type_specifier" {
        return None;
    }
    let name = node.child_by_field_name("name")?;
    if cpp_node_text(name, source) != "g_autoptr" {
        return None;
    }
    let descriptor = node.child_by_field_name("type")?;
    descriptor
        .child_by_field_name("type")
        .or_else(|| cpp_first_type_child(descriptor))
}
...
fn cpp_alias_arrow_target_unit(
    ctx: CppLookupCtx<'_, '_>,
    token: QueryToken<'_>,
    alias: &CodeUnit,
) -> Option<CodeUnit> {
...
fn cpp_call_return_type(
    ...
    let function = call.child_by_field_name("function")?;
    let arity = visibility.call_arity_evidence(file, call, source).exact();
...
mod bounded_tests {
    ...
    #[test]
    fn unknown_macro_arity_uses_unanimous_c_call_return_type_for_receiver() {
        let source = r#"#include UNKNOWN_BUILD_HEADER
struct pending { struct pending *next; };
typedef struct pending pending_t;
struct ring { pending_t items[4]; int count; };
typedef struct ring ring_t;
pending_t *get(ring_t *ring, int index) { return &ring->items[index]; }
void advance(ring_t *ring) { get(ring, ring->count - 2)->next = 0; }
"#;
        let fixture = AnalyzerFixture::new_for_language(Language::Cpp, &[("issue.c", source)]);
        let start = source.find("->next").expect("member use") + "->".len();
    ...
    fn reachable_identical_typedef_still_navigates_to_its_tagged_struct_body() {
        let fixture = AnalyzerFixture::new_for_language(
            ...
            &[
                (
                    "visible.hpp",
                    "typedef struct internal_tag public_name;\nstruct internal_tag { static void member(); };\n",
                ),
                (
                    "hidden.hpp",
                    "typedef struct internal_tag public_name;\nstruct internal_tag { static void member(); };\n",
        ...
        assert_eq!(result.results[0].status, "resolved", "{result:#?}");
        assert!(
            result.results[0].definitions.iter().any(|definition| {
                definition.fqn.as_deref() == Some("internal_tag")
                    && definition.path == "visible.hpp"
            }),
            "the reachable typedef spelling must canonicalize to its reachable tagged body: {result:#?}"
        );
    }
