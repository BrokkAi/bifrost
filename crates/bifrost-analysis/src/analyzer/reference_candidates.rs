...
use brokk_bifrost_go::graph::ast::is_clause_binding_identifier as go_is_clause_binding_identifier;
...
/// census probe frontier for each language, and a range the census proposes and
/// grades can never be reported as an unbacked inverse hit (#2185).
...
pub fn go_is_declaration_or_import_name(node: Node<'_>) -> bool {
    if go_is_clause_binding_identifier(node) {
        return true;
    }
    node.parent().is_some_and(|parent| {
        (matches!(
...
/// Whether a JS/TS terminal is the lowercase name of a JSX intrinsic element.
pub fn jsts_is_jsx_intrinsic_element_name(node: Node<'_>, source: &str) -> bool {
    jsts_jsx_intrinsic_element_name(node, source)
}
...
mod tests {
    ...
    fn cpp_census_membership_backs_only_structured_template_types_inside_recovery() {
        let source = concat!(
            "class Arg {};\n",
            "template <typename A, typename B> int typed();\n",
            "int outer(int, int, int);\n",
            "class Target {\n",
            " public:\n",
            ...
            "#endif\n",
            "        &p,\n",
            "        typed<Target, Arg>).field};\n",
            "  }\n",
            "};\n",
        );
        let census = census_offsets(Language::Cpp, "use.cpp", source);
        let membership = census_membership_offsets(Language::Cpp, "use.cpp", source);
        let recovered_target = source.find("Target, Arg").expect("recovered target");
        let recovered_argument = source.find("Arg>).field").expect("recovered argument");
        let recovery_call = source.find("first()").expect("recovery call");

        assert!(
            !census.contains(&recovered_target) && !census.contains(&recovered_argument),
            "the forward census must keep excluding the ERROR subtree: {census:?}"
        );
        assert!(
            membership.contains(&recovered_target) && membership.contains(&recovered_argument),
            "structured template types must back inverse precision: {membership:?}"
        );
        assert!(
    ...
    /// Membership answers "is there a source occurrence here?", so it must hold
    /// every range the census proposes and grades as one. #2086's declaration
    /// and label test ran on the whole file rather than on recovery, and
    /// tree-sitter-java spells a `name` field on `method_invocation`,
    /// `marker_annotation`, `annotation` and `scoped_identifier` as well as on
    /// its declarations -- so every call name, annotation use and qualified-name
    /// tail left membership while staying a graded census probe. A whole-repo
    /// Guava census charged 3,781 valid inverse hits as
    /// `inverse_precision_unbacked_hits` for that reason, 608 of them at byte
    /// ranges the same run graded as census sites (#2185).
    #[test]
    fn java_census_membership_holds_every_census_probe_outside_recovery() {
        let source = concat!(
            "import java.util.List;\n",
            "\n",
            "@interface Marker {}\n",
            "@interface Tagged { int value(); }\n",
            "class Helper {\n",
            ...
            "  @Tagged(value = 1)\n",
            "  int run(int seed) {\n",
            "    List<Integer> values = List.of(seed);\n",
            "    int total = Helper.compute(values.size());\n",
            "    loop:\n",
        ...
        let missing = census
            .iter()
            .copied()
            .filter(|offset| !membership.contains(offset))
            .collect::<Vec<_>>();

        assert!(
            missing.is_empty(),
            "every graded census probe must be membership-backed; missing {missing:?} \
        ...
        for role in [
            ...
            let offset = source.find(role).unwrap_or_else(|| panic!("{role}"));
            assert!(
                membership.contains(&offset),
                "the {role} reference role must stay in membership: {membership:?}"
            );
        }
    }
