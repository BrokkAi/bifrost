...
fn validate_wrapper(
    ...
    match form {
        ...
        RqlForm::ValueFlow => {
                ...
                || option.is_none_or(|option| option.field() != QueryStepField::PlanRef)
            {
                ...
                match args[1].as_symbol().or_else(|| args[1].as_string()) {
                    Some(value) => {
                        if let Err(error) =
                            value.parse::<super::super::analysis_context::ValueFlowPlanRef>()
                        {
                            analysis.error(
                                args[1].range.clone(),
                                "wrong-value-shape",
        ...
        RqlForm::Scopes | RqlForm::Bindings | RqlForm::Paths => {
...
mod tests {
    ...
    fn value_flow_help_and_diagnostics_are_range_precise() {
        let rql =
            "(witness :max-steps 8 (value-flow :plan-ref test:flow (procedure-of (function))))";
