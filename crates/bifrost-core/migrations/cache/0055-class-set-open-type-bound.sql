-- Admit the typed open remainder carried beside a useful class candidate.
-- SQLite cannot alter a CHECK constraint in place, so rebuild only the leaf
-- row table and preserve every schema-54 row.

CREATE TABLE class_set_finding_free_root_rows_v55 (
  ...
  CHECK(
    ...
    (atom_kind = 'unknown' AND class_name IS NULL
      AND unknown_reason IN (
        ...
        'incomplete_root', 'open_type_bound'))
  )
) WITHOUT ROWID, STRICT;

INSERT INTO class_set_finding_free_root_rows_v55(
...
ALTER TABLE class_set_finding_free_root_rows_v55
