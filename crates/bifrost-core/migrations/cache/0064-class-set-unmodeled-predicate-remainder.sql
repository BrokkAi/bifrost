-- Admit the unmodeled-predicate remainder. A guard whose condition is a call
-- on the receiver says something about that receiver on both of its arms; when
-- the callee cannot be modeled, neither arm's class set is bounded. The reason
-- carries no class, unlike `unmodeled_guard`, which names the class predicate
-- an `isinstance`-style guard spelled. SQLite cannot alter a CHECK constraint
-- in place, so rebuild only the constrained leaf row table and preserve every
-- schema-63 row, including the class-object remainder that migration 0063
-- added.

CREATE TABLE class_set_finding_free_root_rows_v64 (
  ...
  CHECK(
    ...
    (atom_kind = 'unknown' AND class_name IS NULL AND guard_class IS NULL
      AND unknown_reason IS NOT NULL AND unknown_reason IN (
        ...
        'class_creation', 'class_object', 'unmodeled_predicate'))
    OR
...
INSERT INTO class_set_finding_free_root_rows_v64(
...
ALTER TABLE class_set_finding_free_root_rows_v64
