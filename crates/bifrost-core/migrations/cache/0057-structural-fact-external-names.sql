-- Permit source-backed semantic names that are outside a fact's match range.
--
-- Kotlin constructors use the enclosing class identifier as their normalized
-- name even though the constructor fact starts at the constructor syntax. The
-- name remains source-backed and ordered; hydration additionally checks source
-- bounds and UTF-8 boundaries. SQLite cannot alter a CHECK constraint, so the
-- three structural-fact tables are recreated as in migration 0040.
