...
use crate::structural::Role;
use crate::structural::index_query::{
    SourceAnchorGroup, StructuralAccessRequirements, StructuralPostingTerm,
...
fn structural_access_for_query(query: &CodeQuerySeed) -> StructuralAccessRequirements {
    ...
    StructuralAccessRequirements::new(unique)
}
...
mod tests {
    ...
    fn anchors_of(query: serde_json::Value) -> Vec<Vec<String>> {
        ...
        QueryPlan::for_query(query.seed().unwrap())
            ...
            .into_iter()
            .map(|group| group.alternatives().to_vec())
            .collect()
    }
