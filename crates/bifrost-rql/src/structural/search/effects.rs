...
use crate::analyzer::semantic_model::{
    Completeness, ResolvedActiveSemanticModels, SemanticModelMatchDisposition,
};
...
use crate::analyzer::usages::effects::{
    ...
    EffectNodeBasis, EffectProof, EffectReason, ModeledProcedureKey, ProcedureEffectBudget,
    ProcedureEffectReport, call_effect_report, modeled_procedure_key_for_unit,
    summarize_procedure_effects,
};
...
///
/// The language label is the durable dialect spelling the taint side already
/// binds authored summaries with, so an effect declaration and a data-flow
/// summary select the same external member by the same rule.
fn external_modeled_key(target: &UnmaterializedExternalTarget) -> ModeledProcedureKey {
    ModeledProcedureKey {
        language: target.language().stable_label().to_owned(),
