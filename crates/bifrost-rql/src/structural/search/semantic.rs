...
use super::{
    ...
    CodeQuerySemanticRowLimits, CodeQuerySemanticWork, DeclarationValue, SeedMatch, seed_range,
};
...
use crate::analyzer::semantic::{
    ...
    ControlEdgeHandle, ControlEdgeId, Evidence, EvidenceCompleteness, LengthDelimitedDigest,
    MemoryLocation, ProcedureHandle, ProcedureSemantics, ProgramPoint, ProgramPointHandle,
    ProgramPointId, ProofStatus, SemanticArtifact, SemanticBudget, SemanticBudgetDimension,
    SemanticCallSite, SemanticCapability, SemanticEvent, SemanticGap, SemanticLocator,
    SemanticOutcome, SemanticRequest, SemanticValue, SemanticWork, SourceMapping,
};
...
/// them, so each lane is additionally held to a memory-shaped estimate: the
/// half of `max_retained_bytes` that is not owned text, split across the row
/// dimensions and priced at each one's row size. That estimate is a guard
/// against one runaway lane, not the memory bound itself -- the bound is
/// measured in `SemanticQueryState::materialize`, which charges each
/// artifact's own retained bytes against `max_retained_bytes` (#2523).
pub(super) fn semantic_budget_limits(limits: CodeQuerySemanticLimits) -> SemanticWork {
    ...
    SemanticWork {
        ...
        nested_entries: rows_for::<SemanticLocator>(limits, Dimension::NestedEntries),
