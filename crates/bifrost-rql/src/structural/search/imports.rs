...
pub(super) fn acquire_direct_import_layer(
    ...
    let Some(workspace_content) = state.analyzer.workspace_content_identity() else {
        // No identity, no reuse: the cache records the typed reason and the
        // caller falls back to request-local resolution rather than replaying a
        // relation it cannot prove was derived from this content.
        ...
        if layer_requested && state.access_mode == StructuralAccessMode::IndexedRequired {
            state.access_failure.get_or_insert_with(|| {
                "analyzer states no workspace content identity for the derived layer".to_string()
            });
        }
        return None;
    };
        ...
        .is_some_and(|content| content != workspace_content)
    {
        ...
        state.direct_import_layer_content = None;
    }
    let ready = cache.get_ready(request, workspace_content, cancellation);
    ...
    let acquisition = match ready {
        Some(layer) if state.analyzer.workspace_content_matches(workspace_content) => {
    ...
    match acquisition {
        DerivedLayerAcquisition::Ready { lifecycle, .. }
            if !state.analyzer.workspace_content_matches(workspace_content) =>
        {
            if let Some(profile) = &mut state.cache_profile {
                let topology = &mut profile.direct_import_topology;
                topology.misses = topology.misses.saturating_add(1);
            ...
            if layer_requested && state.access_mode == StructuralAccessMode::IndexedRequired {
                state.access_failure.get_or_insert_with(|| {
                    "derived-layer workspace content changed before selection".to_string()
                });
            }
            state.direct_import_layer = None;
            state.direct_import_layer_content = None;
        ...
        DerivedLayerAcquisition::Ready {
            ...
            if crate::profiling::enabled() {
                ...
                crate::profiling::note(format!(
                    "direct-import-topology cache={lifecycle:?} retained={retained} rebuilt={invalidated}"
                ));
            }
            state.direct_import_layer = Some(layer);
            state.direct_import_layer_content = Some(workspace_content);
            Some(lifecycle)
        }
        ...
        DerivedLayerAcquisition::Unavailable {
                ...
                && state.analyzer.workspace_content_matches(workspace_content)
            {
                ...
                state.import_graph_content = Some(workspace_content);
            }
...
pub(super) fn discard_stale_direct_import_layer(
    ...
    let Some(layer_content) = state.direct_import_layer_content else {
        return;
    };
    if state.analyzer.workspace_content_matches(layer_content) {
        return;
    }
    state.direct_import_layer = None;
    state.direct_import_layer_content = None;
...
pub(super) fn discard_stale_request_import_graph(state: &mut QueryExecutionState<'_>) {
    let Some(content) = state.import_graph_content else {
        return;
    };
    if state.analyzer.workspace_content_matches(content) {
        return;
    }
    state.import_graph = None;
    state.import_graph_content = None;
}
