...
pub(super) fn apply_plan_step(
    ...
    if !rows.is_empty() && matches!(step, QueryStep::ImportsOf | QueryStep::ImportersOf) {
        ...
        if use_snapshot_imports {
            ...
            if state.import_graph.is_none() {
                ...
                state.import_graph_content = state.analyzer.workspace_content_identity();
            }
            let graph = state
                .import_graph
                .as_mut()
    ...
    let selected_layer_content = if use_snapshot_imports {
        state.direct_import_layer_content
    } else {
        state.import_graph_content
    };
    ...
    if let Some(selected_content) = selected_layer_content
        && !state.analyzer.workspace_content_matches(selected_content)
    {
        rows.clear();
        exhausted = true;
        step_truncated = true;
    ...
    'rows: while let Some(row) = rows.next() {
        ...
        let expansions = match (&row.value, step) {
            ...
            (
                PipelineValue::Semantic(SemanticPipelineValue::Procedure(procedure)),
                QueryStep::GuardsOf,
            ) => guard_expansions(procedure),
            (PipelineValue::File(file), QueryStep::TargetOf) => {
                build_target_expansions(topology_cache, analyzer, file, diagnostics)
            }
            (PipelineValue::File(file), QueryStep::SourceSetOf) => {
                source_set_expansions(topology_cache, analyzer, file, diagnostics)
            }
            (PipelineValue::BuildTarget(target), QueryStep::TopologyEdgesOf) => {
                topology_edge_expansions(topology_cache, target, diagnostics)
            }
            ...
            (PipelineValue::Declaration(declaration), QueryStep::DeclarationStateOf(filter)) => {
                ...
                let required_axes: &[crate::structural::materialization::MaterializationAxis] =
