...
/// Keep one logical passage request small enough that JSON serialization and the
/// sidecar's eager tokenization cannot retain an entire generated-code extraction.
/// Exact token-length sorting and padded-token batching still happen in the sidecar.
const EMBED_REQUEST_COMPONENTS: usize = 2_048;
...
pub fn embed_group(
    ...
    for range in bounded_batch_ranges(
        ...
        let vectors = metrics::traced(
            &metrics::EMBED_NS,
            format_args!(
                "embed {} texts (total_bytes={total_bytes}, max_bytes={max_bytes})",
                texts.len()
            ),
        ...
        component_items.extend(batch.iter().map(|(key, _)| *key).zip(vectors));
    }
...
mod tests {
    ...
    impl Embedder for RecordingEmbedder {
        ...
        fn embed_passages(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>, String> {
            self.calls
                .lock()
                .unwrap()
                .push((texts.len(), texts.iter().map(|text| text.len()).sum()));
            Ok(texts
                .iter()
                .map(|text| vec![text.as_bytes()[0] as f32])
                .collect())
        }
    ...
    fn extracted_components(texts: Vec<String>) -> ExtractedGroup {
        ExtractedGroup {
            pending_blobs: Vec::new(),
            component_texts: texts
                .into_iter()
                .map(|text| (component_key(&text), text))
                .collect(),
        }
    }
    ...
    #[test]
    fn embed_group_splits_on_component_count_and_preserves_order() {
        let temp = tempfile::tempdir().unwrap();
        let store = SemanticStore::open(&temp.path().join("cache.db")).unwrap();
        let texts: Vec<String> = (0..EMBED_REQUEST_COMPONENTS + 1)
            .map(|index| format!("component-{index}"))
            .collect();
        let expected_keys: Vec<Key> = texts.iter().map(|text| component_key(text)).collect();
        let embedder = RecordingEmbedder::default();

        let embedded = embed_group(&store, &embedder, extracted_components(texts)).unwrap();

        let calls = embedder.calls.lock().unwrap();
        assert_eq!(calls.len(), 2);
        assert_eq!(calls[0].0, EMBED_REQUEST_COMPONENTS);
    ...
    #[test]
    fn embed_group_splits_on_total_text_bytes() {
        let temp = tempfile::tempdir().unwrap();
        let store = SemanticStore::open(&temp.path().join("cache.db")).unwrap();
        let text_bytes = EMBED_REQUEST_BYTES / 2 + 1;
        let texts = vec!["a".repeat(text_bytes), "b".repeat(text_bytes)];
        let embedder = RecordingEmbedder::default();
        ...
        assert_eq!(
            *embedder.calls.lock().unwrap(),
            vec![(1, text_bytes), (1, text_bytes)]
        );
        assert_eq!(embedded.component_items.len(), 2);
        assert_eq!(embedded.component_items[0].1, vec![b'a' as f32]);
        assert_eq!(embedded.component_items[1].1, vec![b'b' as f32]);
    }
