...
/// A finite, union-distributive may-data-flow predecessor relation.
///
/// The callbacks are deliberately separate from [`DistributiveDataflowProblem`]
/// because a backward relation is not, in general, the same operation as
/// running a forward transfer over a reversed graph.  For example, kills and
/// strong updates need explicit preimage semantics, and call/return edges keep
/// their original orientation in the descriptor passed to these callbacks.
pub trait BackwardDistributiveDataflowProblem {
    type Fact: Copy + Eq + Hash + Ord;

    /// The distinguished fact injected at every backward seed node and
    ...
    /// Compute predecessor facts for a call-to-entry edge.
    ...
    /// Compute predecessor facts for a matched return edge.
    ...
    /// Compute predecessor facts for a call-to-continuation edge.
    ...
    /// Compute predecessor facts for local exceptional or async-exceptional
    /// control flow.
...
/// Snapshot-specific demands for a bounded backward solve.
pub trait BoundedSnapshotBackwardDataflowProblem: BackwardDistributiveDataflowProblem {
    /// Append every explicit, context-specific demanded fact for this snapshot
    /// run. The kernel traverses from these demands toward their predecessors.
