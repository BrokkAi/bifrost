...
/// One call that produced a callee entry fact, with the caller-relative path
/// that reached the call.
///
/// A callee-relative path edge is only realizable in a caller context, and the
/// entry fact alone does not say where the value came from. Without this row a
/// client observing a fact inside a callee -- a taint sink in a helper, say --
/// can see *that* the entry was tainted but has no evidence of *what* tainted
/// it, because the callee's own witness begins at its entry seed. The retained
/// call witness ends at that seed and begins at the caller's own entry, so
/// composing the two yields the exact realizable path (#2546).
#[derive(Debug, Clone)]
pub struct SummaryIncomingCall {
    callee: SummaryEntry,
    caller: SummaryEntry,
...
impl PartialEq for SummaryIncomingCall {
    fn eq(&self, other: &Self) -> bool {
        self.callee == other.callee
            ...
            && self.call_point == other.call_point
            && self.call_fact == other.call_fact
            && self.origin == other.origin
            && self.path_qualities == other.path_qualities
    }
}

impl Eq for SummaryIncomingCall {}

impl SummaryIncomingCall {
    ...
    pub(crate) fn new(
        ...
        debug_assert_eq!(
            caller.procedure(),
            call_point.procedure(),
            "an incoming call site must belong to its caller entry procedure"
        );
        Self {
            callee,
            caller,
    ...
    pub const fn callee(&self) -> &SummaryEntry {
        &self.callee
    }

    pub const fn caller(&self) -> &SummaryEntry {
        &self.caller
    }
...
impl<Fact> SummaryDataflowResult<Fact> {
    ...
    ///
    /// A solve that retained no witnesses publishes none of these rows: their
    /// content is the retained call witness, and without one the row answers
    /// nothing a client can use.
    pub fn incoming_calls(&self) -> &[SummaryIncomingCall] {
        &self.incoming_calls
    }

    /// Every call that produced `entry`'s entry fact from a caller context.
    pub fn incoming_calls_for<'result>(
        &'result self,
        entry: &'result SummaryEntry,
    ) -> impl Iterator<Item = &'result SummaryIncomingCall> + 'result {
        self.incoming_calls
            .iter()
            .filter(move |incoming| incoming.callee() == entry)
    }
    ...
    pub fn retained_bytes(&self) -> usize {
        size_of::<Self>()
            ...
            .saturating_add(size_of_val(self.incoming_calls()))
            .saturating_add(
                self.incoming_calls
                    .iter()
                    .map(SummaryIncomingCall::retained_bytes)
                    .fold(0_usize, usize::saturating_add),
            )
    ...
    /// The caller-relative witness that reached `incoming`'s call and produced
    /// its callee entry fact.
    ///
    /// The reconstructed path begins at the caller entry's seed and ends at the
    /// call edge whose target is the callee entry point, so its last step's
    /// output fact is exactly `incoming.callee().entry_fact()`.
    pub fn witness_for_incoming_call(
        &self,
    ...
    pub fn witness_for_incoming_call_derivation(
        ...
        let incoming = self
            ...
            .find(|candidate| {
                *candidate == incoming
                    && witness_owners_match(
                        candidate.witness_owner.as_ref(),
                        incoming.witness_owner.as_ref(),
                    )
            })
            .ok_or(SummaryWitnessError::TargetNotInResult)?;
        // Same dominance rule as `witness_for_reached_index` (#1917).
        ...
        self.witness_store.reconstruct(
            evidence,
            quality,
            incoming.witnesses.is_truncated(quality),
            limits,
            WitnessTarget::Reached {
                entry_point: incoming.caller.entry_point(),
                entry_fact: incoming.caller.entry_fact(),
                point: incoming.callee.entry_point(),
                fact: incoming.callee.entry_fact(),
            },
        )
    }
...
fn compare_incoming_calls(left: &SummaryIncomingCall, right: &SummaryIncomingCall) -> Ordering {
    compare_entries(left.callee(), right.callee())
        .then_with(|| compare_entries(left.caller(), right.caller()))
