...
use super::transfer::{
    TransferEvaluation, TransferScratch, evaluate_backward_transfer, evaluate_transfer,
};
use super::{
    ...
    Fact: Copy + Eq + std::hash::Hash + Ord,
{
    ...
    fn initialize_backward<P>(
        &mut self,
        problem: &P,
        request: &mut DataflowRequest<'_>,
    ) -> Result<Option<SolverTermination>, DataflowError>
    where
        P: BoundedSnapshotBackwardDataflowProblem<Fact = Fact>,
    {
        if request.cancellation.is_cancelled() {
            return Ok(Some(SolverTermination::Cancelled));
        }
        ...
        let (invalid_node, sink_exceeded, emitted_seeds) = {
            let mut seed_outputs =
                BoundedSeedOutputs::new(self.snapshot, request.budget, request.cancellation);
            problem.backward_seeds(&mut seed_outputs);
            let invalid_node = seed_outputs.invalid_node();
            let exceeded = seed_outputs.exceeded();
            (invalid_node, exceeded, seed_outputs.into_values())
        };
    ...
    fn initialize_seed_values(
        &mut self,
        zero_fact: Fact,
        invalid_node: Option<IcfgNodeId>,
        sink_exceeded: Option<SolverBudgetExceeded>,
        emitted_seeds: HashSet<DataflowSeed<Fact>>,
        request: &mut DataflowRequest<'_>,
    ) -> Result<Option<SolverTermination>, DataflowError> {
    ...
    fn propagate_backward<P>(
        &mut self,
        problem: &P,
        request: &mut DataflowRequest<'_>,
    ) -> Result<SolverTermination, DataflowError>
    where
        P: BackwardDistributiveDataflowProblem<Fact = Fact>,
    {
        while let Some(queued) = self.worklist.pop_front() {
            if request.cancellation.is_cancelled() {
                return Ok(SolverTermination::Cancelled);
            ...
            for (edge_id, edge) in self.snapshot.predecessor_edges(queued.state.node) {
                ...
                // `edge` is still the original source-to-target descriptor;
                // only the adjacency traversal is reversed.
                let descriptor = DataflowEdge::from_snapshot(self.snapshot, edge_id)
                    .expect("validated ICFG edge remains in its immutable snapshot");
                let canonical_outputs = match evaluate_backward_transfer(
                    problem,
                    descriptor,
                    output_fact,
                    self.facts[ZERO_FACT_ID.index()],
                ...
                let output_quality = queued.quality.through_edge(edge);
                if let Some(termination) =
                    self.publish_outputs(edge.source, output_quality, &canonical_outputs, request)?
                {
                    return Ok(termination);
                }
            }
...
/// Solve one finite distributive may-data-flow problem over an already
/// materialized bounded ICFG snapshot.
///
/// Snapshot construction is intentionally outside this function. A caller
/// that plans more than one direction can therefore materialize one snapshot,
/// estimate both directions, and invoke only the selected solver without
/// repeating semantic provider work.
pub fn solve_on_snapshot<P>(
...
///
/// This compatibility wrapper retains the original entry point. Use
/// [`solve_on_snapshot`] when the caller has already materialized the input
/// snapshot as part of a larger query plan.
pub fn solve<P>(
    input: IcfgSolveInput<'_>,
    problem: &P,
    request: &mut DataflowRequest<'_>,
...
where
    P: BoundedSnapshotDataflowProblem,
{
    solve_on_snapshot(input, problem, request)
}

/// Solve one finite distributive predecessor relation over an already
/// materialized bounded ICFG snapshot.
///
/// Backward callbacks receive each edge in its original source-to-target
/// orientation while the kernel visits incoming edges and publishes outputs at
/// the edge source. This keeps edge-kind and semantic-boundary attribution
/// stable for clients implementing explicit preimage relations. Snapshot
/// construction is intentionally outside this function so forward and
/// backward plans can share one immutable graph.
pub fn solve_backward_on_snapshot<P>(
    input: IcfgSolveInput<'_>,
    problem: &P,
    request: &mut DataflowRequest<'_>,
) -> Result<DataflowResult<P::Fact>, DataflowError>
where
    P: BoundedSnapshotBackwardDataflowProblem,
{
    let initial_work = request.budget.used();
    let mut state = TabulationState::new(input.snapshot());

    let termination = if let Some(termination) = state.initialize_backward(problem, request)? {
        termination
    } else {
        state.propagate_backward(problem, request)?
    };
    let work = request.budget.used().saturating_sub(initial_work);
    Ok(state.finish(input.status(), termination, work))
}

/// Solve one finite distributive predecessor relation over a bounded ICFG.
///
/// This compatibility wrapper retains the original entry point. Use
/// [`solve_backward_on_snapshot`] when the caller has already materialized the
/// input snapshot as part of a larger query plan.
pub fn solve_backward<P>(
    input: IcfgSolveInput<'_>,
    problem: &P,
    request: &mut DataflowRequest<'_>,
) -> Result<DataflowResult<P::Fact>, DataflowError>
where
    P: BoundedSnapshotBackwardDataflowProblem,
{
    solve_backward_on_snapshot(input, problem, request)
}
