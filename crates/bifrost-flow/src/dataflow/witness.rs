...
impl SummaryWitness {
    ...
    /// Join this call witness with the callee-relative path it entered.
    ///
    /// `self` must end at the call edge whose target is `suffix`'s entry point,
    /// and `suffix` must begin at that entry's seed. The seed is dropped and the
    /// two step sequences join into one realizable caller-to-target path, which
    /// is what a client needs to attribute a callee-relative observation to the
    /// caller context that produced its entry fact (#2546).
    ///
    /// Returns `None` when either side retained no steps: a zero-step witness is
    /// an availability marker, not a path to join.
    pub fn joined_at_call(&self, suffix: &Self, max_steps: usize) -> Option<Self> {
        if self.steps.is_empty() || suffix.steps.is_empty() {
            return None;
        }
        let mut steps = self.steps.to_vec();
        steps.extend(suffix.steps.iter().skip(1).cloned());
        let mut truncation_cause = self.truncation_cause.or(suffix.truncation_cause);
        let mut omitted_steps_lower_bound = self
            .omitted_steps_lower_bound
            .saturating_add(suffix.omitted_steps_lower_bound);
        if steps.len() > max_steps {
            omitted_steps_lower_bound =
                omitted_steps_lower_bound.saturating_add(steps.len() - max_steps);
            truncation_cause =
                truncation_cause.or(Some(WitnessTruncationCause::ReconstructionStepLimit));
        ...
        Some(Self::from_parts(
            steps,
            self.quality.conjoin(suffix.quality),
            truncation_cause,
            omitted_steps_lower_bound,
            self.alternatives_truncated || suffix.alternatives_truncated,
            retained_bytes,
            WitnessReconstructionWork::new(
                self.work
                    .evidence_expansions()
                    .saturating_add(suffix.work.evidence_expansions()),
                emitted_steps,
            ),
        ))
    }
