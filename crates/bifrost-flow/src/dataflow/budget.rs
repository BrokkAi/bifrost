...
use brokk_bifrost_core::analyzer::work_budget::{
    BudgetLedger, WorkBudgetExceeded, define_work_dimensions,
};
...
pub struct DataflowRequest<'request> {
    ...
    query_plan_config: super::DataflowQueryPlanConfig,
}
...
impl<'request> DataflowRequest<'request> {
    pub const fn new(
        ...
        Self {
            budget,
            cancellation,
            query_plan_config: super::DataflowQueryPlanConfig::new(
                super::DataflowDirectionRequest::Auto,
            ),
        }
    }

    /// Return a request carrying a query-plan configuration.
    pub const fn with_query_plan_config(
        mut self,
        query_plan_config: super::DataflowQueryPlanConfig,
    ) -> Self {
        self.query_plan_config = query_plan_config;
        self
    }

    /// Return the direction-planning controls owned by this request.
    pub const fn query_plan_config(&self) -> super::DataflowQueryPlanConfig {
        self.query_plan_config
    }
