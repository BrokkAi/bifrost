...
use crate::value_flow::{ValueFlowCarrierKey, ValueFlowEventKey, ValueFlowObservationPhase};
...
pub struct CarrierSummaryKey {
    ...
    carrier_contract: crate::value_flow::ValueFlowCarrierSummaryIdentity,
    schema_version: u32,
}
        ...
        size_of_val(self.sources.as_ref())
            ...
            .saturating_add(self.transfers.iter().fold(0usize, |total, transfer| {
                total.saturating_add(match transfer {
                    ...
                    StablePropagationTransfer::LocalTransform {
                        input,
                        output,
                        function,
                        ..
...
impl TaintContractSet {
    ...
    fn try_new(
        ...
        for binding in plan.local_transforms() {
            if request.cancellation.is_cancelled() {
                return Ok(Self::empty());
            }
            let Some(builder) = builders.get_mut(binding.point().procedure()) else {
            ...
            builder
                .transfers
                .push(StablePropagationTransfer::LocalTransform {
                    point: binding.point().id(),
                    input: plan
                        .value_flow()
    ...
    Provider: IcfgProvider + ?Sized,
{
    ...
    let result = {
        ...
        solve_taint_with_reusable_provider(
            ...
            &mut reusable,
            plan,
            crate::dataflow::WitnessRetentionLimits::disabled(),
            semantic_budget,
            request,
        )?
    };
