...
use crate::taint::{TaintAnalysisPlan, TaintBatchCompatibilityKey, TaintFindingReport};
...
pub struct ProductionTaintAnalysisResult {
    ...
    projection_limits: TaintProjectionLimits,
...
/// Query-neutral bounds retained with a production taint result.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct TaintProjectionLimits {
...
impl ProductionTaintAnalysisResult {
        ...
        plan.for_each_retained_artifact(|artifact| {
            ...
            if artifact_allocations.insert(Arc::as_ptr(artifact)) {
                retained_artifact_bytes = retained_artifact_bytes.saturating_add(
                    crate::analyzer::semantic::semantic_artifact_retained_bytes(artifact),
                );
            }
        });
    ...
    pub const fn projection_limits(&self) -> TaintProjectionLimits {
    ...
    pub fn set_registration_digest<T: Serialize + ?Sized>(
        &mut self,
        findings: &T,
        ...
        // The compatibility key already covers the workspace snapshot, the
        // root, the value flow's propagation semantics, and the sanitizers,
        // each length-delimited inside its own domain.
        ...
        digest.push(&self.compatibility.universe().bytes());
        digest.push(
            self.expected_root()
                .artifact()
                .key()
        ...
        for limit in [
            self.projection_limits.max_origins_per_finding,
            self.projection_limits.max_witnesses_per_finding,
            self.projection_limits.max_steps_per_witness,
            self.projection_limits.max_witness_bytes,
        ] {
            digest.push(&digest_length(limit).to_le_bytes());
        }
        // The count keeps a shorter key list followed by the findings from
        // aligning with a longer one.
        digest.push(&digest_length(self.artifact_keys.len()).to_le_bytes());
        for key in &self.artifact_keys {
            digest.push(key.fingerprint().as_bytes());
        }
...
fn digest_length(value: usize) -> u64 {
