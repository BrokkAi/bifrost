...
use crate::dataflow::{
    ...
    BackwardSnapshotDataflowResult, BackwardSnapshotDemand, BackwardSnapshotProblem,
...
/// A demanded typestate observation for a backward solve.
///
/// The fact is interpreted at `point` as an observation on the successor
/// side of the edge. The backward client computes the finite protocol-state
/// preimage rather than pretending that running the forward transfer over
/// reversed edges is sound.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct TypestateBackwardDemand {
    point: crate::analyzer::semantic::ProgramPointHandle,
    fact: TypestateFact,
}

impl TypestateBackwardDemand {
    pub const fn new(
        point: crate::analyzer::semantic::ProgramPointHandle,
        fact: TypestateFact,
    ) -> Self {
    ...
    pub const fn point(&self) -> &crate::analyzer::semantic::ProgramPointHandle {
        &self.point
    }
...
/// Plan a typestate query against one already materialized ICFG snapshot.
///
/// Initial-seed binding sites provide forward endpoints, while demanded
/// observations provide backward endpoints. The planner counts the unique
/// snapshot nodes and edges reachable from those endpoints and uses the
/// binding rows as a conservative transfer fan-out estimate. No semantic
/// provider or solver is invoked here, so a caller can reuse the same
/// immutable snapshot for the selected solve without rebuilding it.
///
/// `requirements` remains explicit because typestate has two distinct query
/// families: bounded snapshot propagation can use the finite backward
/// preimage solver, while reusable summaries and normalized witnesses remain
/// forward-only. The adapter supplies the bounded-snapshot and complete
/// reverse-semantics requirements unconditionally, and derives capabilities
/// from the engine's current support plus typestate's exact finite preimage
/// implementation. Passing output requirements through the generic planner
/// preserves typed unsupported-direction errors instead of silently claiming
/// an unavailable backward result.
pub fn plan_typestate_query(
    ...
    let forward_seed_points = bindings
        .initial_seeds()
        .iter()
        .filter_map(|binding| binding.site().program_point_handle())
        .collect::<Vec<_>>();
    let backward_demand_points = demands
        .iter()
        .map(|demand| demand.point())
        .collect::<Vec<_>>();
    let forward_seed_nodes = snapshot_node_ids_for_points(snapshot, &forward_seed_points);
    let backward_demand_nodes = snapshot_node_ids_for_points(snapshot, &backward_demand_points);
    let transfer_fanout = bindings
        .event_bindings()
        .len()
    ...
    plan_snapshot_dataflow_direction(
        ...
        transfer_fanout,
        transfer_fanout,
        bindings.initial_seeds().len(),
        demands.len(),
        typestate_query_capabilities(),
        requirements,
    )
}

fn typestate_query_capabilities() -> DataflowDirectionCapabilities {
    let mut capabilities = DataflowDirectionCapabilities::current();
    // `TypestateFlowProblem::predecessor_transfer` enumerates the finite
    // protocol state and uncertainty domains and checks the existing forward
    // relation, so its bounded reverse model is complete for this client.
    capabilities.backward_reverse_semantics_complete = true;
    capabilities
}

#[derive(Debug)]
pub enum TypestateBackwardSolveError {
    Contract(TypestateFlowProblemError),
    Dataflow(BackwardSnapshotDataflowError),
}

impl fmt::Display for TypestateBackwardSolveError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Contract(error) => error.fmt(formatter),
            Self::Dataflow(error) => error.fmt(formatter),
...
impl std::error::Error for TypestateBackwardSolveError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::Contract(error) => Some(error),
            Self::Dataflow(error) => Some(error),
...
impl From<TypestateFlowProblemError> for TypestateBackwardSolveError {
    fn from(error: TypestateFlowProblemError) -> Self {
        Self::Contract(error)
    }
}

impl From<BackwardSnapshotDataflowError> for TypestateBackwardSolveError {
    fn from(error: BackwardSnapshotDataflowError) -> Self {
        Self::Dataflow(error)
    }
}

/// The result of one root-scoped typestate backward solve.
#[derive(Debug, Clone)]
pub struct TypestateBackwardResult {
...
impl TypestateBackwardResult {
    ...
    pub const fn result(&self) -> &BackwardSnapshotDataflowResult<TypestateFact> {
        &self.result
    }
...
impl<'plan> TypestateFlowProblem<'plan> {
    ...
    /// Construct a terminal observation fact suitable for a backward demand.
    pub fn terminal_fact(
        &self,
        subject: &TypestateSubjectKey,
            ...
            .find(|binding| {
                binding.subject() == subject
                    && self
                        ...
                        .is_some_and(|terminal| terminal.key() == expectation)
            })
            .ok_or(TypestateFlowProblemError::InvalidFactIdentity)?;
        ...
        for demand in demands {
            match demand.fact.0 {
                ...
                TypestateFactKind::Violation {
                    ...
                    if plan != self.bindings.hash()
                        ...
                        || self
                            ...
                            .is_none()
                        || self.protocol.state_key(violation.from).is_none()
                        || self.protocol.state_key(violation.to).is_none()
                    {
                        return Err(TypestateFlowProblemError::InvalidFactIdentity);
                    }
                }
                TypestateFactKind::NonViolation {
                    ...
                    if plan != self.bindings.hash()
                        || self.bindings.subject(subject).is_none()
                        || self.bindings.event_binding(event_binding).is_none()
                    {
                        return Err(TypestateFlowProblemError::InvalidFactIdentity);
                    }
                }
    ...
    /// Compute the exact finite preimage of one successor fact.
    ///
    /// Typestate's state component is finite and the forward transfer is
    /// distributive over the observation facts. Enumerating that finite state
    /// domain and checking the already-defined forward relation gives an
    /// explicit predecessor relation, including event observations,
    /// violations, terminal states, and uncertainty flags. This is more
    /// precise than reversing control-flow edges and reusing a forward state
    /// transition, which cannot invert kills or observation side effects.
    fn predecessor_transfer(
        &self,
        edge: DataflowEdge<'_, TypestateFact>,
        output_fact: TypestateFact,
        family: TransferFamily,
        out: &mut dyn DataflowOutput<TypestateFact>,
    ) {
        if !out.should_continue() {
        ...
        let subjects = match output_fact.subject() {
        ...
        for subject in subjects {
            let Some(subject_binding) = self.bindings.subject(subject) else {
                continue;
            };
            let output_uncertainty = output_fact.uncertainty().0;
            let abstained_candidates: &[bool] = if output_fact.abstained() {
                &[false, true]
            } else {
                &[false]
            };
            let uncertainty_candidates = (0..=u8::MAX)
                .filter(|candidate| candidate & !output_uncertainty == 0)
                .map(TypestateUncertaintySet)
                .collect::<Vec<_>>();
            let mut candidates = Vec::with_capacity(
                self.protocol
                    .states()
                    .len()
                    .saturating_mul(uncertainty_candidates.len())
                    .saturating_mul(abstained_candidates.len())
                    .saturating_add(1),
            );
            candidates.push(TypestateFact::zero());
            for (state, _) in self.protocol.states() {
                for uncertainty in &uncertainty_candidates {
                    for abstained in abstained_candidates {
                        candidates.push(TypestateFact::summary_state(
                            self.bindings.hash(),
                            subject_binding.id(),
                            state,
                            *uncertainty,
            ...
            for candidate in candidates {
                ...
                let mut outputs = TypestateFactOutput::default();
                self.transfer(edge, candidate, family, &mut outputs);
                if outputs.facts.contains(&output_fact) && !out.emit(candidate) {
                    return;
                }
            }
        }
...
struct TypestateBackwardSnapshotProblem<'problem, 'demands> {
    problem: &'problem TypestateFlowProblem<'problem>,
    demands: &'demands [TypestateBackwardDemand],
}

impl BackwardSnapshotProblem for TypestateBackwardSnapshotProblem<'_, '_> {
...
impl BackwardDistributiveDataflowProblem for TypestateBackwardSnapshotProblem<'_, '_> {
    type Fact = TypestateFact;

    fn zero_fact(&self) -> Self::Fact {
        BackwardDistributiveDataflowProblem::zero_fact(self.problem)
    }
...
/// Solve a root-scoped typestate query from demanded successor observations.
///
/// The snapshot adapter supplies context-expanded predecessor traversal while
/// this client supplies the protocol-specific finite preimage relation. The
/// result retains the generic coverage, termination, uncertainty, and
/// incoming-call evidence from that bounded solve.
#[allow(clippy::too_many_arguments)]
pub fn solve_typestate_backward_on_snapshot(
    root: &ProcedureHandle,
    demands: &[TypestateBackwardDemand],
    ...
    let result =
        solve_backward_demands_on_snapshot(input, &backward_problem, snapshot_work, request)?;
    let execution_complete = result
        .facts()
        .iter()
        .all(|fact| fact.uncertainty().is_empty() && !fact.abstained());
...
/// Solve a root-scoped typestate query from demanded successor observations.
///
/// This compatibility wrapper retains the provider-backed entry point while
/// the planned client uses [`solve_typestate_backward_on_snapshot`] to avoid
/// rebuilding an already materialized snapshot.
#[allow(clippy::too_many_arguments)]
pub fn solve_typestate_backward_with_snapshot<Provider>(
    root: &ProcedureHandle,
    demands: &[TypestateBackwardDemand],
    ...
    Provider: IcfgProvider + ?Sized,
{
    let mut semantic_request =
        crate::analyzer::semantic::SemanticRequest::new(semantic_budget, request.cancellation);
    let outcome = provider
        .snapshot(root, limits, &mut semantic_request)
        .map_err(BackwardSnapshotDataflowError::SemanticProvider)?;
    let snapshot_work = outcome.work();
    let status = IcfgInputStatus::from_outcome(&outcome);
    let input = IcfgSolveInput::from_outcome(&outcome).map_err(|_| {
        TypestateBackwardSolveError::Dataflow(BackwardSnapshotDataflowError::MissingIcfgSnapshot {
            status,
        })
    })?;
    solve_typestate_backward_on_snapshot(
        root,
        demands,
        input,
        snapshot_work,
...
impl BackwardDistributiveDataflowProblem for TypestateFlowProblem<'_> {
    type Fact = TypestateFact;
    ...
    fn normal_predecessor_flow(
        ...
        self.predecessor_transfer(edge, output_fact, TransferFamily::Normal, out);
    }
    ...
    fn call_predecessor_flow(
        ...
        self.predecessor_transfer(edge, output_fact, TransferFamily::Call, out);
    }
    ...
    fn return_predecessor_flow(
        ...
        self.predecessor_transfer(edge, output_fact, TransferFamily::Return, out);
    }
    ...
    fn call_to_return_predecessor_flow(
        ...
        self.predecessor_transfer(edge, output_fact, TransferFamily::CallToReturn, out);
    }
    ...
    fn exceptional_predecessor_flow(
        ...
        self.predecessor_transfer(edge, output_fact, TransferFamily::Exceptional, out);
    }
}
...
#[derive(Default)]
struct TypestateFactOutput {
    facts: Vec<TypestateFact>,
}

impl DataflowOutput<TypestateFact> for TypestateFactOutput {
