...
pub struct GoldenRealm {
    ...
    /// This realm's pack-content version. It advances when the realm's shipped
    /// claims change, independently of other realms generated by this foundry.
...
pub const PYTHON_REALM: GoldenRealm = GoldenRealm {
    ...
    provenance_revision: "golden-core",
    // Exact import resolution classifies module-level functions as package
    // functions. The module qualifier contributes owner identity, not a bound
    // receiver, so generated summaries must use the same receiverless key as
    // declaration packs and resolver-owned external targets (#2438).
    qualified_static_call_has_receiver: false,
};
...
pub const JAVASCRIPT_REALM: GoldenRealm = GoldenRealm {
    ...
    // the owner and a receiver too (#2598). The specifier is canonical, with
    // Node's `node:` scheme folded away, so one entry per module serves both
    // spellings (#2609).
...
/// receiverless boundary as the authored static target. Exact Python import
/// resolution likewise treats a module qualifier as owner identity rather
/// than a bound receiver.
///
...
mod tests {
    ...
    fn the_python_realm_ships_a_python_pinned_pack_from_the_same_converter() {
        ...
        // A Python module qualifier contributes owner identity, not a bound
        // receiver, so the generated summary matches the exact import proof.
        assert_eq!(
