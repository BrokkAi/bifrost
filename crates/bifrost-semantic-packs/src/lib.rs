...
///
/// Construction is cheap and does not decode or globally register content.
/// [`EmbeddedPackRegistry::register_all`] performs validation and explicit
/// catalog registration when a driver opts into Bifrost's bundled packs.
...
/// An explicitly registered set of Bifrost-shipped packs.
#[derive(Debug, Clone, Copy)]
...
impl<'a> EmbeddedPackRegistry<'a> {
    ...
    /// Decode every embedded artifact before mutating the catalog, then
    /// register each pack as session-owned embedded content. Callers should use
    /// the same limits configured on the target catalog.
    pub fn register_all(
        &self,
...
///
/// Issues #1152 and #1153 will add reviewed content through this registry. It
/// deliberately begins empty: distribution infrastructure must not invent or
/// silently activate unreviewed semantic behavior.
