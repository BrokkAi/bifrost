...
policy_records! {
    ...
    Result { labels: ["result"], layout: KeywordPairs, owner: OwnerApplicability::BOTH, signature: "(result :index N)", description: "Bind one zero-based normal call result." }
    ...
    StoreWrite { labels: ["store-write"], layout: KeywordPairs, owner: OwnerApplicability::POLICY_TAINT, signature: "(store-write :id ID :selector SELECTOR :store STORE [:key PORT] [:instance PORT] :input PORT)", description: "Declare that the selected call writes its input port's value into the named persistence store, optionally discriminated by a key port and an instance port." }
    StoreRead { labels: ["store-read"], layout: KeywordPairs, owner: OwnerApplicability::POLICY_TAINT, signature: "(store-read :id ID :selector SELECTOR :store STORE [:key PORT] [:instance PORT] :output PORT)", description: "Declare that the selected call's output port returns data from the named persistence store under the same optional key and instance dimensions." }
    EntryPoint { labels: ["entry-point"], layout: KeywordPairs, owner: OwnerApplicability::POLICY_TAINT, signature: "(entry-point :id ID :selector SELECTOR :parameter (argument ...) :labels [LABEL...])", description: "Declare one framework-invoked entry point: the analysis roots the selected procedure declaration and taints the bound formal parameter on entry." }
    ...
    OnUnknown { labels: ["on-unknown"], layout: KeywordPairs, owner: OwnerApplicability::POLICY_ALL, signature: "(on-unknown :verdict abstain|warn-unreliable|fail-closed|treat-may-as-finding)", description: "Declare what the run does when unknown or incomplete evidence blocked a verdict." }
...
impl FieldContextApplicability {
    pub const fn allows(self, context: PolicyRecordContext) -> bool {
        match self {
            ...
            Self::TaintCatalogSetsOnly => matches!(
                context,
        ...
        impl PolicyValueShape {
            ...
            pub fn atom_domain(self) -> Option<AtomDomain> {
                match self {
                    ...
                    | Self::ExternalModelPort
                    | Self::ExternalModelWritePort => Some(AtomDomain::CallPort),
                    ...
                    | Self::StoreEntries
                    | Self::EntryPointEntries
                    | Self::EntryPointParameter
                    | Self::OriginEntries
            ...
            pub fn accepted_records(self) -> &'static [PolicyRecord] {
                match self {
                    ...
                    | Self::TypestateCallBinding
                    | Self::ExternalModelPort => &[PolicyRecord::Argument, PolicyRecord::Result],
                    Self::ExternalModelWritePort => &[
                        PolicyRecord::Argument,
                        PolicyRecord::Result,
                        PolicyRecord::Field,
                    ],
                    Self::EndpointSemantics => &[
                    ...
                    Self::StoreEntries => &[PolicyRecord::StoreWrite, PolicyRecord::StoreRead],
                    Self::EntryPointEntries => &[PolicyRecord::EntryPoint],
                    Self::EntryPointParameter => &[PolicyRecord::Argument],
...
value_shapes! {
    ...
    StoreName => "a lowercase persistence store name",
    ...
    RowPredicates => "zero or more typed row predicates, each (BINDING.FIELD eq|ne|lt|le|gt|ge VALUE-OR-FIELD), (BINDING.FIELD is-null|is-not-null), or (BINDING.FIELD in (VALUE...))",
    RowJoinKind => "inner, semi, or anti",
    RowAggregateOp => "min, max, count, count-distinct, any, all, ordered-equal, set-equal, or subset",
    RowAggregateOperand => "a stable-key COLUMN for set-equal/subset or a (POSITION-FIELD VALUE-FIELD) ordered sequence",
    ...
    EndpointBinding => "matched-value, receiver, return-value, an indexed result record, or an argument record",
    ...
    TypestateBinding => "matched-value, receiver, return-value, an indexed result record, or an argument record",
    TypestateCallBinding => "receiver, return-value, an indexed result record, or an argument record",
    ...
    StoreEntries => "policy-local store-write and store-read records",
    EntryPointEntries => "policy-local entry-point records",
    EntryPointParameter => "an argument record naming the tainted formal parameter",
...
macro_rules! policy_field_context {
    ...
    (TaintCatalogSetsOnly) => {
        FieldContextApplicability::TaintCatalogSetsOnly
    };
    (FlowOriginsOnly) => {
...
policy_fields! {
    ...
    AnalysisOnUnknown { record: Analysis, labels: ["on-unknown"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: OnUnknownSpec, owner: OwnerApplicability::POLICY_ALL, signature: ":on-unknown (on-unknown :verdict abstain|warn-unreliable|fail-closed|treat-may-as-finding)", description: "Choose what a verdict blocked by unknown evidence does; omission defaults to abstain." }
    ...
    BindQuery { record: Bind, labels: ["query"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: Selector, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":query (rql ...)|(rql-file ...)", description: "Execute one typed CodeQuery as the binding source." }
    ...
    JoinKind { record: Join, labels: ["kind"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: RowJoinKind, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":kind inner|semi|anti", description: "Choose an inner join, a semi join that keeps matching left rows without the right relation columns, or an anti-join that keeps left rows with no partner; omission means inner." }
    ...
    AggregateOp { record: Aggregate, labels: ["op"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: RowAggregateOp, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":op min|max|count|count-distinct|any|all|ordered-equal|set-equal|subset", description: "Choose the bounded aggregate operation." }
    AggregateValue { record: Aggregate, labels: ["value"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: RowFieldRef, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":value BINDING.FIELD", description: "Select the typed input field the fold reads: an integer field for min or max, a boolean field for any or all, any field for count-distinct." }
    AggregateWhere { record: Aggregate, labels: ["where"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: ValueMultiplicity::sequence(0, 16), shape: RowPredicates, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":where ((BINDING.FIELD OP VALUE)...)", description: "Conjoin bounded typed predicates a row must satisfy to contribute to the fold." }
    AggregateLeft { record: Aggregate, labels: ["left"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: RowAggregateOperand, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":left COLUMN|(BINDING.POSITION BINDING.VALUE)", description: "Name the left stable-key column for set-equal/subset, or the integer position and value columns for ordered-equal." }
    AggregateRight { record: Aggregate, labels: ["right"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: RowAggregateOperand, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":right COLUMN|(BINDING.POSITION BINDING.VALUE)", description: "Name the right stable-key column for set-equal/subset, or the ordered-equal sequence; both sides must have the same scalar type." }
    ...
    ResultIndex { record: Result, labels: ["index"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: NonNegativeInteger, owner: OwnerApplicability::BOTH, signature: ":index N", description: "Bind the zero-based normal result ordinal." }
    ...
    StoreWriteId { record: StoreWrite, labels: ["id"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: LocalEntryId, owner: OwnerApplicability::POLICY_TAINT, signature: ":id \"entry-id\"", description: "Set the store-write identity in the shared local taint entry namespace." }
    StoreWriteSelector { record: StoreWrite, labels: ["selector"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: Selector, owner: OwnerApplicability::POLICY_TAINT, signature: ":selector SELECTOR", description: "Select the store-writing calls." }
    StoreWriteStore { record: StoreWrite, labels: ["store"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: StoreName, owner: OwnerApplicability::POLICY_TAINT, signature: ":store STORE", description: "Name the persistence store this call writes into; write and read entries sharing one store name form a persistence boundary." }
    StoreWriteKey { record: StoreWrite, labels: ["key"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: PolicyPort, owner: OwnerApplicability::POLICY_TAINT, signature: ":key PORT", description: "Optionally bind the port carrying the key the write is stored under." }
    StoreWriteInstance { record: StoreWrite, labels: ["instance"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: PolicyPort, owner: OwnerApplicability::POLICY_TAINT, signature: ":instance PORT", description: "Optionally bind the port carrying the store instance the write targets." }
    StoreWriteInput { record: StoreWrite, labels: ["input"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: PolicyPort, owner: OwnerApplicability::POLICY_TAINT, signature: ":input PORT", description: "Bind the port whose value the call writes into the store." }
    StoreReadId { record: StoreRead, labels: ["id"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: LocalEntryId, owner: OwnerApplicability::POLICY_TAINT, signature: ":id \"entry-id\"", description: "Set the store-read identity in the shared local taint entry namespace." }
    StoreReadSelector { record: StoreRead, labels: ["selector"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: Selector, owner: OwnerApplicability::POLICY_TAINT, signature: ":selector SELECTOR", description: "Select the store-reading calls." }
    StoreReadStore { record: StoreRead, labels: ["store"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: StoreName, owner: OwnerApplicability::POLICY_TAINT, signature: ":store STORE", description: "Name the persistence store this call reads from; write and read entries sharing one store name form a persistence boundary." }
    StoreReadKey { record: StoreRead, labels: ["key"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: PolicyPort, owner: OwnerApplicability::POLICY_TAINT, signature: ":key PORT", description: "Optionally bind the port carrying the key the read looks up." }
    StoreReadInstance { record: StoreRead, labels: ["instance"], placement: FieldPlacement::Keyword, required: Optional, multiplicity: SCALAR, shape: PolicyPort, owner: OwnerApplicability::POLICY_TAINT, signature: ":instance PORT", description: "Optionally bind the port carrying the store instance the read targets." }
    StoreReadOutput { record: StoreRead, labels: ["output"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: PolicyPort, owner: OwnerApplicability::POLICY_TAINT, signature: ":output PORT", description: "Bind the port that returns data read from the store." }
    EntryPointId { record: EntryPoint, labels: ["id"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: LocalEntryId, owner: OwnerApplicability::POLICY_TAINT, signature: ":id \"entry-id\"", description: "Set the entry-point identity." }
    ...
    TransferFrom { record: Transfer, labels: ["from"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: ExternalModelPort, owner: OwnerApplicability::POLICY_TAINT, signature: ":from receiver|return-value|(result :index N)|(argument ...)", description: "Bind the transfer input call port; matched-value is forbidden." }
    FieldName { record: Field, labels: ["name"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: Name, owner: OwnerApplicability::POLICY_TAINT, signature: ":name \"field-name\"", description: "Name the written field on the base port's object." }
    FieldOfBase { record: Field, labels: ["of"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: ExternalModelPort, owner: OwnerApplicability::POLICY_TAINT, signature: ":of (argument ...)|receiver", description: "Bind the object whose field is written: an argument or the receiver." }
    TransferTo { record: Transfer, labels: ["to"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: ExternalModelWritePort, owner: OwnerApplicability::POLICY_TAINT, signature: ":to receiver|return-value|(result :index N)|(argument ...)|(field :name \"NAME\" :of PORT)", description: "Bind the transfer output call port or one written field of an argument or receiver; matched-value is forbidden." }
    ...
    OnUnknownVerdict { record: OnUnknown, labels: ["verdict"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: UnknownVerdict, owner: OwnerApplicability::POLICY_ALL, signature: ":verdict abstain|warn-unreliable|fail-closed|treat-may-as-finding", description: "Abstain and exit unreliable, gate on retained findings alone, fail closed, or publish explicit may evidence as possible findings while preserving coverage obligations." }
    ...
    CallsSubject { record: Calls, labels: ["subject"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: TypestateCallBinding, owner: OwnerApplicability::POLICY_TYPESTATE, signature: ":subject receiver|return-value|(result :index N)|(argument ...)", description: "Bind the already tracked object at the call." }
    ...
    ValueOriginAssertId { record: AssertValueOrigin, labels: ["id"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: LocalEntryId, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":id \"assert-id\"", description: "Set the stable assertion identity used by finding anchors and messages." }
    ValueOriginAssertAt { record: AssertValueOrigin, labels: ["at"], placement: FieldPlacement::Keyword, required: Required, multiplicity: SCALAR, shape: CaptureName, owner: OwnerApplicability::POLICY_ASSERTION, signature: ":at CAPTURE", description: "Name the subject capture whose value origin is asserted about." }
...
atom_values! {
    ...
    UnknownVerdictTreatMayAsFinding { domain: UnknownVerdict, spellings: ["treat-may-as-finding"], owner: OwnerApplicability::POLICY_ALL, description: "Publish explicit may evidence as possible findings with its reasons; unmet coverage obligations still make the run inconclusive. Flow and taint already retain possible findings under every verdict." }
    ...
    RowJoinSemi { domain: RowJoinKind, spellings: ["semi"], owner: OwnerApplicability::POLICY_ASSERTION, description: "Retain left rows that have at least one matching right-side row, without carrying the right relation columns." }
    ...
    RowAggregateMax { domain: RowAggregateOp, spellings: ["max"], owner: OwnerApplicability::POLICY_ASSERTION, description: "Compute the maximum integer value." }
    RowAggregateAny { domain: RowAggregateOp, spellings: ["any"], owner: OwnerApplicability::POLICY_ASSERTION, description: "Fold a boolean column to one when some contributing row is true, and to zero otherwise." }
    RowAggregateAll { domain: RowAggregateOp, spellings: ["all"], owner: OwnerApplicability::POLICY_ASSERTION, description: "Fold a boolean column to one when every contributing row is true, and to zero otherwise. A group with no contributing row folds to one." }
    RowAggregateSetEqual { domain: RowAggregateOp, spellings: ["set-equal"], owner: OwnerApplicability::POLICY_ASSERTION, description: "One when the distinct present left and right stable-key values are equal as sets, zero otherwise." }
    RowAggregateSubset { domain: RowAggregateOp, spellings: ["subset"], owner: OwnerApplicability::POLICY_ASSERTION, description: "One when every distinct present left stable-key value occurs on the right, zero otherwise." }
...
mod tests {
    ...
    fn applicability_rejects_fields_owned_by_another_analysis() {
        ...
        // Stores are policy-local only: the store entries slot is reachable in
        // exactly the store context, and neither composition field is.
        assert_eq!(
            lookup_field(PolicyRecord::Analysis, "stores")
                .unwrap()
                .child_context,
            PolicyRecordContext::TaintStores
        );
        ...
        let stores = lookup_field(PolicyRecord::Analysis, "stores").unwrap();
        assert!(
            stores
                .applicability
                .allows(RqlpDocumentKind::Policy, Some(PolicyAnalysisKind::Taint))
