...
fn overlay_record(
    ...
    metadata: &crate::evaluator::CvssOverlayEvidenceMetadata,
...
fn source_labels_match(
    projection: CvssFindingProjection<'_>,
    expected: &[crate::definition::TaintLabel],
    quantifier: AnyOrAll,
) -> bool {
    let contains = |label: &crate::definition::TaintLabel| {
        projection
            .taint_sources()
            .iter()
...
mod tests {
    ...
    use crate::finding_identity::{OpaqueFindingKey, SourceScenarioId};
    ...
    fn pair_facts_keep_predicates_hashes_and_scenarios_correlated() {
        ...
        let projection = TaintPolicyProjectionFacts::try_new(
            ...
            facts,
            &crate::budget::PolicyBudget::default(),
        )
        .unwrap();
