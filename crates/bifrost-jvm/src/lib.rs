...
//! a declaration written in any of them can be named from the other two. The
//! JVM source realm is the view that closes that gap, and it makes Kotlin's
//! import, type and hierarchy routes depend on Java's and Scala's declarations;
//! Java's forward scan depends on Scala syntax in the other direction. The
//! three languages also share one
...
//! downcasts that produce the arguments -- including the one that builds the
//! JVM source realm.
