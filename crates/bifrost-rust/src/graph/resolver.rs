...
use crate::usage_index::{
    usage_binding_local_names, usage_binding_seeds, usage_crate_export_targets,
...
pub fn rust_smallest_named_node_covering<'tree>(
...
pub enum RustGraphSeedKind {
    Export,
    LocalDeclaration,
}

pub struct RustGraphSeeds {
    pub roots: BTreeSet<CodeUnit>,
    pub kind: RustGraphSeedKind,
}
...
pub struct ResolvedRustTokenPathSegment<'tree> {
    pub node: Node<'tree>,
    pub path: Vec<Node<'tree>>,
    pub fqn: String,
    pub role: RustTokenPathRole,
}
...
pub fn rust_token_path_segment_is_qualified(node: Node<'_>) -> bool {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RustBareTokenTreeRole {
    Reference,
    TypeReference,
    Pattern,
    Binding,
...
pub fn rust_bare_token_tree_role(node: Node<'_>, source: &str) -> RustBareTokenTreeRole {
...
pub fn token_tree_ancestor(mut node: Node<'_>) -> Option<Node<'_>> {
...
fn infer_export_names_for_local(
    ...
    for (export_name, entry) in &index.exports_by_name {
        if matches!(entry, brokk_bifrost_core::analyzer::usages::model::ExportEntry::Local { local_name: name } if name == local_name)
        {
...
pub fn unresolved_external_frontier_specifiers(
    ...
    if let Some(brokk_bifrost_core::analyzer::usages::model::ExportEntry::ReexportedNamed {
        module_specifier,
        ..
