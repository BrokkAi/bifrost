...
function main(args) {
  ...
  if (command === "manifest") {
    const options = parseArgs(args, "node scripts/release-qualification.mjs manifest --bundle DIR --identity FILE --output FILE");
  ...
  if (command === "verify") {
    const options = parseArgs(args, "node scripts/release-qualification.mjs verify --bundle DIR --manifest FILE --repository OWNER/REPO --commit SHA --version VERSION [--run-id ID]");
  ...
  if (command === "select-run") {
    const options = parseArgs(args, "node scripts/release-qualification.mjs select-run --runs FILE --artifacts FILE --repository OWNER/REPO --commit SHA --version VERSION [--run-id ID]");
  ...
  throw new Error("Usage: node scripts/release-qualification.mjs <manifest|verify|select-run> ...");
}
