...
test("generated release metadata is exported without writing public source", () => {
  const metadataExport = jobBlock(release, "export-release-metadata");
  assert.match(metadataExport, /^      contents: read$/mu);
  assert.match(
    metadataExport,
    /ref: \$\{\{ needs\.release-context\.outputs\.commit \}\}/u,
  );
  assert.doesNotMatch(metadataExport, /ref: master/u);
  assert.match(
    metadataExport,
    /--manifest editors\/vscode\/package\.json/u,
  );
  assert.match(
    metadataExport,
  ...
  for (const forbidden of [
    ...
    assert.doesNotMatch(metadataExport, forbidden);
  }
  assert.doesNotMatch(release, /\bgit commit\b/u);
  assert.doesNotMatch(release, /\bgit push\b/u);
});
...
test("an always-run summary names targets and safe retry guidance", () => {
  ...
  assert.match(summary, /^      - export-release-metadata$/mu);
  assert.match(
    summary,
    /METADATA_EXPORT: \$\{\{ needs\.export-release-metadata\.result \}\}/u,
  );
  assert.doesNotMatch(summary, /METADATA_SYNC/u);
