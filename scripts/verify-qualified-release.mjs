...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    if (!option?.startsWith("--") || value === undefined || value.startsWith("--")) {
      throw new Error("Usage: node scripts/verify-qualified-release.mjs verify --bundle DIR --manifest FILE --repository OWNER/REPO --commit SHA --version VERSION [options]");
    }
...
export function main(args = process.argv.slice(2)) {
  ...
  if (command !== "verify") throw new Error("Usage: node scripts/verify-qualified-release.mjs verify ...");
