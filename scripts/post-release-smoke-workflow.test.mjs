...
test("is manually resumable and reusable after promotion with an exact tag", () => {
  ...
  assert.ok(
    workflow.includes(
      'node scripts/release-version.mjs check --tag "$RELEASE_TAG" --github-output "$GITHUB_OUTPUT"',
    ),
  );
  assert.doesNotMatch(workflow, /RELEASE_TAG.*=~/u);
  const checkout = workflow.indexOf("uses: actions/checkout@");
  const validator = workflow.indexOf("node scripts/release-version.mjs check");
...
test("invokes published Codex and Claude MCP list_policies smoke checks", () => {
  assert.match(workflow, /scripts\/smoke-agent-plugin-release\.mjs/u);
