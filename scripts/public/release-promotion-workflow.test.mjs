...
test("uv CLI package exposes bifrost through its package name", () => {
  ...
  assert.match(wheelBuilder, /node scripts\/public\/prepare-uv-cli-package\.mjs/u);
});
...
test("qualified crates use the exact crates.io archive publisher", () => {
  ...
  assert.match(combined, /scripts\/public\/publish-qualified-crate\.mjs/u);
...
test("agent plugin validation remains in post-release smoke and release assets are immutable", () => {
  assert.match(postReleaseSmoke, /scripts\/public\/smoke-agent-plugin-release\.mjs/u);
  ...
  assert.match(contributing, /scripts\/public\/smoke-agent-plugin-release\.mjs/u);
});
...
test("npm dispatch is deterministically correlated with its child run", () => {
  ...
  assert.match(
    npmDispatch,
    /gh workflow run publish-npm\.yml[\s\S]{0,160}--ref master/u,
    "release recovery must execute the fixed npm workflow from public master",
  );
  ...
  assert.match(npmDispatch, /--event workflow_dispatch --branch master/u);
  assert.match(npmDispatch, /\.displayTitle == \$title/u);
  assert.doesNotMatch(npmDispatch, /\.headSha == \$commit/u);
  assert.match(
    publishNpm,
...
readinessTest("release readiness creates and verifies a retained commit/version qualification bundle", () => {
  ...
  assert.match(readiness, /node\s+scripts\/public\/release-qualification\.mjs\s+manifest/iu);
  assert.match(readiness, /node\s+scripts\/public\/release-qualification\.mjs\s+verify/iu);
