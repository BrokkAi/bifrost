...
const USAGE = "Usage: node scripts/public/release-preflight.mjs --input FILE";
...
function validateVersionCheck(input, errors, validTag) {
  ...
  if (validTag && Array.isArray(check.command)) {
    const expected = ["node", "scripts/public/release-version.mjs", "check", "--tag", input.tag];
    if (check.command.length !== expected.length || check.command.some((part, index) => part !== expected[index])) {
      error(errors, "Version check command must exactly be node scripts/public/release-version.mjs check --tag <tag>.");
    }
  }
}
