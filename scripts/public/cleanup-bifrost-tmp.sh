...
usage() {
  cat >&2 <<'EOF'
Usage: scripts/public/cleanup-bifrost-tmp.sh [--apply] [--include-unmanaged] [--older-than-hours N] [--tmp-root PATH]
...
# Compare by length first, then lexically among equal-length digit strings.
# That is a correct integer comparison for leading-zero-stripped input of any
# size, and unlike -gt it cannot overflow on a caller-supplied number.
# shellcheck disable=SC2071 # the string comparison is the point; see above
