...
Usage:
  scripts/public/mcp-replay.py --binary target/release/bifrost --workspace . \
      [--scenario NAME ...] [--warm] \
...
Set MCP_REPLAY_EXPECT_COMPLETE=true|false to assert the post-change
`complete` and `incomplete` fields in the i1411 scenario; leave it unset for a
before-change baseline that predates those fields.
...
def scenario_i1411(c):
    """Exercise exact, ordinary, mixed, and bounded self-workspace batches.

    These selectors are canonical names returned by search_symbols on the
    Bifrost self-workspace. Keep the larger batch explicit and bounded so this
    remains a reproducible request rather than a workspace-wide glob.
    """
    exact_fqn = "brokk_bifrost_analysis.searchtools.sources.get_symbol_sources"
    ordinary_symbols = [
        exact_fqn,
        "brokk_bifrost_analysis.searchtools.sources.get_symbol_sources_with_source_budget",
        "brokk_bifrost_analysis.searchtools.navigation.SymbolLookupParams",
        "brokk_bifrost_mcp.searchtools_service.SearchToolsService.handle_get_symbol_sources",
    ]
    missing_symbols = [
        "brokk_bifrost_analysis.searchtools.sources.__mcp_replay_missing_i1411_a",
        "brokk_bifrost_analysis.searchtools.sources.__mcp_replay_missing_i1411_b",
    ]
    larger_symbols = [
        "brokk_bifrost_analysis.searchtools.sources.get_symbol_sources",
        "brokk_bifrost_analysis.searchtools.sources.get_symbol_sources_with_source_budget",
        "brokk_bifrost_analysis.searchtools.sources.get_symbol_sources_with_budget",
        "brokk_bifrost_analysis.searchtools.sources.SymbolSourcesResult",
        "brokk_bifrost_analysis.searchtools.sources.SourceBlock",
        "brokk_bifrost_analysis.searchtools.navigation.SymbolLookupParams",
        "brokk_bifrost_analysis.searchtools.selectors.resolve_selectable_definitions_bounded",
        "brokk_bifrost_analysis.analyzer.semantic.ids.SemanticLocator",
        "brokk_bifrost_analysis.analyzer.semantic.ids.SemanticLocator.new",
        "brokk_bifrost_analysis.analyzer.i_analyzer.AnalyzerQueryScope",
        "brokk_bifrost_analysis.analyzer.semantic.oracle.model.ScopedSemanticLocator",
        "brokk_bifrost_analysis.analyzer.semantic.oracle.model.ScopedSemanticLocator.new",
        "brokk_bifrost_mcp.searchtools_service.SearchToolsService.handle_get_symbol_sources",
        "brokk_bifrost_mcp.searchtools_service.SearchToolsService.symbol_sources_output",
        "brokk_bifrost_mcp.searchtools_service.SearchToolsService.symbol_sources_budget_error",
        "brokk_bifrost_analysis.searchtools.sources.preferred_source_blocks_for_resolved_units_with_budget",
    ]

    def structured_counts(res):
        """Return source/missing counts and missing inputs from MCP JSON."""
        result = res.get("result", {})
        structured = result.get("structuredContent")
        if not isinstance(structured, dict):
            return None
        missing = []
        for item in structured.get("not_found", []):
        ...
        return {
            ...
            "not_found": missing,
            "ambiguous": len(structured.get("ambiguous", [])),
            "too_broad": len(structured.get("too_broad", [])),
            "complete": structured.get("complete", None),
            "has_complete": "complete" in structured,
            "incomplete": structured.get("incomplete", None),
            "has_incomplete": "incomplete" in structured,
        }

    expected_complete = os.environ.get("MCP_REPLAY_EXPECT_COMPLETE")
    if expected_complete is not None:
        expected_complete = expected_complete.strip().lower()
        if expected_complete not in ("true", "false"):
            raise ValueError(
                "MCP_REPLAY_EXPECT_COMPLETE must be true or false when set"
            )
        expected_complete = expected_complete == "true"
    ...
    def run_case(
        ...
        budget_ms,
        expected_missing=(),
        reject_ambiguous_or_broad=False,
        timeout=180.0,
    ):
        args = {"symbols": symbols}
        res = c.call_tool("get_symbol_sources", args, timeout=timeout)
        kind, detail = outcome(res)
        ms = res.get("_ms", 0)
        stats = structured_counts(res)
        ok = kind == "OK" and ms <= budget_ms
        if stats is None:
            ok = False
            evidence = "structuredContent=unavailable <-- FAIL"
        else:
            missing = stats["not_found"]
            expected = set(expected_missing)
            ok = ok and stats["sources"] > 0
            if expected:
                ok = ok and set(missing) == expected
            else:
                ok = ok and not missing
            if reject_ambiguous_or_broad:
                ok = ok and stats["ambiguous"] == 0 and stats["too_broad"] == 0
            if expected_complete is not None:
                ok = ok and (
                    stats["has_complete"]
                    and stats["complete"] is expected_complete
                    and stats["has_incomplete"]
                )
                if expected_complete:
                    ok = ok and stats["incomplete"] == []
            evidence = (
                f"sources={stats['sources']} not_found={len(missing)} "
                f"ambiguous={stats['ambiguous']} too_broad={stats['too_broad']} "
                f"complete={stats['complete'] if stats['has_complete'] else '<absent>'} "
                f"incomplete={stats['incomplete'] if stats['has_incomplete'] else '<absent>'}"
            )
            if expected:
                evidence += f" missing_inputs={missing}"
            if expected_complete is not None:
                evidence += f" expected_complete={expected_complete}"
        flag = "" if ok else " <-- FAIL"
        print(
            f"  [i1411/{label}] get_symbol_sources {ms:8.0f} ms  "
            f"{kind:20s} selectors={len(symbols)} {evidence} "
            f"{detail[:100]}{flag}"
        )
        return ok

    ok = run_case("exact-fqn", [exact_fqn], budget_ms=6000)
    ok = run_case(
        "ordinary-four",
        ordinary_symbols,
        budget_ms=10000,
        reject_ambiguous_or_broad=True,
    ) and ok
    ok = run_case(
        "mixed-valid-missing",
        [ordinary_symbols[0], ordinary_symbols[1], *missing_symbols],
        budget_ms=10000,
        expected_missing=missing_symbols,
    ) and ok
    ok = run_case(
        "self-repo-bounded-16",
        larger_symbols,
        budget_ms=30000,
        reject_ambiguous_or_broad=True,
    ) and ok
...
def scenario_i2464(c):
    """Two exact Rust FQNs from the issue #2464 warm-session report."""
    calls = [("get_symbol_sources", {"symbols": [
        "brokk_bifrost_csharp::syntax::csharp_member_name",
        "brokk_bifrost_csharp::syntax::csharp_conditional_member_access",
    ]})]
    ok = True
    for iteration in range(6):
        results = c.call_tools_parallel(calls)
        ok = report(f"i2464-{iteration}", results, calls) and ok
    return ok
...
def emit_stderr_capture(client):
    """Preserve server stderr even when replay setup or a scenario raises."""
    try:
        client.err_reader.join(timeout=1)
    except Exception:
        pass
    ...
    if stderr_path:
        try:
            ...
            # Never replace the replay failure with a diagnostics-write error.
            print(
                f"== failed to write stderr to {stderr_path}: {error}",
                file=sys.stderr,
            )
        else:
...
def main():
    ...
    try:
        ...
        replay_exception = sys.exc_info()[1]
        close_error = None
        try:
            client.close()
        except Exception as error:
            close_error = error
            # Do not mask a warmup/scenario exception while shutting down.
            print(f"== client close failed: {error}", file=sys.stderr)
        emit_stderr_capture(client)
        if close_error is not None and replay_exception is None:
            raise close_error
