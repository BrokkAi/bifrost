...
// The complete set of tracked paths a checksum correction may touch. The
// workflow gates that refuse a correction reaching anywhere else read this
// through the `paths` command rather than restating it, because a hand-copied
// allowlist that falls behind this one either blocks a legitimate correction or
// waves through a change that was never checksum-only.
export const CHECKSUM_CORRECTION_PATHS = [
...
function main(args) {
  if (args[0] === "paths") {
    if (args.length > 1) {
      throw new Error("The paths command takes no options.");
    }
    console.log(CHECKSUM_CORRECTION_PATHS.join("\n"));
    return;
  }
