...
function validInput() {
  ...
  return {
    ...
    versionCheck: {
      command: ["node", "scripts/release-version.mjs", "check", "--tag", "v0.10.3"],
      exitCode: 0,
      version: "0.10.3",
      tag: "v0.10.3",
    },
...
test("requires the exact successful version check command and evidence", () => {
  const input = validInput();
  input.versionCheck.command = ["node", "scripts/release-version.mjs", "sync"];
...
test("accepts only the input-file CLI shape", () => {
  ...
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Usage: node scripts\/release-preflight\.mjs --input FILE/u);
});
